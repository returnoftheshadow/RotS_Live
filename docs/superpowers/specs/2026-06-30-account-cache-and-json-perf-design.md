...
- **Compiler floor = g++ 9.4.0 (live runtime); the Docker build image is g++ 10 (Debian bullseye).** New code must compile and run on **g++ 9.4.0**. This is satisfied: the design uses only the **integer** overloads of `std::from_chars`/`std::to_chars`, which libstdc++ has supported since **GCC 8.1** and implements **inline in `<charconv>`** (no compiled-library symbol, so no runtime `libstdc++.so` dependency even if built on g++ 10 and run on a g++ 9.4 box). **Guardrail: never use floating-point `from_chars`/`to_chars`** (added only in GCC 11) — all character JSON numeric fields are `int`/`long` (verified: no `double`/`float` in `character_json.{h,cpp}`), so the integer path is always sufficient. No other feature in the design exceeds g++ 9.4.0 (`string_view`/`unordered_map`/thread-safe function-local `static` are all ≤ GCC 7–8).
