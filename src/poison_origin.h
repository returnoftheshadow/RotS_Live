...
struct poison_origin {
    ...
    const char_data* identity; // the poisoner's address when recorded; compared, never dereferenced
...
[[nodiscard]] char_data* resolve_poisoner(const char_data& victim);
// Records `poisoner` as the source of `victim`'s poison. A null `poisoner` clears the record.
// `poisoner` is not retained beyond its identity and is never dereferenced later. A null `victim`
// logs a SYSERR and changes nothing.
void record_poison_origin(char_data* victim, const char_data* poisoner);
// Clears `victim`'s poison record. A null `victim` logs a SYSERR and changes nothing.
