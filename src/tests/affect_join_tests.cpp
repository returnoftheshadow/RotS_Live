...
TEST(GetAffectUnbounded, ReturnsTheNewestMatchOrNullptr)
{
    ...
    affected_type older_poison = test_support::inert_affect(SPELL_POISON, 20);
    affect_to_char(&victim, &older_poison);
    affected_type newer_poison = test_support::inert_affect(SPELL_POISON, 5);
...
TEST(AffectJoin, MergesIntoAPoisonJustPastTheCappedFinder)
{
    ...
    test_support::add_filler_affects(victim, MAX_AFFECT);
    ASSERT_EQ(affected_by_spell(&victim, SPELL_POISON), nullptr) << "the capped finder must miss the first poison";

    affected_type second_poison = test_support::inert_affect(SPELL_POISON, 5);
    affect_join(&victim, &second_poison, FALSE, FALSE);

    EXPECT_EQ(test_support::count_affects_of_type(victim, SPELL_POISON), 1) << "the second poison must merge, not sit beside the first";
...
TEST(AffectJoin, MergesIntoAPoisonBuriedFarPastTheCap)
{
    ...
    EXPECT_EQ(test_support::count_affects_of_type(victim, SPELL_POISON), 1) << "the buried poison must be removed, not left beside the new one";
    ASSERT_EQ(victim.affected->type, SPELL_POISON);
...
{
    affected_type resist_magic = test_support::inert_affect(SPELL_RESIST_MAGIC, 20);
...
TEST(AffectRemove, UnlinksAndStripsAnAffectBuriedPastMaxAffectEntries)
{
    ...
    EXPECT_EQ(test_support::count_affects_of_type(victim, SPELL_RESIST_MAGIC), 0);
...
TEST(AffectRemove, LeavesStatsAndFlagsAloneForAnAffectNotOnTheList)
{
    ...
    EXPECT_EQ(test_support::count_affects_of_type(victim, SPELL_RESIST_MAGIC), 1);
...
TEST(AffectJoin, AddsTheAffectWhenNoneOfItsTypeIsPresent)
{
    ...
    affected_type poison = test_support::inert_affect(SPELL_POISON, 5);
    affect_join(&victim, &poison, FALSE, FALSE);

    EXPECT_EQ(test_support::count_affects_of_type(victim, SPELL_POISON), 1);
    ASSERT_EQ(victim.affected->type, SPELL_POISON);
