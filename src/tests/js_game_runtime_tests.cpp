...
TEST(JsGameRuntime, ExecutesFirstTextSettersThroughMutationResults) {
    ...
    JsRuntimeEvalResult result = runtime.evaluate_trigger_body(
        ...
        "  'roomTrack', 'bleedTrack', 'bfsDir', 'bfsNextRaw', 'specialProc', 'special'\n"
        "];\n"
        "const forbiddenZoneMembers = [\n"
        "  'setMinimumLookLevel', 'setAge', 'setTopRoomVnum', 'setVnum',\n"
        "  'setWhitePower', 'setDarkPower', 'setMagiPower', 'setShortDescriptions',\n"
        ...
        "  'minLevelLook', 'minimum_level_look', 'setOwnerIds', 'setCmd', 'setCmdno'\n"
        "];\n"
        "const forbiddenObjectMembers = [\n"
        "  'setVnum', 'setOwnerId', 'setNextContent', 'setNext', 'setLoadedBy',\n"
        "  'ownerId', 'nextContent', 'next', 'loadedBy', 'item_number', 'in_room',\n"
        "  'obj_flags', 'affected', 'short_description', 'action_description',\n"
        "  'ex_description', 'carried_by', 'owner', 'in_obj', 'contains',\n"
        "  'next_content', 'loaded_by'\n"
        "];\n"
        "const forbiddenCharacterMembers = [\n"
        "  'internalIndex', 'playerIndex', 'descriptor', 'nextInRoom', 'next',\n"
        "  'nextFighting', 'nextFastUpdate', 'masterNumber', 'temporaryData',\n"
        "  'delay', 'nextDying', 'group', 'setInternalIndex', 'setPlayerIndex',\n"
        "  'setDescriptor', 'setNextInRoom', 'setNext', 'setNextFighting',\n"
        "  'setNextFastUpdate', 'setMasterNumber', 'setTemporaryData', 'setDelay',\n"
        "  'setNextDying', 'abs_number', 'player_index', 'nr', 'in_room',\n"
        "  'player', 'abilities', 'tmpabilities', 'constabilities', 'profs',\n"
        "  'extra_specialization_data', 'damage_details', 'affected', 'carrying',\n"
        "  'desc', 'next_in_room', 'next_fighting', 'next_fast_update',\n"
        "  'master_number', 'mount_data', 'temp', 'next_die', 'classpoints',\n"
        "  'interrupt_count', 'interrupt_time', 'spec_busy'\n"
        "];\n"
        ...
        "  && [ctx.object, ctx.weapon, ctx.target, ctx.targ1, ctx.targ2].filter(function(role) {\n"
        "    return role && typeof role === 'object' && 'flags' in role;\n"
        "  }).every(function(object) {\n"
        "    return forbiddenObjectMembers.every(function(name) {\n"
        ...
        "  && [ctx.actor, ctx.self, ctx.speaker, ctx.attacker, ctx.victim, ctx.killer,\n"
        "      ctx.target, ctx.targ1, ctx.targ2, ctx.dying].filter(function(role) {\n"
        "    return role && typeof role === 'object' && 'profile' in role;\n"
        "  }).every(function(character) {\n"
        "    return forbiddenCharacterMembers.every(function(name) {\n"
        "      return typeof character[name] === 'undefined';\n"
        "    });\n"
        "  })\n"
        "  && [ctx.object.room, ctx.actor.room, ctx.room].every(function(room) {\n"
        "    return forbiddenRoomMembers.every(function(name) {\n"
        "      return typeof room[name] === 'undefined';\n"
        "    });\n"
        "  })\n"
        "  && [ctx.room.zone, ctx.zone, ctx.object.room.zone, ctx.actor.room.zone,\n"
        "      ctx.weapon.room.zone, ctx.self.room.zone].every(function(zone) {\n"
        "    return forbiddenZoneMembers.every(function(name) {\n"
        "      return typeof zone[name] === 'undefined';\n"
        "    });\n"
        "  })\n"
        ...
        "  && typeof ctx.object.room.setAlignment === 'undefined'\n"
        "  && typeof ctx.object.room.setFlags === 'undefined'\n"
        "  && typeof ctx.object.room.setLight === 'undefined'\n"
        "  && typeof ctx.object.room.tracks === 'undefined'\n"
        "  && typeof ctx.object.room.bleedTracks === 'undefined'\n"
        "  && typeof ctx.object.room.bfsDirection === 'undefined'\n"
        "  && typeof ctx.object.room.bfsNext === 'undefined'\n"
        "  && typeof ctx.object.room.specialProcedure === 'undefined'\n"
        ...
        "  && typeof ctx.actor.room.setAlignment === 'undefined'\n"
        "  && typeof ctx.actor.room.setFlags === 'undefined'\n"
        "  && typeof ctx.actor.room.setLight === 'undefined'\n"
        "  && typeof ctx.actor.room.tracks === 'undefined'\n"
        "  && typeof ctx.actor.room.bleedTracks === 'undefined'\n"
        "  && typeof ctx.actor.room.bfsDirection === 'undefined'\n"
        "  && typeof ctx.actor.room.bfsNext === 'undefined'\n"
        "  && typeof ctx.actor.room.specialProcedure === 'undefined'\n"
        ...
        "  && typeof ctx.room.setAlignment === 'undefined'\n"
        "  && typeof ctx.room.setFlags === 'undefined'\n"
        "  && typeof ctx.room.setLight === 'undefined'\n"
        "  && typeof ctx.room.tracks === 'undefined'\n"
        "  && typeof ctx.room.bleedTracks === 'undefined'\n"
        "  && typeof ctx.room.bfsDirection === 'undefined'\n"
        "  && typeof ctx.room.bfsNext === 'undefined'\n"
        "  && typeof ctx.room.specialProcedure === 'undefined'\n"
...
TEST(JsGameRuntime, ExposesTypedTargetsWhenPresent) {
    ...
    JsRuntimeEvalResult result = runtime.evaluate_trigger_body(
        ...
        "  'setSpecialProcedure', 'tracks', 'bleedTracks', 'bfsDirection', 'bfsNext',\n"
        "  'specialProcedure', 'room_track', 'bleed_track', 'bfs_dir', 'bfs_next',\n"
        "  'funct', 'roomTrack', 'bleedTrack', 'bfsDir', 'bfsNextRaw', 'specialProc',\n"
        "  'special'\n"
        "];\n"
        ...
        "  && roomInternalNames.every(function(name) {\n"
