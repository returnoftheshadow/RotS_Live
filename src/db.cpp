  ...
  for (nr = 0; nr <= top_of_p_table; nr++) {
    if (player_table[nr].level < 20 &&
        ...
        ((tt - player_table[nr].log_time) >
         SECS_PER_REAL_DAY * player_table[nr].level * 7) &&
        (number(0, 100) < 51)) {
...
struct char_data *read_mobile(int nr, int type) {
  ...
  *mob = mob_proto[i];
  ...
  mob->specials2.rawPerception = mob->specials2.perception =
      get_naked_perception(mob);
...
void record_crime(char_data *criminal, char_data *victim, int crime,
  ...
  for (tmpchar = world[victim->in_room].people; tmpchar;
    ...
    if ((tmpchar == criminal) || (IS_NPC(tmpchar)) ||
        (GET_LEVEL(tmpchar) >= LEVEL_IMMORT))
      continue;
...
int know_of_crime(int criminal, int victim, int witness) {
  ...
  for (tmp = 0; tmp < num_of_crimes; tmp++)
    if ((criminal == crime_record[tmp].criminal) &&
        (victim == crime_record[tmp].victim) &&
...
void forget_crimes(char_data *ch, int criminal) {
  ...
  if (criminal == -1) // -1 is forget all crimes witnessed
    for (tmp = 0; tmp < num_of_crimes; tmp++)
      if (crime_record[tmp].witness ==
          find_player_in_table("", ch->specials2.idnum))
        count = 1;
  if (!(criminal == -1))
    for (tmp = 0; tmp < num_of_crimes; tmp++)
      if ((crime_record[tmp].witness ==
  ...
  for (tmp = 0; tmp < num_of_crimes; tmp++) {
    if (criminal == -1) // -1 is forget all - player has died etc
      not_write = !(crime_record[tmp].witness ==
                    find_player_in_table("", ch->specials2.idnum));
    else
      not_write =
          !((crime_record[tmp].witness ==
    ...
    if (not_write) {
      ...
      tmprecord[0].criminal =
          (player_table + crime_record[tmp].criminal)->idnum;
  ...
  case EXPLOIT_PK: {
    ...
    for (killer = combat_list; killer; killer = killer->next_fighting) {
      if (killer->specials.fighting == victim) {
        ...
        if (IS_NPC(killer)) {
          if (killer->master && (MOB_FLAGGED(killer, MOB_PET) ||
                                 MOB_FLAGGED(killer, MOB_ORC_FRIEND))) {
            cur_killer = killer->master;
          }
        }

        // If we have a killer and he's unique, add it to the exploits.
        if (cur_killer && !IS_NPC(cur_killer) &&
            seen_chars.insert(cur_killer).second) {
