...
bool parse_mutation(JsonReader* reader, JsRuntimeMutation* mutation, std::string* error_message)
{
    ...
    return reader->parse_object(
               [&](const std::string& name, JsonReader* nested_reader, std::string* nested_error) {
                   ...
                   if (name == "runtimeHelperBridgeAccepted")
                       return nested_reader->parse_bool(&mutation->runtime_helper_bridge_accepted,
                           nested_error);
    ...
    const JsGameRuntimeEvaluationOptions& evaluation_options = {})
{
    ...
    wrapped
        ...
        << "const __rotsRoomFlagTargetZoneVnum = "
        << evaluation_options.room_flag_target_zone_vnum << ";\n"
        ...
        << "const __rotsLiveHandleTypes = new WeakMap();\n"
        << "function __rotsDeepFreeze(value) {\n"
        << "  if (value && (typeof value === 'object' || typeof value === 'function') && "
           "!Object.isFrozen(value)) {\n"
        << "    const __rotsWasReadOnly = value.__rotsReadOnlySnapshot === true;\n"
        << "    const __rotsHandleType = typeof value.__rotsHandleType === 'string' ? "
           "value.__rotsHandleType : null;\n"
        ...
        << "}\n"
        << "function __rotsValidateRoomFlagHelperName(value) {\n"
        << "  const flags = ['dark','noMob','indoors','noRide','shadowy','noMagic','tunnel',"
           "'drinkWater','drinkPoison','peaceRoom','hideVnum'];\n"
        << "  const adminOnlyFlags = ['death','private','godRoom','securityRoom','noTeleport'];\n"
        << "  if (typeof value !== 'string') return __rotsMutationResult(false, 'invalid-value', "
           "'Expected room flag name.', 'name');\n"
        << "  if (adminOnlyFlags.indexOf(value) !== -1) return __rotsMutationResult(false, "
           "'authority-rejected', 'Room flag requires reviewed admin override authority.', "
           "'name');\n"
        << "  if (flags.indexOf(value) === -1) return __rotsMutationResult(false, 'invalid-value', "
           "'Room flag must be a supported MutableRoomFlagName value.', 'name');\n"
        << "  return __rotsMutationResult(true, 'ok', null, 'name');\n"
        << "}\n"
        << "function __rotsRoomFlagTargetToken(handle) {\n"
        << "  if (!handle || typeof handle !== 'object' || typeof handle.vnum !== 'number' || "
           "!__rotsNumberIsInteger(handle.vnum)) return null;\n"
        << "  return 'room-token:v1:' + __rotsString(__rotsRoomFlagTargetZoneVnum) + ':' + "
           "__rotsString(handle.vnum);\n"
        << "}\n"
        << "function __rotsCanonicalRoomFlags(flags) {\n"
        << "  const order = ['dark','death','noMob','indoors','noRide','permanentAffect',"
           "'shadowy','noMagic','tunnel','private','godRoom','drinkWater','drinkPoison',"
           "'securityRoom','peaceRoom','noTeleport','hideVnum'];\n"
        << "  const current = Array.isArray(flags) ? flags : [];\n"
        << "  return Object.freeze(order.filter(function(flag) { return current.indexOf(flag) !== "
           "-1; }));\n"
        << "}\n"
        ...
        << "  });\n"
        << "}\n"
        << "function __rotsAttachRoomFlagHelpers(handle) {\n"
        << "  if (!handle || typeof handle !== 'object') return;\n"
        << "  let currentFlags = __rotsCanonicalRoomFlags(handle.flags);\n"
        << "  Object.defineProperty(handle, 'flags', {\n"
        << "    enumerable: true,\n"
        << "    configurable: true,\n"
        << "    get: function() { return currentFlags; }\n"
        << "  });\n"
        << "  const applyFlag = function(name, add) {\n"
        << "    const validation = __rotsValidateRoomFlagHelperName(name);\n"
        << "    if (!validation.ok) return validation;\n"
        << "    const token = __rotsRoomFlagTargetToken(handle);\n"
        << "    if (token === null) return __rotsMutationResult(false, 'not-authorized', null, "
           "'room');\n"
        << "    const next = currentFlags.filter(function(flag) { return flag !== name; });\n"
        << "    currentFlags = Object.freeze(add ? next.concat([name]) : next);\n"
        << "    currentFlags = __rotsCanonicalRoomFlags(currentFlags);\n"
        << "    __rotsMutations.push({ kind: 'helper', operation: add ? 'room.flags.add' : "
           "'room.flags.remove', targetToken: token, argumentsJson: __rotsJsonStringify.call(JSON, "
           "{ flag: name }), runtimeHelperBridgeAccepted: true });\n"
        << "    return __rotsMutationResult(true, 'ok', null, add ? 'room.addFlag' : "
           "'room.removeFlag');\n"
        << "  };\n"
        << "  Object.defineProperty(handle, 'addFlag', {\n"
        << "    enumerable: false,\n"
        << "    configurable: true,\n"
        << "    writable: false,\n"
        << "    value: function(name) { return applyFlag(name, true); }\n"
        << "  });\n"
        << "  Object.defineProperty(handle, 'removeFlag', {\n"
        << "    enumerable: false,\n"
        << "    configurable: true,\n"
        << "    writable: false,\n"
        << "    value: function(name) { return applyFlag(name, false); }\n"
        << "  });\n"
        << "}\n"
        ...
        << "    __rotsAttachRoomFlagHelpers(value);\n"
        << "  }\n"
        ...
        << "}\n"
        << "function __rotsCommandMutationResult(operation, args) {\n"
        << "  __rotsMutations.push({ kind: 'command', operation: operation, argumentsJson: "
        ...
        << "  return code === 'ok' || code === 'invalid-value' || code === 'out-of-range' || "
           "code === 'not-authorized' || code === 'authority-rejected' || code === "
           "'stale-handle' || code === 'unsupported' || code === 'deferred' || code === "
           "'invalid-target' || code === 'not-carried' || "
           "code === 'no-drop' || code === 'inventory-full' || code === 'too-heavy' || "
           "code === 'audit-rejected' || code === 'not-found' || code === 'already-waiting' || "
           "code === 'no-recipient' || code === 'blocked-room' || code === 'no-teleport';\n"
        << "}\n"
        ...
        << "  if (typeof value !== 'string') return __rotsMutationResult(false, 'invalid-value', "
           "'Expected text.', field);\n"
        << "  if (value.indexOf('\\u0000') !== -1 || /[\\r\\n]/.test(value)) return "
           "__rotsMutationResult(false, 'invalid-value', 'Text must be a single safe line.', "
           "field);\n"
        << "  if (value.length === 0) return __rotsMutationResult(false, 'invalid-value', 'Text "
           "must not be empty.', field);\n"
        << "  if (value.length > 512) return __rotsMutationResult(false, 'out-of-range', 'Text is "
           "too long.', field);\n"
        << "  return __rotsMutationResult(true, 'ok', null, field);\n"
        << "}\n"
        << "function __rotsValidateCommandInteger(value, field, min, max) {\n"
        << "  if (typeof value !== 'number' || !__rotsNumberIsInteger(value)) return "
           "__rotsMutationResult(false, 'invalid-value', 'Expected integer.', field);\n"
        << "  if (value < min || value > max) return __rotsMutationResult(false, 'out-of-range', "
           "'Integer is outside the supported range.', field);\n"
        << "  return __rotsMutationResult(true, 'ok', null, field);\n"
        << "}\n"
        << "function __rotsHandleId(value, expectedType, field) {\n"
        << "  if (!value || typeof value !== 'object' || typeof value.id !== 'string' || "
           "typeof value.isValid !== 'function' || value.isValid() !== true || "
           "__rotsLiveHandleTypes.get(value) !== expectedType) return null;\n"
        ...
        << "    return value.id.indexOf('character:') === 0 || value.id.indexOf('char:') === 0 || "
           ...
           "value.id === 'dying' || value.id === 'target' || value.id === 'targ1' || "
           "value.id === 'targ2' ? value.id : null;\n"
        << "  }\n"
        << "  if (expectedType === 'object' && (value.id === 'object' || value.id === 'weapon' || "
           "value.id === 'target' || value.id === 'targ1' || value.id === 'targ2')) return "
           "value.id;\n"
        << "  if (expectedType === 'room' && (value.id === 'room' || value.id === 'target' || "
           "value.id === 'targ1' || value.id === 'targ2')) return value.id;\n"
        << "  if (value.id.indexOf(expectedType + ':') !== 0) return null;\n"
        << "  return value.id;\n"
        << "}\n"
        << "function __rotsDoWait(pulses) {\n"
        << "  const result = __rotsValidateCommandInteger(pulses, 'pulses', 1, 10000);\n"
        << "  if (!result.ok) return result;\n"
        << "  const args = { pulses: pulses };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.do_wait', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.do_wait', args) : "
           "bridged;\n"
        << "}\n"
        ...
        << "  const bridged = __rotsCommandBridgeMutationResult('script.do_say', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.do_say', args) : "
           "bridged;\n"
        << "}\n"
        << "function __rotsSendToChar(target, text) {\n"
        << "  const targetId = __rotsHandleId(target, 'character', 'target');\n"
        << "  if (targetId === null) return __rotsMutationResult(false, 'invalid-value', 'Expected "
           "a live character handle.', 'target');\n"
        << "  const result = __rotsValidateCommandText(text, 'text');\n"
        << "  if (!result.ok) return result;\n"
        << "  const args = { targetId: targetId, text: text };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.send_to_char', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.send_to_char', args) : "
           "bridged;\n"
        << "}\n"
        << "function __rotsSendToRoom(room, text) {\n"
        << "  const roomId = __rotsHandleId(room, 'room', 'room');\n"
        << "  if (roomId === null) return __rotsMutationResult(false, 'invalid-value', 'Expected a "
           "live room handle.', 'room');\n"
        << "  const result = __rotsValidateCommandText(text, 'text');\n"
        << "  if (!result.ok) return result;\n"
        << "  const args = { roomId: roomId, text: text };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.send_to_room', args);\n"
        ...
        << "  if (!result.ok) return result;\n"
        << "  if (arguments.length < 2 || target === undefined || target === null) {\n"
        << "    const args = { vnum: vnum };\n"
        << "    const bridged = __rotsCommandBridgeMutationResult('script.load_obj', args);\n"
        << "    return bridged === null ? __rotsCommandMutationResult('script.load_obj', args) : "
           "bridged;\n"
        << "  }\n"
        ...
        << "  if (loadTargetId === null) return __rotsMutationResult(false, 'invalid-target', "
           "'Expected a live character or room handle.', 'target');\n"
        << "  const args = { vnum: vnum, loadTargetId: loadTargetId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.load_obj', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.load_obj', args) : "
           "bridged;\n"
        << "}\n"
        ...
        << "  return bridged === null ? __rotsCommandMutationResult('script.do_give', args) : "
           "bridged;\n"
        << "}\n"
        << "function __rotsMoveObject(object, target) {\n"
        << "  const objectId = __rotsHandleId(object, 'object', 'object');\n"
        << "  const characterId = __rotsHandleId(target, 'character', 'target');\n"
        << "  const roomId = characterId === null ? __rotsHandleId(target, 'room', 'target') : "
           "null;\n"
        << "  const moveTargetId = characterId !== null ? characterId : roomId;\n"
        << "  if (objectId === null || moveTargetId === null) return __rotsMutationResult(false, "
           "'invalid-target', 'Expected live object and character or room handles.', 'target');\n"
        << "  const args = { objectId: objectId, moveTargetId: moveTargetId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.move_object', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.move_object', args) : "
           "bridged;\n"
        << "}\n"
        << "function __rotsTeleportChar(character, room) {\n"
        << "  const characterId = __rotsHandleId(character, 'character', 'character');\n"
        << "  const roomId = __rotsHandleId(room, 'room', 'room');\n"
        << "  if (characterId === null || roomId === null) return __rotsMutationResult(false, "
           "'invalid-target', 'Expected live character and room handles.', 'target');\n"
        << "  const args = { characterId: characterId, roomId: roomId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.teleport_char', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.teleport_char', args) "
           ": bridged;\n"
        << "}\n"
        << "function __rotsTeleportCharOnly(character, room) {\n"
        << "  const characterId = __rotsHandleId(character, 'character', 'character');\n"
        << "  const roomId = __rotsHandleId(room, 'room', 'room');\n"
        << "  if (characterId === null || roomId === null) return __rotsMutationResult(false, "
           "'invalid-target', 'Expected live character and room handles.', 'target');\n"
        << "  const args = { characterId: characterId, roomId: roomId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.teleport_char_x', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.teleport_char_x', args) "
           ": bridged;\n"
        << "}\n"
        << "function __rotsTeleportCharToTargetRoom(character, target) {\n"
        << "  const characterId = __rotsHandleId(character, 'character', 'character');\n"
        << "  const targetId = __rotsHandleId(target, 'character', 'target');\n"
        << "  if (characterId === null || targetId === null) return __rotsMutationResult(false, "
           "'invalid-target', 'Expected live character handles.', 'target');\n"
        << "  const args = { characterId: characterId, targetId: targetId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.teleport_char_xl', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.teleport_char_xl', "
           "args) : bridged;\n"
        << "}\n"
        << "function __rotsExtractObject(object) {\n"
        << "  const objectId = __rotsHandleId(object, 'object', 'object');\n"
        << "  if (objectId === null) return __rotsMutationResult(false, 'invalid-target', "
           "'Expected a live object handle.', 'object');\n"
        << "  const args = { objectId: objectId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.extract_obj', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.extract_obj', args) : "
           "bridged;\n"
        << "}\n"
        << "function __rotsDropObject(character, object) {\n"
        << "  const giverId = __rotsHandleId(character, 'character', 'character');\n"
        << "  const objectId = __rotsHandleId(object, 'object', 'object');\n"
        << "  if (giverId === null || objectId === null) return __rotsMutationResult(false, "
           "'invalid-target', 'Expected live character and object handles.', 'target');\n"
        << "  const args = { giverId: giverId, objectId: objectId };\n"
        << "  const bridged = __rotsCommandBridgeMutationResult('script.do_drop', args);\n"
        << "  return bridged === null ? __rotsCommandMutationResult('script.do_drop', args) : "
           "bridged;\n"
        << "}\n"
        ...
        << "    moveObject: __rotsMoveObject,\n"
        << "    teleportChar: __rotsTeleportChar,\n"
        << "    teleportCharOnly: __rotsTeleportCharOnly,\n"
        << "    teleportCharToTargetRoom: __rotsTeleportCharToTargetRoom,\n"
        << "    extractObject: __rotsExtractObject,\n"
        << "    dropObject: __rotsDropObject,\n"
        ...
        << "    move_object: __rotsMoveObject,\n"
        << "    teleport_char: __rotsTeleportChar,\n"
        << "    teleport_char_x: __rotsTeleportCharOnly,\n"
        << "    teleport_char_xl: __rotsTeleportCharToTargetRoom,\n"
        << "    extract_obj: __rotsExtractObject,\n"
        << "    do_drop: __rotsDropObject,\n"
    ...
    const char* filename)
{
    ...
    wrapped
        ...
        << "const __rotsReturn = (function(ctx, __rotsMutations, __rotsAttachTextSetter, "
           ...
           "__rotsValidateRoomFlagHelperName, "
           "__rotsCanonicalRoomFlags, __rotsRoomFlagTargetToken, "
           "__rotsAttachRoomFlagHelpers, __rotsRoomFlagTargetZoneVnum) {\n"
        << "  'use strict';\n"
        << source << "\n"
    ...
    const JsGameRuntimeEvaluationOptions& evaluation_options, const char* filename)
{
    ...
    std::ostringstream wrapped;
    wrapped
        << trigger_context_preamble(context, evaluation_options)
        << "const __rotsReturn = (function(ctx, __rotsMutations, __rotsAttachTextSetter, "
           "__rotsAttachSymbolSetter, __rotsAttachCoordinateSetter, __rotsAttachResetModeSetter, "
           "__rotsAttachLifespanSetter, __rotsAttachLevelSetter, __rotsAttachObjectLevelSetter, "
           "__rotsAttachObjectRaritySetter, __rotsAttachSectorTypeSetter, "
    ...
    const JsGameRuntimeEvaluationOptions& evaluation_options, const char* filename)
{
    ...
    std::ostringstream wrapped;
    wrapped
        << trigger_context_preamble(context, evaluation_options) << "const exports = Object.create(null);\n"
        << "const __rotsPackage = (function(exports, __rotsMutations, __rotsAttachTextSetter, "
           "__rotsAttachSymbolSetter, __rotsAttachCoordinateSetter, __rotsAttachResetModeSetter, "
           "__rotsAttachLifespanSetter, __rotsAttachLevelSetter, __rotsAttachObjectLevelSetter, "
           "__rotsAttachObjectRaritySetter, __rotsAttachSectorTypeSetter, "
