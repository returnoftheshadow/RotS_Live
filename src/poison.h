...
// Clears `victim`'s poison record once no SPELL_POISON affect remains, at any depth. A null
// `victim` logs a SYSERR and changes nothing.
void forget_poison_origin_if_cured(char_data* victim);

// The standard SPELL_POISON affect at caster level `level`: -2 STR for `level` + 1 ticks.
[[nodiscard]] affected_type poison_victim_affect_at_level(int level);
// poison_victim_affect_at_level() at `caster`'s mystic caster level.
[[nodiscard]] affected_type poison_victim_affect(const caster_snapshot& caster);
// The Pale Lady's bite: -4 STR for 24 ticks, the strongest poison.
[[nodiscard]] affected_type pale_lady_poison_affect();
// Poisoned food or drink: no strength malus, for `duration` ticks.
[[nodiscard]] affected_type consumed_poison_affect(int duration);

// True when `victim` resists a poison from `caster`. A null `victim` logs a SYSERR and returns
// true, so the poison does not land.
[[nodiscard]] bool saves_poison(const char_data* victim, const caster_snapshot& caster);
...
// damage. Returns true when the victim died. A null `victim` or `poison` logs a SYSERR and returns
// false without changing anything.
[[nodiscard]] bool tick_poison_affect(char_data* victim, affected_type* poison);
// The poison damage one tick deals to `victim`, credited to its resolved poisoner or nobody.
// Returns nonzero when the victim died. A null `victim` logs a SYSERR and returns 0.
...
// A poison flag set by worn gear stays. A null `victim` logs a SYSERR and returns false.
...
enum class poison_outcome {
    ...
    extended,            // an equal poison ran; its duration grew, up to its initial duration
    blocked_by_stronger, // a stronger poison is running; nothing changed
    not_applied,         // no victim; nothing changed
};

// The size of `poison`'s strength malus: 4 for a -4 STR poison, 0 for one with no strength malus.
[[nodiscard]] int poison_strength(const affected_type& poison);
...
// resolves. Sends no messages. A null `victim` logs a SYSERR and returns not_applied.
[[nodiscard]] poison_outcome apply_poison(char_data* victim, const affected_type& poison,
...
// shared merge line, and a refusal also tells `caster` when it is not null; a null `caster` means
// there is no caster to tell. Not applied sends nothing. A null `victim` logs a SYSERR and sends
// nothing.
...
// Starts resisting `victim`'s running poison at `cleric_level`, the resist affect's modifier. A
// null `victim` logs a SYSERR and returns not_poisoned.
[[nodiscard]] poison_resistance_outcome start_poison_resistance(char_data* victim,
                                                                int cleric_level);
