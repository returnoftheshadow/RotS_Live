...
constexpr JsScriptingManifestEntry ManifestEntries[] = {
    ...
    {JsScriptingManifestKind::LegacyScriptTrigger, ON_BEFORE_DIE, "ON_BEFORE_DIE", "onBeforeDie",
     ...
     "Defined by the legacy script trigger table but not currently dispatched; keep reserved "
     "until a product decision enables it."},
...
bool js_scripting_api_permission_is_allowed(int legacy_command) {
    const JsScriptingApiPermissionEntry *entry =
        find_js_scripting_api_permission_entry(legacy_command);
    if (!entry)
        return false;
    return false;
}
