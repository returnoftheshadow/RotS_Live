...
- Latest completed slice: JavaScript API `Character.profile` getter promotion slice. `Character.profile` is now a frozen read-only `CharacterProfile` snapshot through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter copies from public character profile fields, BuilderClient fallback typings, offline fixture normalization, and IPC validation. The snapshot exposes copied identity/description/demographic/language/time/size/ranking/practiced-language metadata, preserves nullable and empty text semantics, keeps account/host/password-style fields absent, bounds byte-backed offline fixture fields and tongue slots, and keeps whole-profile mutation unsupported.
- Latest validation: passed for the `Character.profile` getter promotion slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ runtime/contract tests passed (`65` tests), full root `make test -j16` passed (`1530/1530`), BuilderClient focused offline/IPC validation tests passed (`79` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `513` tests), and root/BuilderClient `git diff --check` passed.
...
- Current slice: JavaScript API `GameObject.affects` getter promotion slice. Promote fixed object affect slots into a bounded frozen read-only `readonly ObjectAffect[]` snapshot across server mapping, generated API contract/docs/typings, live adapter/runtime snapshots, BuilderClient offline fixtures, and parity tests.
- Next slice: JavaScript API object extra-description getter audit or `GameObject.flags` remaining value-domain helper design after `GameObject.affects`, depending on the remaining mapping-catalog gaps.
