...
- Latest completed slice: JavaScript API helper mutation gate harness slice. Added server-owned `JsApiHelperMutationGateRequirement` metadata for the reusable helper gate contract: opaque target tokens, target-scoped authority, stale-handle denial, per-helper mutation limits, sanitized `MutationResult`, audit-before-apply policy, and atomic no-partial-write behavior. Struct-mapping tests now pin the requirement order, builder-safe wording, offline parity expectations, adversarial test focus, and coverage by the `helper-guardrail-foundation` plan before any room flag/exit/object/affect helper becomes callable.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused root helper-gate struct-mapping tests passed (`4` tests), full root `make test -j16` passed (`1573/1573`), and root `git diff --check` passed. BuilderClient was unchanged and clean.
- Current slice: JavaScript API typed helper mutation envelope slice. Add a non-callable runtime/dispatch/offline envelope discriminator for future helper methods so scalar validated setters and helper mutations cannot be confused; pin helper envelopes as rejected until a concrete helper implements authority, audit, and atomic apply support.
- Next slice: JavaScript API typed helper mutation envelope slice. Add a non-callable runtime/dispatch/offline envelope discriminator for future helper methods so scalar validated setters and helper mutations cannot be confused; pin helper envelopes as rejected until a concrete helper implements authority, audit, and atomic apply support.
