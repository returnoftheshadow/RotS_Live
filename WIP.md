...
- Current slice: JavaScript API `GameObject.touched` getter promotion. Completed; commit the server and BuilderClient changes.
- Next slice: JavaScript API object raw value/flag domain getter classification. Audit remaining raw `obj_data`/`obj_flags` value-domain fields and keep raw bitvectors/value arrays unavailable until filtered, documented item-type-specific helper APIs are designed.
