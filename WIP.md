...
- Latest completed slice: JavaScript API legacy command-helper executable examples slice. BuilderClient now includes executable TypeScript shape-script examples under `BuilderClient/examples/shape-script/` for gate greeting and quest reward flows. `docs/shape_script.md` links those files, and BuilderClient tests verify every referenced example compiles through the TypeScript pipeline and runs through offline fixtures while using command helpers accepted by the live JavaScript runtime shape.
- Latest validation: focused BuilderClient example/compiler/offline tests passed (`3` files, `96` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1611/1611`), BuilderClient full `npm test -- --run` passed (`50` files, `606` tests), and root/BuilderClient `git diff --check` passed. New Magus, Vincent, and Bazarat agents could not be spawned because the subagent thread pool is full; this remains the current review limitation for new slices.
- Current slice: JavaScript API legacy command-helper live side-effect apply slice. Promote safe apply semantics for output helpers first (`do_say`, `send_to_char`, `send_to_room`) with bounded text, target validation, recursion/depth handling, audit, and parity tests before object load/give or wait continuations.
