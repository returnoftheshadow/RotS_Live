...
- Latest completed slice: JavaScript runtime adversarial budget/hash hardening slice. Runtime coverage now includes stack exhaustion, regexp work under the instruction budget, Proxy trap interruption, and diagnostic-conversion callback bounds. Vendored QuickJS subset drift is pinned by `third_party/quickjs/SHA256SUMS.rots`, CMake target `verify_quickjs_hash`, top-level `make verify-quickjs-hash`, and raw `src/Makefile` target `verify-quickjs-hash`. V1 does not add an external watchdog before trigger dispatch; the decision is to rely on fresh per-invocation QuickJS runtimes, nonblocking mode, memory/stack limits, interrupt budgets, dispatch guards, source policy preflight, and sanitized diagnostics until async/native callback/retained-state requirements change.
- Latest validation: focused `JsRuntime.*` tests passed (`16` tests), top-level and raw QuickJS hash verification targets passed, full root `make test -j16` passed (`1489/1489`), root `git diff --check` passed, and Magus, Vincent, and Bazarat reported no blocking findings.
...
- Next slice: audit the public struct mapping catalog, generated typings/docs, live QuickJS snapshots, and BuilderClient offline fixtures for any remaining `char_data`, `obj_data`, `room_data`, or `zone_data` getter/setter rows that lack final implemented/deferred/unsupported/internal status or parity coverage, then pick the next narrow callable setter or classification slice.
