...
- Latest completed slice: JavaScript command-result offline source-order documentation. Builder-facing docs now explain that accepted offline object helpers update private per-run source-order diagnostic state for later helper classification, live commit order remains server-owned transaction order, failed helpers leave hidden state unchanged, repeated fixture runs start from the fixture, and script-visible frozen snapshots do not mutate.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient offline/compiler/example tests passed (`126` tests), full BuilderClient `npm test -- --run` passed (`50` files, `641` tests), full root `make test -j16` passed (`1673/1673`), and root/BuilderClient `git diff --check` passed. Magus found no blockers; Vincent and Bazarat's hidden-helper leak findings were addressed by converting command-state helpers to lexical bindings and expanding direct/global hidden-internal tests, and Bazarat's weight/polymorphic coverage gaps were fixed.
- Next slice: JavaScript command-result offline room-placement state parity. Add hidden per-run room object placement state for accepted `load_obj(vnum, room)` once a non-script-visible probe or later helper can consume that room state meaningfully.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API room flag helper callable promotion preflight slice: design public helper method contracts, add offline fixture state-application probes, and keep methods absent until per-flag authority and side-effect policies are pinned.
