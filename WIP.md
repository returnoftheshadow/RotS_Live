...
- Latest completed slice: Legacy `DO_FOLLOW` relationship preflight. `RotS.Script.doFollow(follower, leader)` remains non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Legacy scripts call `circle_follow`, may stop the would-be leader's own master to break loops, and then call `add_follower`; JavaScript promotion must reject self-follow and looped graphs instead of silently rewriting the leader, preserve reciprocal follower/master links, define follower caps and protected charm/tame/recruit/pet state, audit both character handles, define follow/stop-follow messaging policy, roll back follower-side previous-master replacement, and model hidden offline relationship state while keeping visible snapshots frozen.
...
- Next slice: Legacy `DO_FLEE` movement/combat preflight. Design `doFlee(character)` only after combat-state authority, eligible-exit selection, no-flee/blocked-room policy, deterministic offline exit choice, follower/mount side effects, wait/combat cleanup, audit fields, rollback behavior, and branchable result codes are pinned.
...
- Small-slice execution queue:
  ...
  - [~] Legacy character movement helper slice: `teleportChar`, `teleportCharOnly`, and `teleportCharToTargetRoom` are callable; `loadMob`, `extractChar`, and `doFollow` have non-callable preflight policies; add audited flee helper policy next.
