...
- Next slice: JavaScript command-result offline accepted object-state mutation parity. Add hidden offline fixture state updates for accepted `load_obj` and `do_give` command events so later helper calls in the same run can observe accepted object placement, inventory count, and carried-weight changes without mutating script-visible frozen snapshots.
