...
- Latest completed slice: Legacy `TELEPORT_CHAR_X` callable parity. `RotS.Script.teleportCharOnly(character, room)` and the `teleport_char_x` migration alias now compile through generated/fallback typings, live QuickJS command bridging, dispatch inline result preflight, transaction apply, BuilderClient offline execution, and builder docs. Live dispatch requires persistent target-zone authority, live character/room handles with current rooms, command audit, non-blocked target rooms, and no `NO_TELEPORT`; apply stops riding, moves only that character, leaves followers behind, does not run movement triggers, and runs after object, room-flag, setter, and wait phases so later fallible phases do not require mount/riding rollback. Offline fixtures return `blocked-room`/`no-teleport` from room flags and keep object/drop helper behavior aligned to live category order rather than source-order teleport state.
- Latest validation: focused server runtime/contract/artifact/dispatch tests passed (`290` tests), full root `make test -j16` passed (`1682/1682`), BuilderClient `npm run typecheck` passed, focused BuilderClient offline/IPC/example tests passed (`224` tests), full BuilderClient `npm test -- --run` passed (`53` files, `679` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy `TELEPORT_CHAR` follower-propagating movement preflight. Design and promote `teleportChar(character, room)` only after follower/mount propagation, movement-trigger policy, blocked/no-teleport parity, audit fields, rollback, and offline fixture state are pinned.
...
- Small-slice execution queue:
  ...
  - [~] Legacy character movement helper slice: `teleportCharOnly` is callable; add audited `loadMob`, follower teleport, target-room teleport, extract, follow, and flee helpers.
