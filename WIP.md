...
- Latest completed slice: JavaScript API `Room.characters` getter classification/promotion. `Room.characters` now exposes frozen `readonly CharacterRelationshipSnapshot[]` snapshots through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter people-list copying, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Entries copy only bounded direct live room occupants whose `in_room` reciprocally references the room, omit recursive followers/master/mount/group/inventory/equipment handles and setter methods, and keep `Room.setCharacters` unsupported until audited character movement/teleport helpers own room lists, combat state, followers, mounts, visibility, security, stale-handle, rollback, and audit semantics.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Full root `make test -j16` passed (`1561/1561`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`136` tests), BuilderClient full `npm test -- --run` passed (`49` files, `562` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API `Room.affects` getter classification/promotion planning. Decide whether room affects should remain deferred or expose bounded shallow read-only affect snapshots without bypassing room affect synchronization and duration authority.
- Next slice: JavaScript API `Room.affects` getter classification. If promoted, expose only bounded shallow read-only room affect snapshots with no direct affect mutation; otherwise pin the deferral with explicit room-affect helper requirements.
