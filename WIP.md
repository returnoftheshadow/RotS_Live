...
- Latest completed slice: Legacy food/drink/pull/examine examples. Added executable BuilderClient TypeScript examples for `ON_EAT`, `ON_DRINK`, `ON_EXAMINE_OBJECT`, and `ON_PULL` under `BuilderClient/examples/legacy-script/`, with fixture-backed happy and branch coverage for object-host contexts, actor/NPC guards, object vnum/name checks, examiner-only output, room output, and `ON_PULL` block/allow returns. Builder-facing docs now point builders to those examples and explain how object-trigger fixtures use `ctx.object`, `ctx.actor`, and `ctx.room`.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient `shapeScriptExamples` tests passed (`7` tests), focused root `JsScriptingManifest` checks passed (`18` tests), and root/BuilderClient `git diff --check` passed. Full BuilderClient `npm test -- --run` passed `680/681` before one unrelated built CLI publish-stage test hit the shared `15s` timeout; the exact timed-out CLI test passed on isolated rerun.
- Next slice: Legacy random/war-status/condition examples. Add deterministic fixture controls for random rolls and fame-war status so old chance-based and condition-heavy scripts can be tested offline.
