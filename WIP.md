...
- Next slice: JavaScript command-result offline accepted `load_obj` room-state parity. Add hidden per-run room object placement state for accepted `load_obj(vnum, room)` so later offline helper probes and diagnostics can observe accepted room placement without mutating script-visible room content snapshots.
