...
- Latest completed slice: JavaScript `load_obj` final result bridge retrofit slice. Live `RotS.Script.loadObj/load_obj` now returns server-owned inline `MutationResult` codes for `not-found`, `invalid-target`, `not-authorized`, `inventory-full`, `too-heavy`, `audit-rejected`, and transaction-accepted `ok`; unauthorized calls fail before prototype lookup so missing/existing vnums are not distinguishable without mutation authority. Accepted loads are audited once, marked as bridge-accepted internally, and create objects only once during transaction apply after final live rechecks. BuilderClient offline now mirrors target-handle and inventory-count failures, plus approximate carried-weight failures until fixture prototype catalogs exist.
- Latest validation: focused server contract/artifact/dispatch tests passed (`38` tests), full root `make test -j16` passed (`1657/1657`), BuilderClient `npm run typecheck` passed, focused BuilderClient offline/compiler/IPC tests passed (`186` tests), full BuilderClient `npm test -- --run` passed (`50` files, `614` tests), and root/BuilderClient `git diff --check` passed. Magus, Vincent, and Bazarat review follow-up moved authority before prototype lookup, aligned malformed target codes with `invalid-target`, documented BuilderClient carried-weight approximation, and added unauthorized, audit-denied, too-heavy, no-target, room-target, and bridge-accepted stale-capacity coverage.
- Next slice: JavaScript `do_wait` final result bridge retrofit. Add inline command result classification for wait authority/audit/applicability, keep accepted waits applied once at transaction commit, mirror BuilderClient offline result codes where possible, and add no-partial transaction coverage.
...
- Current slice: JavaScript `load_obj` final result bridge retrofit is in validation/commit handoff. Live and BuilderClient focused tests pass; full root and full BuilderClient validation are next before commit.
- Next slice: JavaScript `do_wait` final result bridge retrofit. Add inline command result classification for wait authority/audit/applicability, keep accepted waits applied once at transaction commit, mirror BuilderClient offline result codes where possible, and add no-partial transaction coverage.
