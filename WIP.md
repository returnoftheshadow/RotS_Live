...
- Latest completed slice: Legacy room/exit mutation helper preflight hardening. The non-callable `setExitState(room, direction, state)` and `changeExitTo(room, direction, destination)` policies now explicitly require canonical direction/state validation, north/direction 0 and open/state 0 acceptance, bounded direction checks before any exit-array or reverse-direction lookup, missing-exit and invalid/`NOWHERE` destination rejection, broken-door cleanup, reciprocal reverse-exit mirroring, source-only destination-change policy, reset-command impact review, room-message reachability, audit, branchable result codes, rollback/no-partial behavior, and hidden offline exit state before any callable promotion.
- Latest validation: focused `JsApiStructMapping` CTest filter passed (`32` tests), BuilderClient `npm run typecheck` passed, focused BuilderClient shape-script docs/examples test passed (`7` tests), root/BuilderClient `git diff --check` passed, and Heisenberg/Parfit/Galileo reviewer findings were addressed.
- Next slice: Legacy `LOAD_OBJ_X` clone-helper preflight hardening. Tighten the non-callable object clone design around source-object liveness, prototype versus live-instance copy policy, contained/equipped/carried ownership, nested contents and affect copying, destination custody, capacity/weight checks, audit metadata, hidden offline clone state, and rollback/no-partial behavior before any callable promotion.
