...
- Latest completed slice: JavaScript command-helper final `do_give` bridge foundation slice. The dispatch layer now exposes server-owned command-result reason codes and classifies `do_give` applicability as `ok`, `invalid-target`, `not-carried`, `no-drop`, `inventory-full`, or `too-heavy`. Generated and BuilderClient fallback `MutationResult.code` typings now reserve those command-result codes plus `audit-rejected` and `not-found` for final helper outcomes. Existing queued command behavior is intentionally unchanged until the narrow QuickJS callback seam is implemented.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused server dispatch/API contract tests passed (`5` tests), full root `make test -j16` passed (`1641/1641`), BuilderClient `npm run typecheck` passed, focused BuilderClient compiler/IPC tests passed (`2` files, `104` tests), full BuilderClient `npm test -- --run` passed (`50` files, `611` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript command-helper final `do_give` QuickJS callback seam slice. Add the narrow runtime/dispatch callback path so `RotS.Script.doGive()` can return final server preflight/audit applicability inline without applying twice, exposing generic native calls, or changing output rollback ordering.
- Next slice: JavaScript command-helper final `do_give` QuickJS callback seam slice. Add the narrow runtime/dispatch callback path so `RotS.Script.doGive()` can return final server preflight/audit applicability inline without applying twice, exposing generic native calls, or changing output rollback ordering.
