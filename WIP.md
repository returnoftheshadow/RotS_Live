...
- Latest completed slice: JavaScript API `Room.extraDescriptions` getter classification/promotion. `Room.extraDescriptions` now exposes a bounded frozen `readonly ExtraDescription[]` snapshot through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter linked-list copying, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. The room path reuses the object extra-description policy: copied keyword/description strings, capped traversal, cycle stopping, text-length bounds, default empty arrays, null-prototype frozen entries, and no whole-list setter surface. `Room.setExtraDescriptions` remains unsupported until narrow add/update/remove helpers own linked-list allocation, stale-handle protection, sanitization, persistence, rollback, and audit semantics. `room_data::room_data()` now initializes `ex_description` to null so default-constructed rooms are safe for the getter.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused C++ API/adapter/runtime/artifact/manifest tests passed (`185/185`), Magus' legacy dispatch segfault repro passed after `room_data::ex_description` constructor initialization, full root `make test -j16` passed (`1551/1551`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`119` tests), BuilderClient full `npm test -- --run` passed (`49` files, `545` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: JavaScript API `Room.exits` getter classification. Decide whether room exits should become a bounded frozen read-only `RoomExit[]` snapshot or remain deferred until direction, door-state, destination-room, reset-command, bidirectional-link, permission, and persistence semantics are pinned.
