...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, reviewer-requested stale target/legacy-consume behavior regressions, live per-pulse/per-package JavaScript invocation budgeting, live JavaScript trigger-entry depth guarding, sanitized live JavaScript dispatch failure logging, generated builder-facing runtime safety metadata, script-visible trigger metadata parity, and BuilderClient offline live-shaped handle normalization are in place before any world mutation/output helpers.
- Latest completed slice: BuilderClient offline handle normalization. Offline fixture execution now converts persisted fixture handles into live-shaped read-only `Character`, `GameObject`, `Room`, and `Zone` objects with `isValid()`, character stats, nested room/zone data, object owner links, object-room exclusivity for carried/worn objects, object-host `self === null`, player/NPC prototype-vnum semantics, and explicit null/default context fields. VM helper functions and context `isValid()` functions are now created inside the VM realm with null prototypes instead of exposing host-realm function constructors.
- Latest validation: Magus, Vincent, and Bazarat reviewed the handle-normalization slice. Their object-host `self`, impossible object location, player prototype-vnum, identity-flag conflict, and VM host-function findings were addressed; their broader role/scalar fixture modeling findings are recorded as the next slice. Focused server validation passed 50/50 for `JsApiContract`, `JsBuilderArtifacts`, and `JsGameRuntime`; focused BuilderClient validation passed `npm run typecheck` and 40/40 compiler/offline/IPC/parity tests. Full validation passed `git diff --check`, the C++ suite passed 1382/1382, and the BuilderClient production build plus full test suite passed 440/440.
