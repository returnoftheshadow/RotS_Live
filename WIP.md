...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: finalize and commit the JavaScript API zone remaining scalar/power/reset-command classification slice.
- Next slice: JavaScript API final public mapping audit slice, confirming every public top-level `char_data`, `obj_data`, `room_data`, and `zone_data` getter/setter policy has either callable implementation coverage or explicit unsupported/deferred/internal coverage in server artifacts and BuilderClient fallback/runtime tests.
