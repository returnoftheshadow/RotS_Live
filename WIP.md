...
- Latest completed slice: Legacy `LOAD_MOB` mobile loading preflight. `RotS.Script.loadMob(vnum, room)` remains non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Legacy scripts allocate a mobile prototype into a temporary character variable before later placement; JavaScript promotion must use an explicit room argument, reject unknown or unauthorized prototypes through a server-owned catalog, require target-room authority and per-script load limits, initialize Mudlle/mobile-procedure state through the normal server loader, account for live mob-index counts, define any future zone reset-line accounting explicitly, generate deterministic offline temporary ids, avoid returning mutable live character handles, and preserve no-partial rollback behavior.
...
- Next slice: Legacy `DO_FOLLOW` relationship preflight. Design `doFollow(follower, leader)` after the callable teleport helpers and preflight-pinned `loadMob`/`extractChar` policies, focusing on reciprocal follower/master links, loop prevention, charm/tame/ownership policy, follower caps, mount/group interactions, movement propagation, combat XP sharing, audit fields, rollback behavior, and offline hidden relationship state.
...
- Small-slice execution queue:
  ...
  - [~] Legacy character movement helper slice: `teleportChar`, `teleportCharOnly`, and `teleportCharToTargetRoom` are callable; `loadMob` and `extractChar` have non-callable preflight policies; add audited follow and flee helper policies next.
