...
- Latest completed slice: Legacy `TELEPORT_CHAR` follower-propagating movement parity. `RotS.Script.teleportChar(character, room)` and the `teleport_char` migration alias now compile through generated/fallback typings, live QuickJS command bridging, dispatch inline result preflight, transaction apply, BuilderClient offline execution, and builder docs. Live dispatch uses the same target-zone authority, live handle/current-room, audit, blocked-room, and `NO_TELEPORT` checks as `teleportCharOnly`; accepted apply stops riding, moves same-room NPC followers first, moves the selected character, leaves player followers behind, emits no movement text, does not run movement triggers, and stays after object, room-flag, setter, and wait phases. Offline fixtures record same-room NPC follower movement in hidden per-run state while keeping visible snapshots frozen and object/drop helpers aligned to live category order.
- Latest validation: full root `make test -j16` passed (`1682/1682`), BuilderClient `npm run typecheck` passed, focused BuilderClient offline/IPC/example tests passed (`225` tests), full BuilderClient `npm test -- --run` passed (`53` files, `680` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy `TELEPORT_CHAR_XL` target-room movement preflight. Design and promote `teleportCharToTargetRoom(character, target)` only after destination target-room resolution, stale target handling, follower mode, blocked/no-teleport parity, audit fields, rollback, and offline fixture state are pinned.
...
- Small-slice execution queue:
  ...
  - [~] Legacy character movement helper slice: `teleportChar` and `teleportCharOnly` are callable; add audited `loadMob`, target-room teleport, extract, follow, and flee helpers.
