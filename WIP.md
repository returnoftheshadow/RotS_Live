...
- Latest validation: passed for the builder-owned setter execution path slice. Focused legacy/dispatch checks passed (`3` tests), `git diff --check` passed, and full `make test -j16` passed (`1415/1415`).
- Current slice: JavaScript API builder-owned setter execution path slice commit.
