...
- Latest completed slice: JavaScript API first generated typings getter promotion. `GameObject.description`, `GameObject.shortDescription`, `Room.description`, `Room.level`, `Room.alignment`, and `Zone.level` are now in the server API contract, implemented read-only struct mapping statuses, generated typings/artifacts, live QuickJS snapshots, BuilderClient offline normalization, IPC validation, and role-filtered fixture editor controls.
- Latest validation: Magus, Vincent, and Bazarat reviewed the getter-promotion slice. Follow-up pinned every promoted mapping row/status, aligned live/offline `shortDescription` fallback behavior, preserved and bounded new fixture fields through IPC validation, added editor-state/control coverage, role-filtered fixture fields, and ran focused C++/BuilderClient tests successfully before the final validation pass.
- Next slice: promote the next safe read-only getter group from the mapping catalog into generated TypeScript declarations, live QuickJS snapshots, and BuilderClient offline fixtures together, starting with nullable text/scalar fields that can be copied safely (`GameObject.actionDescription`, `Zone.description`, `Zone.map`, and similar low-risk scalar zone fields) before moving to setter foundation.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API next generated typings getter slice: promote nullable text/scalar getter groups into generated `rots.d.ts`, live QuickJS snapshots, and BuilderClient offline fixtures together.
