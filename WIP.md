...
- Latest completed slice: JavaScript API room flag helper callable promotion preflight slice. BuilderClient now has a non-script-visible offline application probe that applies prepared `room.flags.add`/`room.flags.remove` helper mutations to cloned fixture room state, preserves canonical room-flag ordering, treats repeated add/remove calls idempotently, fails closed when a registry-accepted helper lacks a concrete offline applier, and rolls room flags back in reverse order when a later helper apply precondition fails. `Room.addFlag`, `Room.removeFlag`, and `Room.setFlags` remain absent from generated/fallback typings, live QuickJS handles, and offline script handles.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Full root `make test -j16` passed (`1604/1604`), BuilderClient full `npm test -- --run` passed (`49` files, `596` tests), focused BuilderClient offline runner tests passed (`67` tests), BuilderClient `npm run typecheck` passed, and root/BuilderClient `git diff --check` passed. Reviewer follow-up added preservation coverage for read-only visible room flags such as `permanentAffect`, exact canonical ordering with deliberately unsorted initial flags, and rollback reporting with a prepared scalar setter in the same batch.
- Current slice: JavaScript API room flag helper per-flag authorization policy slice. Pin which room flags are builder-zone safe, admin-only, or blocked; document movement/combat/teleport/light/drink/security side effects and audit fields; and add generated/offline/server guard tests before making `Room.addFlag`/`Room.removeFlag` callable.
- Next slice: JavaScript API room flag helper per-flag authorization policy slice. Pin which room flags are builder-zone safe, admin-only, or blocked; document movement/combat/teleport/light/drink/security side effects and audit fields; and add generated/offline/server guard tests before making `Room.addFlag`/`Room.removeFlag` callable.
