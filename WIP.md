...
- Latest completed slice: Legacy character movement helper foundation. Added server-owned non-callable operation metadata for planned `loadMob`, `teleportChar`, `teleportCharOnly`, `teleportCharToTargetRoom`, `extractChar`, `doFollow`, and `doFlee` helpers, with legacy opcode mapping, target policy, authority requirements, side-effect notes, audit fields, stable diagnostics, rollback expectations, offline fixture parity, and adversarial test focus. Builder-facing docs now explain the planned commands and explicitly state they are not callable yet; generated/fallback typing tests pin that they remain absent from script-visible APIs.
- Latest validation: focused server API contract/mapping/artifact tests passed (`48` tests), BuilderClient typecheck passed, focused BuilderClient IPC/docs validation passed (`96` tests), full BuilderClient `npm test -- --run` passed (`53` files, `676` tests), full root `make test -j16` passed (`1682/1682`), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy character movement live/offline callable preflight slice. Start with one narrow helper candidate, likely `teleportCharOnly(character, room)`, and add live dispatch plus BuilderClient offline result parity only after target-token, authority, liveness, blocked-room/noTeleport, trigger-blocked, audit, rollback, and docs tests are in place.
...
- Small-slice execution queue:
  ...
  - [ ] Legacy object movement/extract compatibility slice: add audited object move/extract/drop helpers and no-partial rollback coverage.
  - [ ] Legacy character movement helper slice: add audited `loadMob`, teleport, extract, follow, and flee helpers.
