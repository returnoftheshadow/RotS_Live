...
- Latest completed slice: JavaScript output helper final result bridge retrofit. Live `RotS.Script.doSay/do_say`, `sendToChar/send_to_char`, and `sendToRoom/send_to_room` now return inline `MutationResult` outcomes for audited output helper execution: malformed or stale live targets return `invalid-target`, audit denial returns `audit-rejected`, no connected playing descriptor recipient returns `no-recipient`, and accepted output returns `ok` while queuing one bridge-marked command for the final descriptor-output transaction category. Script room/say output now uses a connected-playing descriptor filter at apply time instead of legacy room broadcast behavior, and native command-result bridge messages are forced to `null` so private host diagnostics do not enter builder code.
- Latest validation: focused server runtime/contract/artifact/dispatch tests passed (`66` tests), full root `make test -j16` passed (`1673/1673`), BuilderClient `npm run typecheck` passed, focused BuilderClient offline/compiler/example tests passed (`105` tests), full BuilderClient `npm test -- --run` passed (`50` files, `615` tests), and root/BuilderClient `git diff --check` passed. Magus, Vincent, and Bazarat review follow-up added audited reachability precedence docs, final apply filtering for non-playing room and direct-character descriptors, private bridge-message suppression, runtime bridge-marker coverage for all output helper wrappers, and BuilderClient offline no-recipient non-emulation coverage.
- Next slice: JavaScript command-result offline parity fixture catalog. Add the BuilderClient/server parity fixture catalog for command-result branches that still need richer offline state, starting with output descriptor reachability, wait-list state, and object prototype catalogs.
