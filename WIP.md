...
- Latest completed slice: JavaScript API generated documentation/export. Public server artifacts now include non-internal struct accessor mapping rows in API markdown and manifest JSON using public `fieldId` values, getter/setter callable flags, and documentation-only markers. BuilderClient now parses, bounds, preserves, and IPC-validates public `apiStructFieldMappings` while filtering `internal-only` rows.
- Latest validation: Magus, Vincent, and Bazarat reviewed the generated documentation/export slice. Follow-up removed public `sourceField` exposure, filtered internal-only rows from public artifacts and BuilderClient validation, added callable/documentation-only flags, added row-exact JSON and Markdown regression checks, preserved mappings through BuilderClient IPC validation, and ran `make test` successfully with `1389/1389` passing plus BuilderClient `npm run typecheck`, focused artifact/IPC tests, full `npm test` (`49` files, `466` tests), and `npm run build`.
- Next slice: promote the next safe read-only getter group from the mapping catalog into generated TypeScript declarations, live QuickJS snapshots, and BuilderClient offline fixtures together, with parity tests.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API generated typings getter slice: promote implemented safe read-only scalar/string/relationship getters into generated `rots.d.ts`, live QuickJS snapshots, and BuilderClient offline fixtures together.
