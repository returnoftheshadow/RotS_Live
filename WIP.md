...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, and live Mudlle `SPECIAL_SELF` heartbeat tick backing are in place before any world mutation/output helpers.
- Latest completed slice: Mudlle `SPECIAL_SELF` heartbeat/tick backing. The periodic mobile activity heartbeat now runs targeted `mudlle-mobile` JavaScript packages after legacy self handling declines to consume the tick, exposes the current server pulse as `ctx.tick`, preserves host/actor/room live validation, skips stale or masked inputs, and keeps runtime errors fail-open for this non-blocking heartbeat path.
- Latest validation: Magus and Bazarat reviewed the `SPECIAL_SELF` slice, with Bazarat's behavioral call-site coverage, dead `MOB_SPEC` hook, and stale/invalid guard findings addressed before commit. Vincent had no blocking trust-boundary changes to act on for this read-only heartbeat hook. Focused JavaScript/API/manifest/dispatch validation passed 227/227, `git diff --check` passed, and the full C++ suite passed 1355/1355.
