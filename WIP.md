...
- Latest completed slice: Legacy equipment helper preflight. `RotS.Script.doWear(character, object, slot?)`, `RotS.Script.doRemove(character, slotOrObject)`, and `RotS.Script.equipChar(character, prototypes)` remain non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Promotion must preserve direct carried-object ownership, equipped-slot membership, canonical wear-slot validation, race/body restrictions, anti-alignment and item race-flag zap restrictions, pet restrictions, missing-belt and shield/two-handed conflicts, `ON_WEAR` blocking before alternate slot fallback, forced remove of occupied slots, waist-belt cascade removal, inventory-full room drops, light counters, object affects, poison damage/death side effects, crash-save state, deterministic prototype loads, audit, rollback or no-partial behavior, and hidden offline equipment/inventory/room state. Future `equipChar` no-partial behavior is a deliberate JavaScript V1 tightening over legacy partial `do_wear("all")` behavior.
- Latest validation: focused root `JsApiStructMapping`/`JsBuilderArtifacts` checks passed (`38` tests), full root `make test -j16` passed (`1685/1685`), BuilderClient `npm run typecheck` passed, focused BuilderClient docs/IPC tests passed (`96` tests), full BuilderClient `npm test -- --run` passed (`53` files, `681` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy room/exit mutation helper preflight. Design `setExitState`, `changeExitTo`, door-state fixture modeling, destination-room authority, reset-command impact, bidirectional-link policy, audit fields, rollback/no-partial policy, and live/offline result-code parity.
...
- Small-slice execution queue:
  ...
  - [ ] Legacy room/exit mutation helper slice: add door-state and exit-destination helpers with fixture modeling.
