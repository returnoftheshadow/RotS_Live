...
- Latest completed slice: Legacy object lookup helper preflight hardening. The non-callable `findInventoryObject(character, query)`, `findEquippedObject(character, slotOrQuery)`, and `findRoomObject(room, query)` policies now explicitly require rejected unknown vnums, direct versus recursive inventory search semantics instead of the legacy first-container traversal quirk, equipment-slot bounds before array reads, direct room-content ownership, stable stale/not-found/multiple-match result codes, hidden offline lookup catalogs, and rollback-safe interaction with later custody helpers before any callable promotion.
...
- Next slice: Legacy exchange/reward helper preflight hardening. Tighten the non-callable `giveReward` and `exchangeReceivedObject` designs around multi-reward capacity/weight preflight, default item-return branches, received-item preservation, not-found reward prototypes, lookup result handling, audit metadata, hidden offline exchange state, and no-partial rollback before any callable promotion.
