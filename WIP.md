...
- Latest completed slice: JavaScript API room flag helper offline/generated contract parity slice. BuilderClient offline helper probes now validate server-shaped room target tokens, filtered room flag arguments, invalid target/argument statuses, prepared helper counts, and no-applier fail-closed behavior without exposing `Room.addFlag`, `Room.removeFlag`, or `Room.setFlags`. Server-owned helper policy text now documents apply-time previous-flag recording and reverse rollback before scalar setters commit.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Full root `make test -j16` passed (`1603/1603`), BuilderClient full `npm test -- --run` passed (`49` files, `592` tests), focused BuilderClient offline/IPC tests passed (`147` tests), BuilderClient `npm run typecheck` passed, focused C++ mapping/artifact/contract tests passed (`20` tests), and root/BuilderClient `git diff --check` passed. Reviewer-requested strict decoded-key helper argument parsing, explicit fixture token authority, server-catalog flag drift checks, every-allowed-flag offline acceptance coverage, optional room-role helper absence coverage, and tighter rollback policy assertions were added.
- Current slice: JavaScript API room flag helper callable-shape design guard slice. Add same-room multi-step rollback coverage, pin helper-method absence in generated/fallback typings and live/offline runtime surfaces, and document the remaining side-effect/authorization decisions required before `Room.addFlag`/`Room.removeFlag` can become callable.
- Next slice: JavaScript API room flag helper callable-shape design guard slice. Add same-room multi-step rollback coverage, pin helper-method absence in generated/fallback typings and live/offline runtime surfaces, and document the remaining side-effect/authorization decisions required before `Room.addFlag`/`Room.removeFlag` can become callable.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API room flag helper callable-shape design guard slice: add same-room multi-step rollback coverage, pin helper-method absence in generated/fallback typings and live/offline runtime surfaces, and document side-effect/authorization decisions before callable helper promotion.
