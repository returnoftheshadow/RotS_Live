...
- Latest completed slice: JavaScript API command-helper rollback and audit metadata completion slice. Command-helper audit requests now carry operation count/summary, dispatch host/kind/legacy trigger value, package vnum/id, handler name, and authority target zone for registry-backed dispatch. Command target ids now use bounded numeric parsing for `character:`, `char:`, `player:`, `mob:`, `room:`, and `object:` ids, rejecting overflow, signs, decimals, blank suffixes, whitespace, and duplicate JSON keys before audit. Live apply now runs object commands before room-flag, setter, wait, and output commits so a rare `read_object()` failure after preflight cannot happen after room flags or setters are written.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused `JsTriggerDispatch` CTest passed (`158` tests), full root `make test -j16` passed (`1627/1627`), and root/BuilderClient `git diff --check` passed. BuilderClient was unchanged in this slice.
- Current slice: JavaScript API command-helper polymorphic role resolution slice. Decide and implement live/offline support or explicit rejection for legitimate `ctx.target`, `ctx.targ1`, and `ctx.targ2` character/object/room command helper targets, with cloned/fake versions still rejected and parity tests for every supported helper.
