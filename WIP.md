...
- Latest completed slice: JavaScript API `Zone.symbol` setter promotion slice. `Zone.setSymbol(value: string): MutationResult` is now promoted through server API mapping, generated typings/manifest/docs, live QuickJS scalar setter validation, target-scoped persistent mutation dispatch, world-map redraw on committed global zone writes, BuilderClient fallback/offline fixtures, and parity tests.
- Latest validation: passed for the `Zone.symbol` setter promotion slice. Focused C++ mapping/artifact/manifest/runtime/dispatch checks passed (`29` tests), focused BuilderClient typecheck/offline/compiler/IPC checks passed (`59` tests), root and BuilderClient `git diff --check` passed, full `make test -j16` passed outside the sandbox for socket listener tests (`1433/1433`), and full BuilderClient `npm run typecheck && npm test -- --run && npm run build` passed (`49` files, `485` tests).
- Current slice: JavaScript API coordinate setter bounds-definition slice. Pin `Zone.x`/`Zone.y` validation ranges against legacy map dimensions and `symbol_to_map` behavior before making either setter callable.
- Next slice: JavaScript API coordinate setter bounds-definition slice. Define fail-closed validation for negative, oversized, non-integer, and out-of-map `Zone.x`/`Zone.y` values, add server/BuilderClient non-callable or promotion tests as appropriate, and only then promote the first coordinate setter.
