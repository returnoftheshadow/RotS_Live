...
- Latest completed slice: Legacy room/exit mutation helper preflight. `RotS.Script.setExitState(room, direction, state)` and `RotS.Script.changeExitTo(room, direction, destination)` remain non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Promotion must preserve canonical direction validation, open/closed/locked door states, existing-exit checks, `EX_ISDOOR`, `EX_ISBROKEN`, `NOWHERE` destination handling, conditional reverse-exit mirroring, source-only versus reciprocal destination policy, destination-zone authority, reset-command impact checks, room-message reachability, audit, rollback or no-partial behavior, and hidden offline exit state. JavaScript V1 deliberately hardens legacy behavior by allowing north/direction 0 for `setExitState`, rejecting invalid door states instead of defaulting them to open, and rejecting invalid directions or missing source exits for `changeExitTo`. Legacy `SET_EXIT_STATE` preserves all exit flags except `EX_CLOSED|EX_LOCKED`, clears broken-door state, and mirrors only reciprocal reverse exits; legacy `CHANGE_EXIT_TO` changes only the source exit destination and does not maintain reverse links.
- Latest validation: full root `make test -j16` passed (`1686/1686`) and full BuilderClient `npm test -- --run` passed (`53` files, `681` tests) before reviewer hardening; post-review focused root `JsApiStructMapping`/`JsBuilderArtifacts` checks passed (`39` tests), BuilderClient `npm run typecheck` passed, focused BuilderClient docs/IPC tests passed (`96` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy food/drink/pull/examine examples. Add executable examples for `ON_EAT`, `ON_DRINK`, `ON_PULL`, and `ON_EXAMINE_OBJECT` with fixture-backed condition/output coverage before adding more mutation helpers.
...
- Next slice: Legacy `DO_FLEE` movement/combat preflight. Design `doFlee(character)` after the callable teleport helpers and preflight-pinned `loadMob`/`extractChar`/`doFollow` policies, focusing on combat-state authority, eligible-exit selection, no-flee/blocked-room policy, deterministic offline exit choice, follower/mount side effects, wait/combat cleanup, audit fields, rollback behavior, and branchable result codes.
...
- Small-slice execution queue:
  ...
  - [ ] Legacy food/drink/pull/examine examples slice: add executable examples for `ON_EAT`, `ON_DRINK`, `ON_PULL`, and `ON_EXAMINE_OBJECT`.
  - [ ] Legacy random/war-status/condition examples slice: add deterministic fixture controls for chance-based scripts.
  - [ ] JavaScript command-result offline source-order diagnostic parity: add offline result metadata/tests that clarify source-order command logs versus committed server category order without changing live transaction ordering.
