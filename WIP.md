...
- Next slice: wire a concrete game-side parsed HTTP request source or listener boundary to `js_builder_http_ingress`, attach the server publish context resolver, keep BuilderClient traffic separate from live port `3791`, and preserve the Rust proxy trust boundary.
- Socket-boundary note: the parsed HTTP ingress is intentionally not connected to the descriptor loop until publish/status requests can be given trusted server-side authority context. Without that resolver, real publish traffic would either fail every authorization check or be tempted to trust BuilderClient-supplied zone data.
...
- Small-slice execution queue:
  ...
  - [ ] Server concrete ingress transport slice: wire an actual game-side parsed HTTP request source or listener boundary to `js_builder_http_ingress`, attach the server publish context resolver, keep BuilderClient traffic separate from live port `3791`, and preserve the Rust proxy trust boundary.
