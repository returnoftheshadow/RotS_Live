...
- Latest completed slice: JavaScript API room flag helper callable-shape design guard slice. Same-room multi-step rollback coverage now proves reverse rollback restores the original flags when a later helper fails after multiple successful mutations against one room. Live QuickJS, generated/fallback typings, and offline role guards continue to pin `Room.addFlag`, `Room.removeFlag`, and `Room.setFlags` absence while the builder-facing callable helper shape remains deferred.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Full root `make test -j16` passed (`1604/1604`), BuilderClient full `npm test -- --run` passed (`49` files, `592` tests), focused C++ rollback/runtime/artifact/contract tests passed (`27` tests), BuilderClient focused compiler/offline/IPC tests passed (`166` tests), BuilderClient `npm run typecheck` passed, and root/BuilderClient `git diff --check` passed. Reviewer follow-up added fallback compiler diagnostics for helper-method absence, nested optional character-room live absence including `ctx.dying.room`, and same-room prepare-count assertions before apply rollback.
- Current slice: JavaScript API room flag helper callable promotion preflight slice. Design the public `Room.addFlag`/`Room.removeFlag` callable method contract, add offline fixture state-application probes for add/remove/idempotent/rollback behavior, and keep the methods non-callable until per-flag authority and side-effect policies are fully pinned.
- Next slice: JavaScript API room flag helper callable promotion preflight slice. Design the public `Room.addFlag`/`Room.removeFlag` callable method contract, add offline fixture state-application probes for add/remove/idempotent/rollback behavior, and keep the methods non-callable until per-flag authority and side-effect policies are fully pinned.
