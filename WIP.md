...
- Latest completed slice: JavaScript API `Zone.level` setter promotion slice. `Zone.setLevel(value: number): MutationResult` is promoted through server API mapping, generated typings/manifest/docs, live QuickJS integer validation, target-scoped persistent mutation dispatch, BuilderClient offline fixtures, and parity tests. The callable range is integer `0` through `100`; inspection found this is persisted by legacy zone shaping and displayed by legacy zone inspection as builder-facing metadata, with no reset or map redraw side effects required.
- Latest validation: passed for the `Zone.level` setter promotion slice before commit. Focused C++ mapping/runtime/dispatch/artifact/manifest checks passed (`25` tests), full root `make test -j16` passed (`1458/1458`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/artifact/compiler/IPC tests passed (`71` tests), BuilderClient full `npm test -- --run` passed (`49` files, `485` tests), BuilderClient `npm run build` passed, and root/BuilderClient `git diff --check` passed.
- Latest completed slice: JavaScript API `Zone.minimumLookLevel` setter decision slice. `Zone.setMinimumLookLevel(...)` remains deferred and absent from generated typings because the inspected legacy zone loader, saver, and shaper paths do not currently persist or edit `zone_data.min_level_look` with the other zone scalar fields. The mapping docs now explain the persisted-builder-edit-path requirement and BuilderClient/server tests pin that the setter is not callable.
- Latest validation: passed for the `Zone.minimumLookLevel` decision slice before commit. Focused C++ mapping/manifest/artifact checks passed (`15` tests), BuilderClient focused IPC/typings fallback checks passed (`32` tests), and root/BuilderClient `git diff --check` passed.
