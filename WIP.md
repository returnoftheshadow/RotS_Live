...
- Latest completed slice: JavaScript API `Character.damageDetails` getter assessment slice. `Character.damageDetails` is now a frozen read-only `DamageDetails` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes elapsed combat seconds, total damage, damage per second, and sorted per-source entries with source id/kind/name/count/total/max/average/percent while excluding combat ownership, threat, XP sharing, timer internals, cleanup state, raw damage-map pointers, and direct mutation. `Character.setDamageDetails` remains unsupported because combat, timer, trigger, and persistence helpers own those counters.
- Latest validation: passed for the `Character.damageDetails` getter assessment slice. Focused C++ API/adapter/runtime tests passed (`118` tests), full root `make test -j16` passed (`1499/1499`), BuilderClient focused offline/IPC validation tests passed (`61` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `495` tests), and root/BuilderClient `git diff --check` passed. Reviewer follow-up addressed aggregate overflow by storing the server snapshot total as `long`, matching the legacy damage report accumulator.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.skills` getter assessment slice. Determine the safe read-only skill-practice shape for `char_data.skills`, promote stable diagnostic data with live/offline parity if feasible, and keep `Character.setSkill` unsupported unless a bounded admin/training helper is deliberately designed.
- Next slice: JavaScript API `Character.knowledge` getter assessment slice. Determine whether `char_data.player.knowledge` can expose a bounded frozen diagnostic snapshot with live/offline parity, while keeping direct knowledge mutation unsupported unless a separate audited training/admin helper is designed.
