...
- Latest completed slice: JavaScript API command-helper polymorphic role resolution slice. Live QuickJS and BuilderClient offline command helpers now private-brand invocation handles with concrete `character`, `object`, or `room` kinds, serialize `ctx.target`, `ctx.targ1`, and `ctx.targ2` aliases only when the helper parameter kind matches, and reject wrong-kind polymorphic roles without queuing command events. Live dispatch resolves those aliases back to explicit target or Mudlle target slots with existing liveness and zone-authority checks; raw numeric character/object lookup remains unpromoted beyond parser validation. `loadObj(vnum, target?)` typings now match the `load_obj` helper.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused root JS API/runtime/dispatch/artifact tests passed (`241` tests), focused BuilderClient offline/fallback/compiler tests passed (`97` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1631/1631`), full BuilderClient `npm test -- --run` passed (`50` files, `609` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API command-helper polymorphic stale/no-partial hardening slice. Add dispatch-level wrong-kind/stale polymorphic role rejection tests that prove command audit is not called and output/object/wait/setter/helper side effects remain unapplied, plus BuilderClient fixture tests for malformed polymorphic fixture ids and fake/spread/prototype polymorphic handles.
- Next slice: JavaScript API command-helper polymorphic stale/no-partial hardening slice. Add dispatch-level wrong-kind/stale polymorphic role rejection tests that prove command audit is not called and output/object/wait/setter/helper side effects remain unapplied, plus BuilderClient fixture tests for malformed polymorphic fixture ids and fake/spread/prototype polymorphic handles.
