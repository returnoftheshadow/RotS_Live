...
- Latest completed slice: JavaScript API legacy output command-helper live apply slice. The live dispatcher now treats `do_say`, `send_to_char`, and `send_to_room` as bounded output-only command helpers that do not require persistent mutation authority, resolves only current live role/room handles from the trigger context, writes through existing descriptor queues, and rejects forged or stale output targets without emitting text. `load_obj`, `do_give`, and `do_wait` remain validated command intents but still require persistent mutation authority and have no live side-effect application until their dedicated audited slices.
- Latest validation: focused `JsTriggerDispatch` CTest passed (`145` tests), full root `make test -j16` passed (`1613/1613`), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`50` files, `606` tests), and root/BuilderClient `git diff --check` passed. New Magus, Vincent, and Bazarat agents could not be spawned because the subagent thread pool is full; this remains the current review limitation for new slices.
- Current slice: JavaScript API legacy object command-helper live apply slice. Design and promote live semantics for `load_obj` and `do_give` with object vnum resolution, placement/ownership transfer, carrying limits, crash-save flags, stale-handle checks, audit, rollback/no-partial-write behavior, and BuilderClient offline parity before enabling wait continuations.
- Next slice: JavaScript API legacy object command-helper live apply slice. Design and promote live semantics for `load_obj` and `do_give` with object vnum resolution, placement/ownership transfer, carrying limits, crash-save flags, stale-handle checks, audit, rollback/no-partial-write behavior, and BuilderClient offline parity before enabling wait continuations.
