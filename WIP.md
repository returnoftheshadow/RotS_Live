...
- Latest completed slice: JavaScript command-helper final `MutationResult` bridge planning slice. `FEATURES.md` and `docs/shape_script.md` now record the product decision that builder-facing command helpers should return final operation outcomes when the engine can safely know them during script execution, without requiring duplicate `can*` preflight calls. The planned bridge starts with shared reason-code metadata, then a narrow dispatch-owned QuickJS command callback mode, then `do_give`, followed by `load_obj`, `do_wait`, output helpers, and BuilderClient offline parity.
- Latest validation: documentation-only planning slice; root/BuilderClient `git diff --check` passed. Full root/BuilderClient test suites were not rerun because no executable code changed in this slice.
- Current slice: JavaScript command-helper final `do_give` bridge foundation slice. Add shared command result reason codes and begin the runtime/dispatch callback seam needed for `RotS.Script.doGive()` to return final `inventory-full`, `too-heavy`, `no-drop`, stale, authority, audit, or ok outcomes inline.
- Next slice: JavaScript command-helper final `do_give` bridge foundation slice. Add shared command result reason codes and begin the runtime/dispatch callback seam needed for `RotS.Script.doGive()` to return final `inventory-full`, `too-heavy`, `no-drop`, stale, authority, audit, or ok outcomes inline.
