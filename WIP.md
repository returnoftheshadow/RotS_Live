...
- Latest completed slice: JavaScript API legacy object command-helper live apply slice. The live dispatcher now applies explicit-target `load_obj(vnum, target?)` and current-context `do_give(giver, recipient, object)` helpers after validating prototype vnums, live role handles, direct carried-object ownership, and target-zone authority. `load_obj(vnum, ctx.actor)`/`load_obj(vnum, ctx.room)` place objects through the existing object helpers, one-argument `load_obj(vnum)` remains a validated no-placement intent, and `do_give` transfers through `perform_give()`. Forged/stale targets fail closed without creating or moving objects. BuilderClient typings, offline command events, and runnable examples now mirror the explicit-target shape.
- Latest validation: focused `JsTriggerDispatch`/`JsBuilderArtifacts`/`JsGameRuntime` CTest passed (`214` tests), BuilderClient focused examples/compiler/offline tests passed (`3` files, `96` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1616/1616`), BuilderClient full `npm test -- --run` passed (`50` files, `606` tests), and root/BuilderClient `git diff --check` passed. New Magus, Vincent, and Bazarat agents could not be spawned because the subagent thread pool is full; this remains the current review limitation for new slices.
- Current slice: JavaScript API legacy wait command-helper continuation slice. Design and apply bounded `do_wait` live behavior against the current script host without breaking legacy `SCRIPT_DO_WAIT`, including continuation ownership, wait-state flags, dispatch short-circuit behavior, offline fixture parity, and no-retained-JS-handle guarantees.
- Next slice: JavaScript API legacy wait command-helper continuation slice. Design and apply bounded `do_wait` live behavior against the current script host without breaking legacy `SCRIPT_DO_WAIT`, including continuation ownership, wait-state flags, dispatch short-circuit behavior, offline fixture parity, and no-retained-JS-handle guarantees.
