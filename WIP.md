...
- Active slice: BuilderClient `rots-script publish status` complete. The CLI can query package status through the proxy/test-server publish client, requires explicit package id and bearer token inputs, never reads local package artifacts for status, redacts tokens and suspicious package ids from diagnostics, rejects malformed or mismatched accepted responses, and uses bounded publish response parsing with abortable requests.
- Next slice: reassess and pick the next publish workflow gap, likely CLI activate/rollback transport or renderer activation/rollback wiring.
