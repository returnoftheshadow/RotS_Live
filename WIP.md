...
- Latest completed slice: Legacy `EXTRACT_CHAR` character extraction preflight. `RotS.Script.extractChar(character)` remains non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Legacy scripts extract only NPC targets and clear the legacy character variable; JavaScript promotion must reject player/account-backed targets, require NPC liveness and source-room authority, audit target/source/package context, clean up combat/riding/follower/master/equipment/carried-object side effects, mark extracted handles stale, model offline hidden extraction state, and either reject mixed batches containing extraction or prove complete rollback before descriptor output commits.
- Latest validation: focused root `JsApiStructMapping`/`JsBuilderArtifacts` checks passed before review (`36` tests) and after review (`8` tests), full root `make test -j16` passed (`1683/1683`), BuilderClient `npm run typecheck` passed, focused BuilderClient docs/IPC tests passed (`96` tests), full BuilderClient `npm test -- --run` passed (`53` files, `681` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy `LOAD_MOB` mobile loading preflight. Design `loadMob(vnum, room)` only after mobile prototype catalogs, generated temporary IDs, room membership, script/proc initialization policy, reset-balance effects, audit fields, no returned mutable handle, and offline fixture state are pinned.
