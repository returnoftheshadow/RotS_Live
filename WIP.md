...
- Current slice: JavaScript API `Room.affects` getter classification/promotion final validation and commit.
- Next slice: JavaScript API `Room` residual setter classification/audit. Reconfirm `Room.setExit`, `Room.setFlags`, `Room.setAlignment`, `Room.setLight`, internal `tracks`, and internal `bleedTracks` stay absent or explicitly deferred/unsupported with generated/runtime/offline parity, then move to the next owner surface.
