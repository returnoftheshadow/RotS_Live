...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.followers`/`Character.master` getter assessment slice. Determine bounded follower/master relationship snapshots that avoid raw linked-list traversal and document loop-prevention plus group/mount side-effect helper requirements.
- Next slice: JavaScript API `Character.mount`/`Character.group` getter assessment slice. Determine safe relationship snapshots for mounts and groups while keeping movement, follow, and group membership mutation behind explicit future helpers.
