...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, reviewer-requested stale target/legacy-consume behavior regressions, live per-pulse/per-package JavaScript invocation budgeting, live JavaScript trigger-entry depth guarding, sanitized live JavaScript dispatch failure logging, generated builder-facing runtime safety metadata, and initial script-visible trigger metadata parity are in place before any world mutation/output helpers.
- Latest completed slice: JavaScript trigger metadata parity. Live QuickJS now injects generated `TriggerInfo.kind` and `TriggerInfo.handlerName`, keeps `name` as a documented handler-name compatibility alias, exposes root `ctx.zone` through the generated contract, and BuilderClient fallback/offline execution normalizes fixture trigger metadata to the live script-visible shape (`legacyName`, `handlerName`, `blocksGameplay`) instead of leaking the fixture-editor-only `blocking` field.
- Latest validation: Magus, Vincent, and Bazarat reviewed the script-visible API parity slice. Their trigger metadata and `ctx.zone` findings were addressed in this slice; their broader offline handle-shape findings are recorded as the next slice. Focused server validation passed 95/95 for `JsGameRuntime`, `JsApiContract`, `JsBuilderArtifacts`, and `JsTriggerDispatch`; focused BuilderClient validation passed `npm run typecheck` and 14/14 compiler/offline/parity tests. Full validation passed `git diff --check`, the C++ suite passed 1382/1382, and the BuilderClient production build plus full test suite passed 434/434.
