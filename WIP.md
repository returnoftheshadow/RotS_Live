...
- Latest completed slice: JavaScript API `Character.points` getter assessment slice. `Character.points` is now a frozen read-only `CharacterPoints` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. `Character.setPoints` remains unsupported because direct point writes need separate gameplay/admin helpers for clamping, death handling, combat state, regen, encumbrance, persistence, and audit semantics.
- Latest validation: passed for the `Character.points` getter assessment slice. Focused root mapping/contract/adapter/runtime tests passed (`109` tests) plus targeted bodypart default regression reruns passed, full root `make test -j16` passed (`1491/1491`), BuilderClient focused offline/IPC validation tests passed (`57` tests), BuilderClient `npm run typecheck` passed, and root/BuilderClient `git diff --check` passed.
...
- Next slice: JavaScript API `Character.specials` getter assessment slice. Determine the safe read-only `CharacterSpecials` shape for `char_data.specials`, promote stable scalar/finite-vocabulary diagnostics with live/offline parity if feasible, and keep `Character.setSpecials` unsupported until combat/procedure/timer mutation semantics exist.
