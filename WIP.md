...
- Active slice complete: added canonical staged package digest/version identity helpers for JavaScript packages and connected them to publish preflight metadata without live activation.
- Next slice: add an in-memory staged package repository/service that stores immutable validated package identities for later activate/rollback preflight, still without live activation.
- User requirement:
