...
- Active slice: BuilderClient `rots-script publish activate` complete. The CLI activation command uses the existing publish client transport, requires package id, staged digest, base live checksum, bearer token, proxy/test-server URL, and workspace id, validates locally before network access, redacts token-bearing metadata, rejects malformed accepted responses, and requires activation-specific success metadata including `activate.accepted`, package version id, audit id, matching package id, matching staged digest, and live checksum.
- Next slice: add CLI rollback transport over the existing publish client contract with target live checksum/reason validation, redacted diagnostics, strict accepted-response matching, and proxy/test-server-only access.
