...
- Latest completed slice: JavaScript API `Character.baseAbilities` getter promotion slice. `Character.baseAbilities` is now a frozen read-only `AbilityScores` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. `Character.setBaseAbilities` remains unsupported because direct base-ability writes bypass creation/leveling rules, derived stat recalculation, persistence, and audit semantics.
...
- Next slice: JavaScript API `Character.rolledAbilities` getter promotion slice. Promote `Character.rolledAbilities` as a frozen read-only `AbilityScores` snapshot with live/offline parity, generated typings/docs, adapter/runtime tests, and keep `Character.setRolledAbilities` unsupported until explicit admin/training helper semantics exist.
