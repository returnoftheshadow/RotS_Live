...
- Latest completed slice: JavaScript API `Room.exits` getter classification/promotion. `Room.exits` now exposes frozen `readonly RoomExit[]` snapshots through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter exit copying, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Entries copy direction index/name, destination room vnum or `null` for `NOWHERE`/stale destinations, keyword/description text, key vnum, width, and canonical symbolic exit flags while raw `dir_option` pointers stay unavailable. `Room.setExit` remains deferred until direction, door-state, destination-room, reset-command, bidirectional-link, permission, persistence, rollback, and audit semantics are designed.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused C++ API/adapter/runtime/artifact/manifest tests passed (`186/186`), focused dispatch regression tests passed (`221/221`), reviewer follow-up focused C++ JS/runtime/dispatch tests passed (`361/361`), full root `make test -j16` passed (`1552/1552`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`125` tests), BuilderClient full `npm test -- --run` passed (`49` files, `551` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API `Room.contents` getter classification/promotion planning. Decide whether room contents should remain deferred or expose bounded shallow read-only object snapshots without bypassing object load/extract/movement authority.
- Next slice: JavaScript API `Room.contents` getter classification. If promoted, expose only bounded direct live room object snapshots with no recursive placement setters and no ownership mutation; otherwise pin the deferral with explicit movement/load/extract helper requirements.
