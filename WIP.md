...
- Latest completed slice: JavaScript API `Zone.x` setter promotion slice. `Zone.setX(value: number): MutationResult` is now promoted through server API mapping, generated typings/manifest/docs, live QuickJS integer validation, target-scoped persistent mutation dispatch with guarded world-map redraw, BuilderClient offline fixtures, and parity tests while `Zone.y` stays planned until its own slice.
- Latest validation: passed for the `Zone.x` setter promotion slice. Focused C++ mapping/runtime/dispatch checks passed (`9` tests), full root `make test -j16` passed (`1439/1439`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/artifact tests passed (`31` tests), BuilderClient full `npm test -- --run` passed (`49` files, `485` tests), BuilderClient `npm run build` passed, and root/BuilderClient `git diff --check` passed. Earlier reviewer hardening follow-up fixed the promoted-getter status pin, hidden helper exposure, mutable JS intrinsic reliance, non-canonical coordinate envelopes, unsafe redraw on stale bad coordinates, lower-bound coverage, and mixed-batch atomicity coverage before final validation.
- Current slice: JavaScript API `Zone.y` setter promotion slice. Promote `Zone.setY(value: number): MutationResult` through server API mapping, generated typings/manifest/docs, live QuickJS integer validation, target-scoped persistent mutation dispatch with guarded world-map redraw for committed global zone writes, BuilderClient offline fixtures, and parity tests.
- Next slice: JavaScript API `Zone.y` setter promotion slice. Implement fail-closed validation for non-integer, negative, and greater-than-`25` y coordinates with the same helper hiding, immutable intrinsic, value-kind, and atomic mutation guarantees now pinned for `Zone.x`.
