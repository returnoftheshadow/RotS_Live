...
- Latest completed slice: Legacy `LOAD_OBJ_X` clone-helper preflight hardening. The non-callable `cloneObjectFrom(source, destination?)` policy now explicitly requires an explicit source object handle instead of the legacy `ob1` temp slot, prototype-backed source-object liveness, prototype-default versus live-instance copy semantics, optional destination custody validation, capacity and weight preflight, audit metadata, hidden offline clone state, and rollback/no-partial behavior for created clones before any callable promotion.
...
- Next slice: Legacy object lookup helper preflight hardening. Tighten the non-callable `findInventoryObject`, `findEquippedObject`, and `findRoomObject` designs around vnum/name matching, deep versus direct inventory search, equipment-slot bounds, room-content ownership, absent/multiple-match result codes, stale handles, hidden offline lookup catalogs, and rollback-safe interaction with later custody helpers before any callable promotion.
