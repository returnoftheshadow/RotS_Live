...
- Current slice: JavaScript API room flag helper public binding slice. Add guarded `Room.addFlag(name: RoomFlagName)` and `Room.removeFlag(name: RoomFlagName)` methods to generated/server typings, live QuickJS room handles, BuilderClient fallback/offline handles, and builder docs; add concrete example files under `BuilderClient` and reference them from `docs/shape_script.md`; preserve authority enforcement, audit-before-apply, rollback, and absence of raw `Room.setFlags`.
- Next slice: JavaScript API room flag helper public binding slice. Add guarded `Room.addFlag(name: RoomFlagName)` and `Room.removeFlag(name: RoomFlagName)` methods to generated/server typings, live QuickJS room handles, BuilderClient fallback/offline handles, and builder docs; add concrete example files under `BuilderClient` and reference them from `docs/shape_script.md`; preserve authority enforcement, audit-before-apply, rollback, and absence of raw `Room.setFlags`.
