...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API remaining character getter audit follow-up after `Character.profile`. Confirm that `Character.group` and raw linked-list/runtime fields stay deferred/internal until safe lifetime/helper models exist, and identify the next getter/setter family that still lacks a final implemented/deferred/unsupported audit.
- Next slice: JavaScript API next remaining getter/setter audit family after the character follow-up, likely object extra-description/affect/value-domain helpers or room nested-list/exit helper design depending on the mapping catalog gaps found.
