...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, reviewer-requested stale target/legacy-consume behavior regressions, live per-pulse/per-package JavaScript invocation budgeting, live JavaScript trigger-entry depth guarding, sanitized live JavaScript dispatch failure logging, generated builder-facing runtime safety metadata, script-visible trigger metadata parity, BuilderClient offline live-shaped handle normalization, BuilderClient offline role/scalar fixture modeling, and BuilderClient fixture template/editor support for those payload fields are in place before any world mutation/output helpers.
- Latest completed slice: BuilderClient fixture template/editor payload support. Trigger templates now seed manifest-declared nullable roles/scalars only, use live-shaped identity defaults for Mudlle damage/death targets, and avoid actor defaults when the manifest omits actor. The fixture editor can now edit all generated/live context role handles and scalar payloads, clear nullable handles by blanking ids, validate live target-type/wear-slot/direction vocabularies, and model common readable handle properties such as race, prototype vnum, experience, rank, hit points, NPC/player flags, and room sunlit flags.
- Latest validation: Magus, Vincent, and Bazarat reviewed the fixture template/editor payload-support slice. Their field-driven template, target identity, clearable nullable handle, finite scalar vocabulary, shared target-type constants, exhaustive scalar callback, expanded handle-field, omission-template, scalar-boundary, and wiring-guard findings were addressed. Focused BuilderClient validation passed `npm run typecheck` and 90/90 targeted fixture editor/template, IPC validation, offline runner, project-loader, and wiring tests. Full BuilderClient production build plus full test suite passed 457/457.
