...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, and live Mudlle `SPECIAL_SELF` heartbeat tick backing are in place before any world mutation/output helpers.
- Latest completed slice: JavaScript context manifest/API parity cleanup. The server-owned manifest now exposes live-backed `ctx.room` for `ON_DIE`, receive, hear, damage, and object action triggers; removes `target` from the `SPECIAL_COMMAND` required context fields because typed targets are backed by `SPECIAL_TARGET`; keeps `SPECIAL_DELAY` continuation unsupported without advertising a runtime context field; documents `SPECIAL_TARGET` command/args backing; and pins unsupported Mudlle target type names plus `SPECIAL_COMMAND` target defaults in tests.
- Latest validation: Magus, Vincent, and Bazarat reviewed the parity cleanup. Addressed Magus's `ON_DIE` room, `SPECIAL_COMMAND` target-field, and `SPECIAL_DAMAGE` wording findings; Vincent's deferred continuation/high-frequency/safe-side-effect cautions remain documented future work; Bazarat's unsupported continuation, null target defaults, and exact unsupported target-name coverage were addressed while the brittle legacy-consumes-before-JS Mudlle text fixture remains a follow-up. Focused manifest/API/builder-artifact/adapter/dispatch validation passed 165/165, `git diff --check` passed, and the full C++ suite passed 1356/1356.
- Next slice: add the remaining behavior-test hardening from review, starting with a reliable legacy `SPECIAL_SELF` consume-before-JavaScript regression if Mudlle fixture syntax can be pinned, then stale `TARGET_OBJ` target_data coverage; after that reassess whether side-effect/output API planning can begin.
