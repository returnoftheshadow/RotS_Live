...
- Latest completed slice: Legacy random/war-status/condition examples. Added readonly `ctx.randomRolls` and `ctx.warStatus` fixture controls plus executable `1840-random-war-condition.ts`, so migrated `SET_INT_RANDOM`, `SET_INT_WAR_STATUS`, and hunger/thirst/drunk condition branches can be tested offline with deterministic inputs. Builder docs explain that `ctx.randomRolls` replaces chance calls in fixtures, `ctx.warStatus` uses legacy `1`/`-1`/`0` fame-war semantics, and `ctx.actor.specials2.conditions` carries condition snapshots.
- Latest validation: focused root `JsApiContract`/`JsBuilderArtifacts`/`JsGameRuntime` checks passed (`91` tests), BuilderClient `npm run typecheck` passed, focused BuilderClient example/offline/IPC tests passed (`227` tests), and root/BuilderClient `git diff --check` passed.
