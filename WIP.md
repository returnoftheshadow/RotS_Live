...
- Latest completed slice: JavaScript API command-helper polymorphic detached-room and IPC validation slice. Dispatch now pins detached room-valued `targ1`/`targ2` aliases as rejected for both `send_to_room` and `load_obj` before command audit and before mixed-batch side effects. BuilderClient IPC validation now rejects malformed fixture handle suffixes for polymorphic handles, including `ctx.targ1.id` and `ctx.targ2.id`, before offline normalization can rewrite them to role aliases.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused polymorphic dispatch CTest passed (`6` tests), focused BuilderClient IPC/offline runner tests passed (`163` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1637/1637`), full BuilderClient `npm test -- --run` passed (`50` files, `611` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API command-helper object-command compensation design slice. Add or document staged/rollback behavior for multi-command object batches where a later object command fails after an earlier object load/give applies, then pin the chosen policy with no-partial tests.
- Next slice: JavaScript API command-helper object-command compensation design slice. Add or document staged/rollback behavior for multi-command object batches where a later object command fails after an earlier object load/give applies, then pin the chosen policy with no-partial tests.
