...
- Latest completed slice: JavaScript API command-helper handle authenticity and audit slice. Live QuickJS and BuilderClient offline command helpers now require invocation-created, non-read-only handles tracked through a private WeakSet before serializing character/object/room command targets, so builder-created object literals, spreads, prototype copies, borrowed `isValid` functions, and cloned ids fail without queuing command events. Dispatch also has a command-helper audit callback for output/object/wait helper batches; audit rejection clears pending setters, room flags, object commands, waits, and output before any live side effects. The private handle brand is shadowed from builder source and omitted from `globalThis`.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused `JsGameRuntime`/`JsTriggerDispatch` CTest passed (`217` tests), focused BuilderClient offline runner test passed (`75` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1626/1626`), BuilderClient full `npm test -- --run` passed (`50` files, `607` tests), and root/BuilderClient `git diff --check` passed. Reviewer follow-up drove forged-handle tests, private handle branding, command audit no-partial coverage, and the remaining rollback/metadata follow-up.
- Current slice: JavaScript API command-helper rollback and audit metadata completion slice. Expand command-helper audit payloads with dispatch/package/request metadata where available, wire activation-grade audit policy, add broader malformed-id tables for every command target field, and design rollback or staged-commit handling for rare apply-time object/wait failures after preflight.
- Next slice: JavaScript API command-helper rollback and audit metadata completion slice. Expand command-helper audit payloads with dispatch/package/request metadata where available, wire activation-grade audit policy, add broader malformed-id tables for every command target field, and design rollback or staged-commit handling for rare apply-time object/wait failures after preflight.
