...
- Latest completed slice: JavaScript API `Character.mount` getter assessment and `Character.group` deferral slice. `Character.mount` is now a frozen read-only `MountData` snapshot with nullable shallow `mount`, `rider`, and `nextRider` relationship snapshots plus `isRiding`/`isMounted`. Live adapter snapshots require live reciprocal mount/rider stored-number checks before exposing relationship data and validate rider-chain links before reading `next_rider` fields. BuilderClient offline fixtures, fallback typings, fixture validation, and local execution now mirror the live mount shape while stripping recursive handles. `Character.group` remains deferred because the adapter does not yet have a live `group_data` registry or equivalent lifetime token that can fail closed before dereferencing raw group pointers; `setMount` remains deferred and `setGroup` remains unsupported.
- Latest validation: passed for the `Character.mount` getter assessment and `Character.group` deferral slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ API/adapter/runtime/artifact tests passed (`155` tests), full root `make test -j16` passed (`1529/1529`), BuilderClient focused offline/IPC validation tests passed (`76` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `510` tests), and root/BuilderClient `git diff --check` passed.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API remaining character relationship/state getter audit. Confirm whether any remaining `char_data` relationship/state fields after mount/group can be safely exposed as read-only snapshots, otherwise keep them documented as deferred/unsupported with tests.
- Next slice: JavaScript API remaining character relationship/state getter audit follow-up implementation, based on the remaining `char_data` field audit results.
