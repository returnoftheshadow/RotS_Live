...
- Latest completed slice: JavaScript API object flag/scalar getter promotion. `GameObject.flags` is now in the server API contract, implemented read-only struct mapping status, live QuickJS snapshots as a deeply frozen `ObjectFlags` object, and BuilderClient offline normalization/IPC validation/typings with server-vocabulary drift guards. The promoted fields are symbolic item type, wear flags, extra flags, material, and scalar level/weight/cost/rent/timer/rarity fields. Raw `obj_flags` bitvectors and legacy `value[5]` slots remain unavailable until they have item-type-specific builder-facing domains.
- Latest validation: passed for the object flag/scalar getter slice. Focused C++ getter/manifest/runtime/adapter tests passed (`7` tests); previously failing artifact/doc/literal checks passed after doc wording updates (`5` tests); full `make test` passed (`1404/1404`); BuilderClient `npm run typecheck`, `npm run build`, isolated offline runner rerun (`17` tests), and full `npm test` passed (`49` files, `480` tests). Magus, Vincent, and Bazarat review findings were addressed before commit.
- Current slice: JavaScript API object flag/scalar getter slice commit.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API object affect/deferred-classification slice: classify object affects, nested relationships, and remaining object list/relationship fields before setter foundation.
  - [ ] JavaScript API setter foundation slice: add a validated mutation-result contract for explicit setter methods without exposing raw struct writes.
