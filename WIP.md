...
- Latest completed slice: JavaScript API `Character.professions` getter assessment slice. `Character.professions` is now a frozen read-only `readonly Profession[]` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes canonical mage, mystic, ranger, and warrior profession progression entries with key, name, level, points, coefficient, and experience when a live character has `char_data.profs`; color settings and specialization state stay out of this surface. `Character.setProfessions` remains unsupported because direct profession writes would bypass profession-domain, level-progression, skill recalculation, training/admin, persistence, and audit semantics.
- Latest validation: passed for the `Character.professions` getter assessment slice. Full root `make test -j16` passed (`1494/1494`), BuilderClient focused offline/IPC validation tests passed (`58` tests), BuilderClient `npm run typecheck` passed, and Magus, Vincent, and Bazarat review is complete with no blocking findings.
...
- Next slice: JavaScript API `Character.specializations` getter assessment slice. Determine the safe read-only public specialization shape for `char_data.extra_specialization_data` and `char_prof_data` specialization fields, promote stable diagnostic data with live/offline parity if feasible, and keep `Character.setSpecializations` unsupported until class-specific invariants, respec rules, combat bonuses, persistence, and audit semantics exist.
