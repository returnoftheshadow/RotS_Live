...
- Current slice: JavaScript API object raw value/flag domain getter classification. Completed; commit the server and BuilderClient changes.
- Next slice: JavaScript API `Room.extraDescriptions` getter classification/promotion. Decide whether room extra-description linked-list text can reuse the bounded frozen `ExtraDescription[]` snapshot policy from objects; then align mapping docs, contract/generated typings, live adapter/runtime/offline fixtures, IPC validation, and absence of whole-list setters.
