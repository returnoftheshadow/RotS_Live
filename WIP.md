...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_ENTER` scalar payload backing, and live Mudlle typed target/damage/death backing are in place before any world mutation/output helpers.
- Latest completed slice: Mudlle typed target/damage/death backing. The live special dispatcher now runs targeted `mudlle-mobile` JavaScript packages for `SPECIAL_TARGET`, `SPECIAL_DAMAGE`, and `SPECIAL_DEATH`; exposes safe typed live character/object/room target snapshots through `target`, `targ1`, `targ2`, `targetTypes`, and `dying`; keeps unsupported legacy union payloads as `null` plus type names; and pins `SPECIAL_TARGET` errors fail-open while `SPECIAL_DAMAGE`/`SPECIAL_DEATH` errors fail-closed.
- Latest validation: Magus, Vincent, and Bazarat reviewed the Mudlle typed target slice. Follow-up fixes made activated targ2-host target/damage calls expose the activated host as the primary target/victim, added TARGET_ROOM pointer coverage, expanded manifest/API/generated TypeScript target fields, required `TARGET_CHAR` `target_data` to match both live pointer and `ch_num`/`abs_number`, prevented stale explicit targets from falling back to unrelated target slots, made Mudlle manifest context-field tests exact-set checks, and added `room` to `SPECIAL_DEATH` manifest metadata. Focused JavaScript/API/manifest/dispatch validation passed 223/223, `git diff --check` passed, and the full C++ suite passed 1351/1351.
