...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, stable explicit defaults for remaining documented context payload fields, and live Mudlle `SPECIAL_COMMAND`/`SPECIAL_ENTER` scalar payload backing are in place before any world mutation/output helpers.
- Latest completed slice: Mudlle special scalar payload backing. The live special dispatcher now runs targeted `mudlle-mobile` JavaScript packages for Mudlle mobile program hosts after legacy special handling declines to consume the call, with `SPECIAL_COMMAND` exposing `command`/`args` and `SPECIAL_ENTER` exposing `direction`/`reverseDirection`.
- Latest validation: Magus, Vincent, and Bazarat reviewed the Mudlle scalar slice. Follow-up fixes normalized leading command whitespace, kept `SPECIAL_COMMAND`/`SPECIAL_ENTER` runtime errors fail-open, guarded invalid Mudlle tactics before `PROG_NUMBER()`, used unsigned-char whitespace classification, refreshed API contract docs, and added live regression coverage for tactics-slot selection, invalid scalar indexes, runtime-error fail-open behavior, live-snapshot guard skips, and legacy-before-JavaScript ordering. Focused JavaScript/API/manifest/dispatch validation passed 210/210, `git diff --check` passed, and the full C++ suite passed 1338/1338.
