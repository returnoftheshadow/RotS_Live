...
- Latest completed slice: JavaScript command-result offline descriptor-buffer failure parity. BuilderClient now has regression coverage proving `no-recipient`, invalid target, and invalid text output helpers do not queue committed descriptor output, while later accepted fallback output still appears in both source-order command logs and committed descriptor output.
- Latest validation: BuilderClient `npm run typecheck` passed and focused BuilderClient offline runner tests passed (`111` tests).
- Next slice: JavaScript command-result offline descriptor-buffer bounding parity. Bound descriptor output entries and total payload size so accepted output buffers cannot overwhelm offline result rendering.
