...
- Latest completed slice: Legacy `TELEPORT_CHAR_XL` target-room movement parity. `RotS.Script.teleportCharToTargetRoom(character, target)` and the `teleport_char_xl` migration alias now compile through generated/fallback typings, live QuickJS command bridging, dispatch inline result preflight, transaction apply, BuilderClient offline execution, and builder docs. Live dispatch resolves the destination from the target character's current room, requires persistent target-zone authority for the moved character, target character, and destination room, rejects stale/no-room targets, blocked rooms, and `NO_TELEPORT`, audits before accepting, leaves followers behind, stops riding, emits no movement text, does not run movement triggers, and stays in the same reversible character-movement transaction phase as `teleportCharOnly`. Offline fixtures resolve the target character's hidden per-run current room while keeping visible snapshots frozen, return the same `invalid-target`/`blocked-room`/`no-teleport` result codes, and keep object/drop helpers aligned to live category order.
- Latest validation: full root `make test -j16` passed (`1683/1683`), BuilderClient `npm run typecheck` passed, focused BuilderClient offline/IPC/example tests passed (`226` tests), full BuilderClient `npm test -- --run` passed (`53` files, `681` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy `EXTRACT_CHAR` character extraction preflight. Design `extractChar(character)` only after player/account-backed character rejection, NPC extraction side effects, follower/master cleanup, combat/riding cleanup, audit fields, rollback/no-mixed-batch policy, and offline fixture state are pinned.
...
- Next slice: Legacy character movement helper slice. Add audited `loadMob`, teleport/extract/follow/flee helper design and initial live/offline parity around room/follower/combat/liveness semantics.
...
- Small-slice execution queue:
  ...
  - [~] Legacy character movement helper slice: `teleportChar`, `teleportCharOnly`, and `teleportCharToTargetRoom` are callable; add audited `loadMob`, extract, follow, and flee helpers.
