...
- Current slice: JavaScript API room flag helper transaction plumbing slice. Add the internal room-target token and rollback-safe apply scaffolding for `room.flags.add/remove` helper envelopes while keeping helper methods non-callable until live/offline execution and generated docs are ready.
- Next slice: JavaScript API room flag helper transaction plumbing slice. Add the internal room-target token and rollback-safe apply scaffolding for `room.flags.add/remove` helper envelopes while keeping helper methods non-callable until live/offline execution and generated docs are ready.
