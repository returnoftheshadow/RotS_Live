...
- Latest completed slice: JavaScript command-result offline descriptor-buffer probe foundation. BuilderClient now has a non-script-visible descriptor-output buffer probe that accepts validated output command intents, reports queued output separately from committed descriptor output, supports queued-only mode, and rejects unsupported operations, invalid targets, and empty text before commit.
- Latest validation: BuilderClient `npm run typecheck` passed and focused BuilderClient offline runner tests passed (`110` tests).
- Next slice: JavaScript command-result offline descriptor-buffer execution parity. Wire accepted offline `do_say`, `send_to_char`, and `send_to_room` calls into private per-run descriptor output buffers while preserving existing source-order command logs.
