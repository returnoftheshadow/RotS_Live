...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, reviewer-requested stale target/legacy-consume behavior regressions, live per-pulse/per-package JavaScript invocation budgeting, live JavaScript trigger-entry depth guarding, sanitized live JavaScript dispatch failure logging, generated builder-facing runtime safety metadata, script-visible trigger metadata parity, BuilderClient offline live-shaped handle normalization, and BuilderClient offline role/scalar fixture modeling are in place before any world mutation/output helpers.
- Latest completed slice: BuilderClient offline role/scalar fixture modeling. Offline fixtures now preserve and normalize `speaker`, `attacker`, `victim`, `killer`, `weapon`, `wearSlot`, `command`, `args`, `target`, `tick`, `direction`, `reverseDirection`, `targ1`, `targ2`, `targetTypes`, and `dying` so local scripts see the same generated/live `ScriptContext` fields instead of hardcoded null/defaults. Fixture validation now enforces role-specific handle prefixes, the live two-slot Mudlle target-type vocabulary, nested handle validation, present-but-empty scalar strings, and CLI project fixture validation; offline runner log capture and failure diagnostics are bounded before returning to the UI/CLI.
- Latest validation: Magus, Vincent, and Bazarat reviewed the role/scalar fixture-modeling slice. Their role-kind, target-type vocabulary, target-kind inference, malformed nested fixture, empty-scalar, default/null, populated read-only array, CLI validation bypass, log bounding, and diagnostic bounding findings were addressed. Focused BuilderClient validation passed `npm run typecheck` and 120/120 targeted offline runner, IPC validation, workspace fixture, IPC handler, project-loader, and CLI tests. Full validation passed root `git diff --check`, the C++ suite passed 1382/1382, and the BuilderClient production build plus full test suite passed 449/449.
