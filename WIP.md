...
- Latest completed slice: JavaScript API `Character.specializations` getter assessment slice. `Character.specializations` is now a frozen read-only `SpecializationData` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes persisted selected specialization id/key/name, runtime current specialization id/key/name, mage-specialization classification, and runtime-state presence while intentionally excluding specialization subclass counters, exposed character targets, off-hand object pointers, timestamps, and direct mutation. `Character.setSpecializations` remains unsupported because direct specialization writes would bypass class-specific invariants, respec rules, combat bonuses, persistence, and audit semantics.
- Latest validation: passed for the `Character.specializations` getter assessment slice. Full root `make test -j16` passed (`1496/1496`), BuilderClient focused offline/IPC validation tests passed (`59` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `493` tests), root/BuilderClient `git diff --check` passed, and Magus, Vincent, and Bazarat reported no blocking findings. Magus's low-severity fallback-typing request was addressed by narrowing `SpecializationData.selectedKey/currentKey` to a finite `SpecializationKey` union.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API remaining getter/setter completion audit slice.
- Next slice: JavaScript API `Character.damageDetails` getter assessment slice. Determine the safe read-only combat bookkeeping shape for `char_data.damage_details`, promote stable diagnostic data with live/offline parity if feasible, and keep `Character.setDamageDetails` unsupported because combat, timer, trigger, and persistence helpers own those counters.
