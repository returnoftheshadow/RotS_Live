...
- Electron Builder publish fix: BuilderClient `package:linux` and `package:win` now pass `--publish never` so GitHub Actions only uploads workflow artifacts and Electron Builder does not try to auto-publish through GitHub Releases when it detects CI. Added a package-script regression test for the publish mode.
- Validation: BuilderClient `npm run package:linux` passed locally and completed AppImage creation without a `GH_TOKEN` publish failure, BuilderClient `npm run typecheck` passed, focused package-script/renderer tests passed (`2` tests), full BuilderClient `npm test -- --run` passed (`52` files passed, `1` skipped, `648` tests passed, `12` skipped), and root/BuilderClient `git diff --check` passed.
- Packaged TypeScript library fix: BuilderClient now copies TypeScript's default `lib.*.d.ts` declarations into `dist-electron/typescript-lib` with a package-safe `.txt` suffix during `build:main`, and the runtime script compiler falls back to those bundled declarations when Electron Builder's packaged `node_modules/typescript/lib` copy omits `.d.ts` files. This fixes packaged Windows script compilation errors such as missing global `Array`/`Object` types and missing `lib.es2022.full.d.ts`.
- Validation: BuilderClient `npm run build:main` passed and emitted the bundled TypeScript lib resources, BuilderClient `npm run typecheck` passed, focused package-script/compiler tests passed (`22` tests), full BuilderClient `npm test -- --run` passed (`52` files passed, `1` skipped, `649` tests passed, `12` skipped), BuilderClient `npm run package:linux` passed, packaged `app.asar` contains `dist-electron/typescript-lib/lib.es2022.full.d.ts.txt`, `lib.es2022.d.ts.txt`, `lib.es5.d.ts.txt`, and `node_modules/typescript/lib/typescript.js`, and BuilderClient `git diff --check` passed.
