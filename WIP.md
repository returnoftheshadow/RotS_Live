...
- Latest completed slice: JavaScript API command-helper transaction hardening and documentation audit slice. Server dispatch now requires command audit for mutating command helpers when persistent authority is present, prechecks `load_obj(vnum, character)` capacity before allocation, uses a silent internal `do_give` transfer path instead of `perform_give()`, and keeps a transaction-level object journal so loaded objects and give transfers roll back if later room-flag apply rejects. `FEATURES.md` and `docs/shape_script.md` now define the live helper transaction order and the remaining BuilderClient offline state-emulation gap.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused command-helper dispatch tests passed (`21` tests), full root `make test -j16` passed (`1640/1640`), and root/BuilderClient `git diff --check` passed. BuilderClient was unchanged in this slice.
- Current slice: JavaScript command-helper final `MutationResult` bridge design slice. Decide and begin the safest retrofit for builder-friendly final helper outcomes, likely by introducing narrow immediate C++ command-helper callbacks for `do_give`, `load_obj`, and other side-effect helpers so scripts can branch on real success/failure without duplicate `can*` checks.
- Next slice: JavaScript command-helper final `MutationResult` bridge design slice. Decide and begin the safest retrofit for builder-friendly final helper outcomes, likely by introducing narrow immediate C++ command-helper callbacks for `do_give`, `load_obj`, and other side-effect helpers so scripts can branch on real success/failure without duplicate `can*` checks.
