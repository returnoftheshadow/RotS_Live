...
- Latest completed slice: JavaScript API `GameObject.affects` getter promotion slice. `GameObject.affects`, shallow worn equipment object snapshots, and shallow carried inventory object snapshots now expose frozen read-only `readonly ObjectAffect[]` entries through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter copies from fixed object affect slots, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Empty none-location plus zero-modifier slots are omitted, malformed nonzero legacy slots remain visible for diagnostics, object affect locations preserve the live byte range, and `GameObject.setAffects` remains unsupported pending slot-specific audited helpers.
- Latest validation: passed for the `GameObject.affects` getter promotion slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ JS/API/adapter/runtime/artifact tests passed (`165/165`), full root `make test -j16` passed (`1531/1531`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/IPC/compiler tests passed (`91` tests), BuilderClient full `npm test -- --run` passed (`49` files, `517` tests), and one earlier transient account-menu `SIGPIPE` passed on direct rerun before the clean full rerun.
...
- Current slice: JavaScript API object extra-description getter audit. Classify `GameObject.extraDescriptions` and related extra-description list storage into an implemented bounded frozen getter or keep it deferred with explicit helper requirements, then align generated docs/typings, live/offline absence or snapshot behavior, and tests.
- Next slice: JavaScript API `GameObject.flags` remaining value-domain helper design after the extra-description audit, unless that audit identifies a smaller object getter promotion first.
