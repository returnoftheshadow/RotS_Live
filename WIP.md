...
- Latest completed slice: Legacy `DO_FLEE` movement/combat preflight. `RotS.Script.doFlee(character)` remains non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Legacy `do_flee` rejects berserk characters, can stand lower-position characters, tries up to six random exits, filters `EX_NOFLEE`, `EX_NOWALK`, death-room, NPC stay-zone, and NPC stay-type branches, calls `check_simple_move`, then moves through `do_move(..., SCMD_FLEE)`. JavaScript promotion must use deterministic eligible-exit selection, audit source/destination/combat/follower/mount policy, return branchable movement/combat result codes, roll back movement points, room membership, riding/follower propagation, combat cleanup, `AFF_HUNT`, stay-zone drops, XP loss, and descriptor output, and model hidden offline movement/combat state while keeping visible snapshots frozen.
- Latest validation: focused root `JsApiStructMapping`/`JsBuilderArtifacts` checks passed (`8` tests), full root `make test -j16` passed (`1683/1683`), BuilderClient `npm run typecheck` passed, focused BuilderClient docs/IPC tests passed (`96` tests), full BuilderClient `npm test -- --run` passed (`53` files, `681` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Modern combat/proc effect helper preflight. Design guarded damage, healing, attack, kill, and experience helpers from corpus combat/proc patterns with blocking-trigger semantics, target authority, audit fields, rollback/no-partial policy, and live/offline result-code parity.
...
- Small-slice execution queue:
  ...
  - [ ] Modern combat/proc effect helper slice: add guarded damage, healing, attack, kill, and experience helpers using custom spell/proc examples from the corpus as inputs.
  - [ ] Legacy equipment helper slice: add `doWear`, `doRemove`, `equipChar`, and `ON_WEAR` examples.
