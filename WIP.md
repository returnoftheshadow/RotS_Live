...
- Current slice: JavaScript API `GameObject.container` getter promotion. Commit the completed nullable shallow container snapshot after final diff checks.
- Next slice: JavaScript API `GameObject.contents` getter audit. Decide whether direct contained object lists can be exposed as bounded frozen shallow snapshots with liveness/cycle protections, or keep them deferred with explicit helper/lifetime requirements; then align generated docs/typings, live/offline behavior, and tests.
