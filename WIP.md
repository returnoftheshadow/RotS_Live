...
- Next slice: JavaScript command-result offline loaded-object identity parity. Accepted offline `load_obj(vnum, character)` should register a hidden loaded object identity so later helper probes can reason about object ownership and capacity using a stable generated object id without exposing mutable local object handles to builder scripts.
