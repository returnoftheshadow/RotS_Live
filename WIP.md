...
- Active slice: wire the raw BuilderClient HTTP server-transport boundary into the game descriptor/socket loop while keeping BuilderClient traffic separate from live port `3791` and preserving the Rust proxy trust boundary.
- Next slice: connect the configured BuilderClient test-server port path to the raw transport, mount the session/publish services and loaded-world catalog, and close accepted BuilderClient HTTP requests after a single bounded response.
- Socket-boundary note: the raw HTTP server transport now derives publish/status authority from trusted server state before calling ingress. The descriptor loop still needs an explicit BuilderClient path so Rust proxy requests can reach that boundary without being treated as telnet player traffic.
...
- Small-slice execution queue:
  ...
  - [ ] Server descriptor ingress transport slice: wire the raw BuilderClient HTTP server-transport boundary into the actual game descriptor/socket loop, keep BuilderClient traffic separate from live port `3791`, and preserve the Rust proxy trust boundary.
