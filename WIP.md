...
- Latest completed slice: JavaScript API `Room` residual setter/internal-field classification audit. `Room.setExit`, `Room.setFlags`, `Room.setAlignment`, `Room.setLight`, internal room tracking/bleed tracking setters, BFS scratch setters, and special-procedure setters now have explicit generated contract, live runtime, BuilderClient offline, fallback declaration, compiler, IPC canonicalization, manifest, and documentation absence coverage. Raw internal aliases such as `room_track`, `bleed_track`, `bfs_dir`, `bfs_next`, `funct`, and plausible camelCase variants are stripped or absent from builder-visible room handles. `Room.light` remains an implemented read-only derived light-source counter with an unsupported direct setter.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Full root `make test -j16` passed (`1565/1565`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`143` tests), BuilderClient full `npm test -- --run` passed (`49` files, `569` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API `Room` residual setter/internal-field classification audit final validation and commit.
- Next slice: JavaScript API `Zone` residual setter/internal-field classification audit. Reconfirm `Zone.setMinimumLookLevel`, age/topology/vnum/faction-power/list/reset-command setters, raw owner/reset-command fields, and internal aliases stay absent or explicitly deferred/unsupported with generated/runtime/offline parity before moving to the next owner surface.
