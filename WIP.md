...
- Latest completed slice: JavaScript API `Room.level` setter promotion slice. `Room.setLevel(value: number): MutationResult` is promoted through server API mapping, generated typings/manifest/docs, live QuickJS integer validation, target-scoped persistent mutation dispatch to byte `room_data.level`, BuilderClient offline fixtures, and parity tests. The callable range is integer `0` through `100`; inspection found this is persisted as the room-file level scalar and used by legacy same-level room filtering. `Room.alignment` remains deferred because a clear room-file persistence path was not found, while `Room.sectorType` and `Room.flags` stay read-only because their broad movement/visibility/combat side effects need explicit setter policy.
- Latest validation: passed for the `Room.level` setter promotion slice before commit. Focused C++ mapping/runtime/dispatch/artifact/manifest checks passed (`23` tests), full root `make test -j16` passed (`1463/1463`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`59` tests), BuilderClient full `npm test -- --run` passed (`49` files, `485` tests), BuilderClient `npm run build` passed, and root/BuilderClient `git diff --check` passed.
