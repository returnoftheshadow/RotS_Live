...
- Latest completed slice: JavaScript API room value-domain getter promotion. `Room.sectorType` and `Room.light` are now in the server API contract, implemented read-only struct mapping statuses, live QuickJS snapshots, BuilderClient offline normalization, IPC validation, and role-filtered fixture editor controls.
- Latest validation: passed for the room value-domain getter slice. Focused C++ getter/manifest/runtime/adapter tests passed; focused BuilderClient offline/IPC/editor tests passed; full `make test` passed (`1400/1400`); BuilderClient `npm run typecheck`, `npm run build`, and full `npm test` passed (`49` files, `469` tests). Magus, Vincent, and Bazarat review findings were addressed before commit.
- Next slice: promote or explicitly classify bounded flag/value-domain getters, starting with a filtered `Room.flags` design and object flag/value snapshots while keeping raw reset command arrays, owner/account fields, traversal pointers, descriptors, and function pointers internal/unsupported.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API filtered flag/value getter slice: promote bounded flag/value snapshots and document deferred/unsupported relationship/list/value-domain getters before setter foundation.
