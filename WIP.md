...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, reviewer-requested stale target/legacy-consume behavior regressions, live per-pulse/per-package JavaScript invocation budgeting, live JavaScript trigger-entry depth guarding, sanitized live JavaScript dispatch failure logging, and generated builder-facing runtime safety metadata are in place before any world mutation/output helpers.
- Latest completed slice: JavaScript runtime safety metadata. Extracted the live QuickJS runtime limits, trigger budget/depth limits, and dispatch failure logging policy into a shared server-owned policy; exported that policy through the builder manifest, generated markdown docs, editor config, and BuilderClient artifact model; and kept the generated TypeScript declarations honest by documenting `RuntimeSafetyPolicy` as metadata without exposing a script-visible `RotS.runtimeSafety` value that the live runtime does not inject.
- Latest validation: Magus, Vincent, and Bazarat reviewed the runtime-safety slice. Their findings were addressed by adding the missing test-build policy object, bounding/redacting server-provided policy text in BuilderClient, validating every runtime-safety numeric field, warning on stale generated artifacts, always including compact logging policy metadata, and removing the incorrect script-visible runtime-safety declaration. `git diff --check` passed, focused runtime-safety/server artifact/client validation passed, the full C++ suite passed 1381/1381, and the full BuilderClient build/test suite passed 431/431.
