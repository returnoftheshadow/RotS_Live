...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript runtime source-policy hardening slice.
- Next slice: replace substring-only dynamic-import rejection and harden runtime-code-generation constructor paths with parser/validator-backed source-policy tests before trigger dispatch is broadened.
