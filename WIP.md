...
- Next slice: JavaScript command-result offline source-order documentation and full validation. Document that offline hidden command state is source-order diagnostic state for later helper classification, while live commit order remains server-owned transaction order, then run full BuilderClient/root validation and reviewer follow-up.
