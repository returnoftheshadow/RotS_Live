...
- Latest completed slice: JavaScript API `Character.knowledge` getter assessment slice. `Character.knowledge` is now a frozen read-only `readonly KnowledgeValue[]` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes nonzero computed knowledge entries with skill id, builder-facing name, profession, required level, computed knowledge value, position/cost/beat/target metadata, learning metadata, fast-update flag, and specialization id while excluding the raw `MAX_SKILLS` byte pointer and direct recalculation mutation. `Character.setKnowledge` remains unsupported because knowledge writes must go through audited training/admin helpers that validate derived skill knowledge, body/confusion/teaching/guild recalculation, persistence, and audit semantics.
- Latest validation: passed for the `Character.knowledge` getter assessment slice. Focused C++ API/adapter/runtime tests passed (`124` tests), full root `make test -j16` passed (`1505/1505`), BuilderClient focused offline/IPC validation tests passed (`67` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `501` tests), and root/BuilderClient `git diff --check` passed.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.affects` getter assessment slice. Determine a bounded frozen active-affect snapshot shape that excludes raw linked-list pointers and mutation while documenting add/remove helper requirements.
- Next slice: JavaScript API `Character.equipment` getter assessment slice. Determine a stable per-wear-slot equipment snapshot shape with read-only nested object handles, slot names, stale-handle behavior, and explicit setter/helper deferrals.
