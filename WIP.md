...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.inventory` getter assessment slice. Determine a bounded carried-object snapshot shape that excludes raw linked-list pointers and documents add/remove/container helper requirements.
- Next slice: JavaScript API `Character.followers`/`Character.master` getter assessment slice. Determine bounded follower/master relationship snapshots that avoid raw linked-list traversal and document loop-prevention plus group/mount side-effect helper requirements.
