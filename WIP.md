...
- Latest completed slice: JavaScript API `Character.affects` getter assessment slice. `Character.affects` is now a frozen read-only `readonly Affect[]` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes active affect entries capped at `MAX_AFFECT` with spell/skill type id, builder-facing name, duration, tick phase, modifier, apply-location id/name, affect-flag bitvector id/names, and counter while excluding raw linked-list pointers and direct add/remove mutation. `Character.setAffects` remains deferred because affect writes must go through audited add/remove helpers that preserve duration accounting, affect bit/stat recalculation, combat side effects, room/mount interactions, persistence, rollback, and audit semantics.
- Latest validation: passed for the `Character.affects` getter assessment slice. Focused C++ API/adapter/runtime tests passed (`127` tests), full root `make test -j16` passed (`1508/1508`), BuilderClient focused offline/IPC validation tests passed (`70` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `504` tests), and root/BuilderClient `git diff --check` passed.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.equipment` getter assessment slice. Determine a stable per-wear-slot equipment snapshot shape with read-only nested object handles, slot names, stale-handle behavior, and explicit setter/helper deferrals.
- Next slice: JavaScript API `Character.inventory` getter assessment slice. Determine a bounded carried-object snapshot shape that excludes raw linked-list pointers and documents add/remove/container helper requirements.
