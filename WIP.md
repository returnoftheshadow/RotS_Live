...
- Latest completed slice: JavaScript API public struct mapping catalog. Server-owned metadata now inventories every public top-level field on `char_data`, `obj_data`, `room_data`, and `zone_data`, with getter/setter names, TypeScript types, nullability, getter/setter policy documentation, implemented-vs-planned read status, side-effect class, and unsupported/internal rationale.
- Latest validation: Magus, Vincent, and Bazarat reviewed the public struct mapping catalog. Follow-up fixed `obj_data.item_number` to map to the existing `GameObject.vnum` API, kept object/zone owner identifiers internal-only, added source-level struct-field drift guards, pinned critical implemented mappings against `js_api_contract`, expanded unsafe-field tests, added metadata vocabulary validation, and ran `make test` successfully with `1389/1389` passing.
- Next slice: include the public struct mapping catalog in generated API markdown/manifest output so BuilderClient can surface getter/setter policy docs offline without making planned or unsupported setters callable.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API generated documentation slice: include the public struct mapping catalog in generated API markdown/manifest output so BuilderClient can surface getter/setter policy docs offline.
