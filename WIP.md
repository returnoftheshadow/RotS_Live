...
- Latest completed slice: Legacy `EXTRACT_CHAR` destructive batch-isolation preflight. The non-callable `extractChar(character)` policy now explicitly requires NPC-only target validation, player/account-backed and no-current-room rejection, stale extracted handles, carried/worn object placement into hidden room contents only with a current room, conditional clamped mob-index/load-line count modeling, descriptor/account safety, and mixed-batch rejection unless a future staged-extraction design proves complete rollback before descriptor output commits.
...
- Next slice: Legacy `DO_FOLLOW` relationship-state preflight hardening. Tighten the non-callable `doFollow(follower, leader)` design around reciprocal master/follower links, previous-master replacement, self-follow and loop rejection, protected charm/tame/recruit/pet state, follower caps, message policy, hidden offline relationship state, and rollback before any callable promotion.
