...
- Latest completed slice: Legacy `DO_FOLLOW` relationship-state preflight hardening. The non-callable `doFollow(follower, leader)` policy now explicitly rejects self-follow, already-following, and looped graphs without rewriting the would-be leader's master, requires explicit previous-master replacement policy, pins reciprocal `master`/`followers` links, protected charm/tame/recruit/pet state, follower caps, follow/stop-follow message policy, hunting-memory and pet/group cleanup/restoration, accepted hidden offline relationship state, and rollback before any callable promotion.
...
- Next slice: Legacy equipment helper preflight hardening. Tighten the non-callable `doWear(character, object, slot?)`, `doRemove(character, slotOrObject)`, and `equipChar(character, prototypes)` designs around carried ownership, canonical wear slots, `ON_WEAR` ordering, occupied-slot removal, belt/shield/two-handed constraints, inventory-full room drops, light/object-affect recalculation, prototype loads, no-partial behavior, hidden offline equipment state, and rollback before any callable promotion.
