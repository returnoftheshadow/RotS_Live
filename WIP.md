...
- Latest completed slice: Legacy exchange/reward helper preflight hardening. The non-callable `giveReward(recipient, rewards)` and `exchangeReceivedObject(ctx.object, rewards)` policies now explicitly require multi-reward capacity and weight preflight, explicit default item-return branches, received input item preservation until exchange acceptance, missing reward prototype result codes, lookup failure handling, reward/source/package audit metadata, hidden offline exchange state, and no-partial rollback before any callable promotion.
...
- Next slice: Legacy room-stash/container custody helper preflight hardening. Tighten the non-callable `stashObject`, `moveObjectToRoom`, and container-helper designs around direct ownership, nested-container cycle prevention, container capacity and weight propagation, room custody authority, light/crash-save side effects, hidden offline container state, and rollback/no-partial behavior before any callable promotion.
