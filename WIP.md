...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: classify the remaining object/room nested, list, and value-domain setter groups and promote the first field with clear persistence, validation, side-effect, ownership, live QuickJS, BuilderClient offline, and atomic dispatch semantics.
- Next slice: JavaScript API object/room nested-list setter classification slice, starting with `GameObject.affects`, `GameObject.extraDescriptions`, `GameObject.container`, remaining `ObjectFlags`/value domains, `Room.extraDescriptions`, `Room.exits`, `Room.contents`, `Room.characters`, and `Room.affects`.
