...
- Latest completed slice: JavaScript API `Character.skills` getter assessment slice. `Character.skills` is now a frozen read-only `readonly SkillValue[]` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes nonzero practiced skill entries with skill id, builder-facing name, profession, required level, practice count, position/cost/beat/target metadata, learning metadata, fast-update flag, and specialization id while excluding the raw `MAX_SKILLS` byte pointer and direct training mutation. `Character.setSkill` remains unsupported because skill writes must go through training/admin helpers that validate practice sessions, guild restrictions, derived knowledge, recalculation, persistence, and audit semantics.
- Latest validation: passed for the `Character.skills` getter assessment slice. Focused C++ API/adapter/runtime tests passed (`121` tests), full root `make test -j16` passed (`1502/1502`), BuilderClient focused offline/IPC validation tests passed (`64` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `498` tests), and root/BuilderClient `git diff --check` passed.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.knowledge` getter assessment slice. Determine whether `char_data.knowledge` can expose a bounded frozen diagnostic snapshot with live/offline parity, while keeping direct knowledge mutation unsupported unless a separate audited training/admin helper is designed.
- Next slice: JavaScript API `Character.affects` getter assessment slice. Determine a bounded frozen active-affect snapshot shape that excludes raw linked-list pointers and mutation while documenting add/remove helper requirements.
