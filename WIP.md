...
- Latest validation: focused `JsApiStructMapping` tests passed (`28` tests), focused BuilderClient shape-script example tests passed (`4` tests), and root/BuilderClient `git diff --check` passed. Full root/BuilderClient validation plus reviewer subagent checks are next before commit.
- Next slice: Legacy object movement/extract compatibility slice. Add audited `moveObject`, `extractObject`, `dropObject`, and no-partial rollback tests, keeping legacy opcode names as migration aliases only where useful.
