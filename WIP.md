...
- Current slice: JavaScript API helper mutation gate harness slice. Add the reusable server/offline mutation gate contract for future helper methods: opaque target tokens, target-scoped authority, stale-handle denial, per-helper mutation limits, sanitized `MutationResult`, audit-before-mutation policy, and atomic no-partial-write behavior before any room flag/exit/object/affect helper becomes callable.
- Next slice: JavaScript API helper mutation gate harness slice. Add the reusable server/offline mutation gate contract for future helper methods: opaque target tokens, target-scoped authority, stale-handle denial, per-helper mutation limits, sanitized `MutationResult`, audit-before-mutation policy, and atomic no-partial-write behavior before any room flag/exit/object/affect helper becomes callable.
