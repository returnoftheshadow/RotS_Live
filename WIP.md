...
- Latest completed slice: JavaScript API command-helper polymorphic stale/no-partial hardening slice. Dispatch now resolves explicit `target` aliases only from explicit live target fields and no longer falls back to `targ1`/`targ2`; stale explicit character/object targets and wrong-kind object/output polymorphic aliases reject before command audit and before mixed-batch room-flag, setter, object, wait, or descriptor side effects. BuilderClient offline fixtures reject malformed polymorphic fixture ids before alias branding and cover fake/spread/prototype polymorphic handle rejection.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused polymorphic dispatch CTest passed (`6` tests), focused BuilderClient offline runner tests passed (`79` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1635/1635`), full BuilderClient `npm test -- --run` passed (`50` files, `611` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API command-helper polymorphic detached-room and IPC validation slice. Add detached-room `targ1`/`targ2` no-partial dispatch regressions for `send_to_room` and `load_obj`, plus BuilderClient IPC validation tests for malformed `ctx.targ1.id` and `ctx.targ2.id`.
- Next slice: JavaScript API command-helper polymorphic detached-room and IPC validation slice. Add detached-room `targ1`/`targ2` no-partial dispatch regressions for `send_to_room` and `load_obj`, plus BuilderClient IPC validation tests for malformed `ctx.targ1.id` and `ctx.targ2.id`.
