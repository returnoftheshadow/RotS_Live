...
- Latest completed slice: JavaScript API nullable text/scalar getter promotion. `GameObject.actionDescription`, `Zone.description`, `Zone.map`, `Zone.lifespan`, `Zone.age`, `Zone.topRoomVnum`, `Zone.x`, `Zone.y`, `Zone.symbol`, `Zone.minimumLookLevel`, and `Zone.resetMode` are now in the server API contract, implemented read-only struct mapping statuses, live QuickJS snapshots, BuilderClient offline normalization, IPC validation, and role-filtered fixture editor controls.
- Latest validation: Magus, Vincent, and Bazarat reviewed the nullable text/scalar getter slice. Follow-up preserved new zone scalars through IPC validation, constrained top-level `ctx.zone` fixture handles to zone ids, pinned empty-string nullable text parity, and expanded BuilderClient validation/offline/editor tests. Focused C++/BuilderClient tests passed, full `make test` passed (`1397/1397`), BuilderClient `npm run typecheck`, `npm run build`, and full `npm test` passed (`49` files, `468` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: promote or explicitly classify the remaining safe getter rows that need relationship/list/value-domain handling, starting with frozen snapshot candidates such as `Room.sectorType`, `Room.flags`, `Room.light`, object flag/value snapshots, and bounded extra-description arrays before moving to setter foundation.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API remaining getter classification/promotion slice: promote remaining safe frozen getter snapshots and document deferred/unsupported relationship/list/value-domain getters before setter foundation.
