...
- Latest completed slice: JavaScript API helper offline parity slice. BuilderClient now has a non-script-visible offline runtime mutation transaction probe that mirrors the server mixed setter/helper transaction states for tooling/tests only. It validates owner/property setter surfaces, server-shaped text/null/numeric envelopes, malformed helper envelopes, unknown helper operations, sorted unique helper audit summaries, setter-validation-before-helper-audit ordering, audit-rejection atomicity, and helper-audit success staging while keeping helper probes and helper mutation APIs absent from `RotS`, `globalThis`, and offline script handles.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. BuilderClient focused offline/compiler/IPC tests passed (`3` files, `159` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `585` tests), and BuilderClient `git diff --check` passed. Root validation for the previous server slice remains `make test -j16` (`1584/1584`).
- Current slice: JavaScript API room flag helper server foundation slice. Add the first real helper operation registry/apply shape for room flags without making it builder-callable yet: filtered flag vocabulary, opaque room target token model, authority checks, audit-before-apply, rollback hooks, no-partial-write tests, and live/offline parity requirements.
- Next slice: JavaScript API room flag helper server foundation slice. Add the first real helper operation registry/apply shape for room flags without making it builder-callable yet: filtered flag vocabulary, opaque room target token model, authority checks, audit-before-apply, rollback hooks, no-partial-write tests, and live/offline parity requirements.
