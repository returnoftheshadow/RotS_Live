...
- Latest completed slice: JavaScript API `Character.rolledAbilities` getter promotion slice. `Character.rolledAbilities` is now a frozen read-only `AbilityScores` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. `Character.setRolledAbilities` remains unsupported because direct rolled-ability writes bypass character-creation history, persistence, and audit semantics.
- Latest validation: passed for the `Character.rolledAbilities` getter promotion slice. Full root `make test -j16` passed (`1490/1490`), BuilderClient focused offline/IPC validation tests passed (`56` tests), BuilderClient `npm run typecheck` passed, and root/BuilderClient `git diff --check` passed.
...
- Next slice: JavaScript API `Character.points` getter assessment slice. Determine the safe read-only `CharacterPoints` shape for `char_data.points`, promote only stable scalar diagnostics with live/offline parity if feasible, and keep `Character.setPoints` unsupported until explicit gameplay/admin helper semantics exist.
