...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, reviewer-requested stale target/legacy-consume behavior regressions, and live per-pulse/per-package JavaScript invocation budgeting are in place before any world mutation/output helpers.
...
- Latest validation: Magus, Vincent, and Bazarat reviewed the budgeting slice. Magus and Vincent's fail-closed policy finding was addressed by mapping `BudgetExceeded` through the existing trigger exception policy, adding a centralized script dispatch option helper, documenting the temporary budget defaults, and adding gameplay coverage for before-enter budget exhaustion. Bazarat's denied-attempt, zero-limit, pulse-wrap, and call-site wiring concerns were addressed with focused dispatch and source-guard tests. Focused dispatch/manifest/API validation passed 173/173, `git diff --check` passed, and the full C++ suite passed 1366/1366.
