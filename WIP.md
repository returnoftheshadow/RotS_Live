...
- Latest completed slice: JavaScript API `Character.specials` getter assessment slice. `Character.specials` is now a frozen read-only `CharacterSpecials` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes safe runtime diagnostics while excluding live pointers, aliases, unions, procedure pointers, memory-list entries, immortal poof strings, and overloaded invisibility/procedure stack state. `Character.setSpecials` remains unsupported because direct special-state writes would bypass combat, timer, trigger, prompt, procedure, and persistence semantics.
- Latest validation: passed for the `Character.specials` getter assessment slice. Focused root mapping/contract/adapter/runtime tests passed (`95` tests), full root `make test -j16` passed (`1493/1493`), BuilderClient focused offline/IPC validation tests passed (`58` tests), BuilderClient `npm run typecheck` passed, and root/BuilderClient `git diff --check` passed.
...
- Next slice: JavaScript API `Character.specials2` getter assessment slice. Determine the safe read-only `CharacterSpecials2` shape for `char_data.specials2`, promote stable scalar/finite-vocabulary diagnostics with live/offline parity if feasible, and keep `Character.setSpecials2` unsupported until persistence-sensitive player/NPC flag mutation semantics exist.
