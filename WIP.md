...
- Latest completed slice: JavaScript API filtered room flag getter promotion. `Room.flags` is now in the server API contract, implemented read-only struct mapping status, live QuickJS snapshots as a frozen array with safe array helpers, BuilderClient offline normalization in canonical live order, IPC validation, and role-filtered fixture editor controls. `BFS_MARK`, unnamed bits, and raw/internal flag integers remain unavailable, and flag setters are still deferred.
- Latest validation: passed for the filtered room flag getter slice. Focused C++ getter/manifest/runtime/adapter tests passed (`9` tests); BuilderClient `npm run typecheck` and focused offline/IPC/editor/compiler/constants tests passed (`6` files, `82` tests); full `make test` passed (`1402/1402`); BuilderClient `npm run typecheck`, `npm run build`, and full `npm test` passed (`49` files, `472` tests). Magus, Vincent, and Bazarat review findings were addressed before commit.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API object flag/value getter slice: promote bounded object flag/value snapshots after safe string/value domains are defined.
