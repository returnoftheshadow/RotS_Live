...
- Latest completed slice: JavaScript command-result offline descriptor-buffer execution parity. Accepted offline `do_say`, `send_to_char`, and `send_to_room` now queue private per-run descriptor-output events and `runFixture` returns committed descriptor output separately from existing source-order command logs; descriptor-buffer internals are shadowed from direct builder scope and `globalThis`.
- Latest validation: BuilderClient `npm run typecheck` passed and focused BuilderClient offline runner tests passed (`110` tests).
- Next slice: JavaScript command-result offline descriptor-buffer failure parity. Add regression coverage that `no-recipient`, invalid target, and invalid text output helpers do not queue committed descriptor output.
