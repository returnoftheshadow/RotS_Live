...
- Latest validation: Magus, Vincent, and Bazarat reviewed the depth-guard slice. Their findings were addressed with RAII-scoped depth acquisition, depth-before-budget precedence, cheap `script.cpp` prechecks before snapshot/context work, fail-closed/fail-open policy reuse, and focused tests for runtime-error/interruption unwind, denied-depth no-leak, depth denial not consuming budget, invalid/no-match paths not acquiring depth, zero-limit behavior, facade mapping, and source-guarded call-site wiring. Focused dispatch/manifest/API validation passed 182/182, `git diff --check` passed, and the full C++ suite passed 1375/1375.
