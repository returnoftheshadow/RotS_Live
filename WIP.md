...
- Latest completed slice: JavaScript API `Room.contents` getter classification/promotion. `Room.contents` now exposes frozen `readonly EquipmentObjectSnapshot[]` snapshots through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter room-content copying, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Entries copy only bounded direct live room contents whose `in_room` reciprocally references the room, omit recursive contents/container/owner handles and setter methods, and keep `Room.setContents` unsupported until audited object movement/load/extract helpers own linked-list, light, crash-save, stale-handle, rollback, and audit semantics.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Full root `make test -j16` passed (`1557/1557`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`130` tests), BuilderClient full `npm test -- --run` passed (`49` files, `556` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API `Room.characters` getter classification/promotion planning. Decide whether room characters should remain deferred or expose bounded shallow read-only occupant snapshots without bypassing character movement/teleport/follow/combat authority.
- Next slice: JavaScript API `Room.characters` getter classification. If promoted, expose only bounded shallow read-only character relationship snapshots for direct live occupants with no recursive followers/master/mount/group mutation; otherwise pin the deferral with explicit character movement/teleport/helper requirements.
