...
- Latest completed slice: Legacy equipment helper preflight hardening. The non-callable `doWear(character, object, slot?)`, `doRemove(character, slotOrObject)`, and `equipChar(character, prototypes)` policies now explicitly require carried-object validation before slot lookup, bounded remove-slot validation before equipment array reads, requested-slot `ON_WEAR` blocking before alternate slot fallback, anti-alignment/race-flag zap-to-inventory branches with no-current-room rejection, wear-all pet restrictions for `equipChar`, deliberate rejection of the legacy `SCRIPT_DO_REMOVE` chained-comparison hazard, and `equipChar` no-partial preflight/rollback before any callable promotion.
- Latest validation: focused `JsApiStructMapping` CTest filter passed (`32` tests), BuilderClient `npm run typecheck` passed, focused BuilderClient shape-script docs/examples test passed (`7` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy room/exit mutation helper preflight hardening. Tighten the non-callable `setExitState(room, direction, state)` and `changeExitTo(room, direction, destination)` designs around bounded direction validation, north/direction 0 acceptance, missing-exit rejection, broken-door cleanup, reciprocal reverse-exit mirroring, source-only destination changes, reset-command impact, room-message reachability, hidden offline exit state, and rollback before any callable promotion.
