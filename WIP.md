...
- Latest completed slice: JavaScript API `Character.currentAbilities` getter promotion slice. `Character.currentAbilities` is now a frozen read-only `AbilityScores` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. `Character.setCurrentAbilities` remains unsupported because direct current-ability writes bypass base abilities, active affects, equipment, recalculation helpers, persistence, and audit semantics.
- Latest validation: full root `make test -j16` passed (`1490/1490`), BuilderClient focused offline/IPC validation tests passed (`2` files, `56` tests), BuilderClient `npm run typecheck` passed, root/BuilderClient `git diff --check` passed, and Magus, Vincent, and Bazarat reported no blocking findings.
...
- Next slice: JavaScript API `Character.baseAbilities` getter promotion slice. Promote `Character.baseAbilities` as a frozen read-only `AbilityScores` snapshot with live/offline parity, generated typings/docs, adapter/runtime tests, and keep `Character.setBaseAbilities` unsupported until explicit admin/training helper semantics exist.
