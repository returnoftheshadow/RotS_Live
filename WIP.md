...
- Latest completed slice: JavaScript API next safe setter group selection slice. The next implementation target is `GameObject.setLevel(value: number)` over `obj_data.obj_flags.level`, because the scalar is already exposed read-only as `ctx.object.flags.level`, is persisted by the object loader/shaper/writer path alongside rarity/material/script number, has a bounded builder-facing numeric domain, and has lower immediate side-effect risk than object weight, cost, rent cost, timer, rarity, material, item type, wear flags, extra flags, or raw value slots. Those other object fields remain deferred until their value domains and recalculation/behavior policies are explicit.
- Latest validation: documentation-only selection slice; root/BuilderClient `git diff --check` passed before the prior `Room.level` commit, and this slice requires a fresh diff check before commit.
