...
- Latest completed slice: Legacy `DO_FLEE` movement/combat preflight. The non-callable `doFlee(character)` policy now explicitly requires deterministic eligible-exit selection from an ordered fixture catalog, branchable `MutationResult` codes, hidden offline movement/combat state, no generic player-command path, and rollback of movement points, room membership, follower/mount propagation, combat cleanup, XP loss, `AFF_HUNT`, and descriptor output before any callable promotion.
...
- Next slice: Legacy `EXTRACT_CHAR` destructive batch-isolation preflight. Tighten the non-callable `extractChar(character)` design around NPC-only target validation, stale-handle marking, carried/worn object placement, combat/follower/riding cleanup, descriptor/account safety, mixed-batch rejection versus full rollback, and offline hidden extraction state before any callable promotion.
