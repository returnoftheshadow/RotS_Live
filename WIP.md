...
- Latest completed slice: Room flag helper public binding. Generated and
  ...
  opaque `room.flags.add`/`room.flags.remove` helper envelopes with secretless
...
- Earlier completed slice: BuilderClient beginner fixture presets and one-click
  ...
  panel. Raw fixture status/details/editing are now behind Advanced controls,
...
- Latest CI follow-up validation: clean-temp reproduction with `Noobinabox/rots-world` confirmed every indexed `lib/world/scr/*.scr` file was zero-byte and reproduced the negative script-table count behavior; replacing the empty script catalog with an inert `ci-empty.scr` allowed clean-temp `make smoke-account` to pass; the rendered workflow shell normalized the current world repo into that same inert catalog, preserved mixed real-script catalogs, and failed explicitly for missing indexed script files; `.github/workflows/ci.yml` parsed with `python3`/PyYAML; `git diff --check -- .github/workflows/ci.yml WIP.md` passed; and final clean-temp `make smoke-account` passed.
...
- Latest validation: parent workflow YAML parsed with `python3`/PyYAML, and parent `git diff --check -- .github/workflows/ci.yml WIP.md` passed. Full server/BuilderClient tests were not run because this was a CI trigger-only change.
- Latest completed slice: BuilderClient GitHub Actions downloadable build packaging. The nested BuilderClient repo now has a `main`/manual workflow that installs dependencies, stamps a run-number-derived CI version into the workflow workspace, typechecks, tests, builds, smoke-tests the Electron preload bridge, packages Windows x64 portable and Linux x64 AppImage artifacts with Electron Builder, and uploads the packaged outputs as GitHub workflow artifacts. BuilderClient package metadata now includes schema-valid Electron Builder configuration, release output is ignored locally, README download instructions describe the workflow artifacts, and tests avoid hard-coding the development version so CI-stamped builds preserve publish provenance consistency.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient CI/version/publish identity tests passed (`39` tests), full BuilderClient `npm test -- --run` passed (`51` files, `658` tests), BuilderClient `npm run build` passed, raw `npm run smoke:preload` passed, local `npx electron-builder --linux dir --x64` passed, and local AppImage packaging reached Electron Builder but the helper download returned an upstream `504 Gateway Time-out`. Local `xvfb-run` smoke could not be used because `xvfb-run` is not installed in this shell. Magus/Vincent/Bazarat subagent review was not spawned because this session's subagent tool policy requires an explicit user request for delegation.
- Latest CI follow-up: GitHub Actions `npm ci` failed on Windows with `E401` because the committed BuilderClient lockfile resolved tarballs through an authenticated Azure Artifacts mirror. The lockfile now resolves tarballs through `https://registry.npmjs.org/`, and the workflow forces npmjs through `actions/setup-node` `registry-url`, `npm config set registry`, and `npm ci --registry=...` so public GitHub runners do not inherit a private feed.
- Latest CI follow-up validation: lockfile JSON parsed successfully with no `pkgs.dev.azure.com` references, BuilderClient `npm run typecheck` passed, focused BuilderClient CI/client identity tests passed (`3` tests), and root/BuilderClient `git diff --check` passed. Local `npm ci` against npmjs was not run because npmjs is blocked on this network; the fix is targeted at GitHub-hosted runners.
- Latest CI test-shape follow-up: BuilderClient parent-repo parity tests now skip when the standalone BuilderClient checkout lacks an adjacent RotS parent source tree instead of reading `/home/runner/work/BuilderClient/src`, `/docs`, or `/lib` paths. The parity checks still run in the local nested checkout when the parent files exist.
- Latest CI test-shape validation: BuilderClient `npm run typecheck` passed, focused nested parent-parity tests passed (`4` files, `216` tests), focused temporary standalone BuilderClient copy passed with expected parent-parity skips (`3` files passed, `1` skipped, `202` passed, `14` skipped), full BuilderClient `npm test -- --run` passed (`51` files, `658` tests), and root/BuilderClient `git diff --check` passed.
- Latest Windows CI follow-up: `rots-script package --write` now uses a platform-safe default artifact filename by replacing canonical package-id colons with underscores, so Windows runners can create and read `packages/js_1_character_3001.json`. The CLI executable smoke tests now spawn `npm.cmd` on Windows while keeping `npm` elsewhere, and the test client root uses `fileURLToPath(import.meta.url)` for portable path resolution.
- Latest Windows CI validation: BuilderClient `npm run typecheck` passed, focused BuilderClient `rotsScript` CLI tests passed (`53` tests), full BuilderClient `npm test -- --run` passed (`51` files, `658` tests), and root/BuilderClient `git diff --check` passed.
- Latest Linux CI follow-up: The Electron preload smoke script now appends Chromium's `no-sandbox` command-line switch for the smoke-only headless run so GitHub Linux runners do not abort on an unconfigured `chrome-sandbox` SUID helper. The normal app runtime is unchanged.
- Latest Linux CI validation: BuilderClient `npm run smoke:preload` passed, BuilderClient `npm run typecheck` passed, focused BuilderClient main-process tests passed (`24` tests), and root/BuilderClient `git diff --check` passed.
- Latest Linux CI smoke follow-up: The preload smoke npm script now launches Electron with `ELECTRON_DISABLE_SANDBOX=1` and `--no-sandbox` so sandbox disabling is applied before Electron's Linux SUID sandbox check runs. The earlier in-script `app.commandLine.appendSwitch("no-sandbox")` was too late for the GitHub runner startup failure.
- Latest Linux CI smoke validation: BuilderClient `npm run smoke:preload` passed, BuilderClient `npm run typecheck` passed, focused BuilderClient CI/client identity tests passed (`3` tests), and root/BuilderClient `git diff --check` passed. Vincent, Magus, and Bazarat reported no blockers after reviewing the CI packaging, registry, standalone-test, Windows CLI, and Linux preload-smoke fixes.
- Latest Windows CI spawn follow-up: Built `rots-script` CLI smoke tests no longer spawn `npm`/`npm.cmd`; they invoke the local TypeScript compiler through `process.execPath` with `tsc -p tsconfig.node.json`, then execute the generated `dist-electron/cli/rotsScript.js` artifact. This avoids Windows `execFile` `.cmd` shim failures while preserving coverage for the generated CLI entry point.
- Latest Windows CI spawn validation: BuilderClient `npm run typecheck` passed, focused BuilderClient `rotsScript` CLI tests passed (`53` tests), full BuilderClient `npm test -- --run` passed (`51` files, `658` tests), root/BuilderClient `git diff --check` passed, and Magus/Vincent/Bazarat reported no blockers.
- Latest completed slice: BuilderClient modern TypeScript API cleanup. Starter and legacy-corpus example docs now teach camelCase `RotS.Script` helpers first, generated/fallback typings list camelCase helpers before snake-case migration aliases, primary compiler/offline tests exercise camelCase helpers, alias tests preserve migration coverage, and fixture editor copy distinguishes diagnostic command events from committed descriptor output.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient compiler/offline/example/editor tests passed (`153` tests), full BuilderClient `npm test -- --run` passed (`53` files, `664` tests), focused root `JsBuilderArtifacts`/`JsGameRuntime` CTest filters passed (`76` tests), full root `make test -j16` passed (`1673/1673`), root/BuilderClient `git diff --check` passed, and Magus/Vincent/Bazarat reported no blockers after review follow-up.
- Latest completed slice: Legacy output/social compatibility examples slice. Live and offline `RotS.Script` now expose modern TypeScript helpers for `sendToRoomExcept`, `yell`, `emote`, `social`, and `pageZoneMap`, plus snake-case migration aliases. The live dispatcher bridges them through bounded descriptor output semantics, validates live handles and loaded zone maps before writing, allows output-only scripts without persistent mutation authority, and preserves no-recipient/not-found branchability. BuilderClient docs and executable legacy-corpus examples now teach the new helper family.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient compiler/offline/example tests passed (`144` tests), focused root `JsGameRuntime`/`JsTriggerDispatch`/`JsBuilderArtifacts` tests passed (`271` tests), full BuilderClient `npx vitest run --testTimeout 60000 --run` passed (`53` files, `666` tests), full root `make test -j16` passed (`1674/1674`), and root/BuilderClient `git diff --check` passed. Magus/Vincent/Bazarat reviewer follow-up fixed the example trigger label, alias-order coverage, `not-found` docs, and live `do_yell` same-zone preflight/apply parity.
- Latest completed slice: Modern reward/custody helper design slice. The server-owned `inventory-equipment-object-movement` deferred helper plan now records modern TypeScript helper candidates for active reward/custody corpus patterns: `giveReward`, `exchangeReceivedObject`, typed inventory/equipment/room lookups, stash/move-to-room custody, `extractObject`, wear/remove, and container helpers. The plan requires actor/target authority, zone ownership, reciprocal list validation, crash-save policy, nested-container cycle prevention, receive/wear trigger ordering, batch capacity/weight preflight, hidden accepted custody state in offline fixtures, default return paths, and no-partial rollback for multi-reward/exchange flows without exposing legacy temp object slots or raw ownership setters.
- Latest validation: focused `JsApiStructMapping` tests passed (`28` tests), focused BuilderClient shape-script example tests passed (`6` tests), BuilderClient `npm run typecheck` passed, full BuilderClient `npx vitest run --testTimeout 60000 --run` passed (`53` files, `668` tests), full root `make test -j16` passed (`1675/1675`), root/BuilderClient `git diff --check` passed, and Magus/Vincent/Bazarat reported no blockers after review follow-up.
- Latest completed slice: Legacy object movement/extract compatibility slice. Live and offline `RotS.Script` now expose `moveObject`, `extractObject`, and `dropObject` with snake-case migration aliases. Live dispatch validates invocation-local direct object locations, target-zone authority, no-drop/capacity constraints, command audit, destructive-extract batch isolation, and rollback/no-partial-write behavior for reversible object movement. BuilderClient offline fixtures mirror hidden per-run ownership, room-placement, carry count/weight, stale-handle, no-drop, direct-location, polymorphic target, and isolation behavior while keeping visible snapshots frozen.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient compiler/offline/CI-version tests passed (`148` tests), full BuilderClient `npm test -- --run` passed (`53` files, `675` tests), focused root object-helper CTest filters passed (`5` tests), full root `make test -j16` passed (`1681/1681`), root/BuilderClient `git diff --check` passed, and Magus/Vincent/Bazarat review follow-up fixed docs/contract drift, extract-plus-setter rejection, no-drop room-move rejection, offline direct-location parity, and polymorphic object-helper coverage. Pauli re-review found no blockers.
...
- Latest completed slice: BuilderClient builder tutorial/help documentation. Added builder-facing tutorial and quick-help docs under `BuilderClient/docs/`, covering Git-backed workspaces, TypeScript authoring, IntelliSense/generated artifacts, project metadata, fixture creation, fixture expectations including descriptor output, offline runs, packaging, test-server publishing, result codes, troubleshooting, and safe builder rules. Linked the docs from `BuilderClient/README.md`, `docs/shape_script.md`, and `FEATURES.md`.
- Latest validation: BuilderClient `npm run typecheck` passed, root/BuilderClient `git diff --check` passed, link/path existence checks passed, and Magus/Vincent documentation review findings were addressed.
- Latest completed slice: Legacy script corpus inventory/examples. Scanned active `lib/world/scr/index` entries, added `BuilderClient/docs/legacy-script-corpus.md`, linked it from `docs/shape_script.md`, and added executable supported-subset BuilderClient TypeScript examples from current `.scr` scripts for `#1100`, `#1101`, `#27500`, and `#6300`. The example tests now guard docs links, active-index inventory coverage, happy-path execution, and branch fixtures for non-human gate/reward/fall behavior.
- Latest validation: BuilderClient `npm run typecheck` passed, focused BuilderClient example/compiler tests passed (`24` tests), root/BuilderClient `git diff --check` passed, and Magus/Vincent/Bazarat review findings were addressed by removing invented output/dialogue, documenting supported-subset gaps, adding branch fixtures, and adding the active-index doc guard.
- Latest completed slice: BuilderClient legacy modernization policy documentation. Recorded that legacy `.scr` files drive compatibility coverage and migration examples, while BuilderClient authoring must remain modern TypeScript instead of a numeric ASIMA port.
- Latest validation: BuilderClient `npm run typecheck` passed, and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy output/social compatibility examples slice. Add executable examples and modern TypeScript helpers for `sendToRoomExcept`, `yell`, `emote`, `social`, and `pageZoneMap`, with snake-case aliases only where useful for migration.
...
- Latest completed slice: JavaScript API room flag helper server foundation slice. The server now has an internal `JsApiRoomFlagHelperOperation` catalog for `room.flags.add` and `room.flags.remove` with exact operation names, non-callable helper signatures, filtered allowed room flags, explicit `BFS_MARK`/`PERMAFFECT`/`permanentAffect`/unnamed-bit exclusions, opaque room target token authority policy, audit-before-apply policy, rollback policy, offline parity requirements, and adversarial test focus. No `Room.addFlag`, `Room.removeFlag`, or raw `Room.setFlags` API is script-visible yet.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused root mapping tests passed (`5` tests), full root `make test -j16` passed before the final test-hardening patch (`1585/1585`), focused hardened mapping tests passed again (`5` tests), and root `git diff --check` passed. BuilderClient was unchanged.
- Latest completed slice: JavaScript API room flag helper transaction plumbing slice. Dispatch now exposes an internal `js_trigger_dispatch_room_flag_helper_operation_registry()` derived from the server-owned room flag helper catalog for `room.flags.add/remove`, fails closed if that catalog drifts, accepts only cataloged helper operation envelopes through the helper transaction harness, preserves sorted unique audit summaries, rejects raw/prototype-like/unknown helper names before audit, and proves mixed setter/helper transaction probing can use the registry without exposing `Room.addFlag`, `Room.removeFlag`, or raw `Room.setFlags` to builders.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused dispatch tests passed (`8` tests), full root `make test -j16` passed (`1590/1590`), and reviewer-requested fail-closed registry behavior plus adversarial helper-operation cases were added. BuilderClient was unchanged.
- Latest completed slice: JavaScript API room flag helper target-token and argument validation slice. Room flag helper transaction prep now fails closed without live validation context, requires authority-scoped room target tokens shaped as `room-token:v1:<zone-vnum>:<room-vnum>:<secret>`, validates bounded numeric token fields, checks the token secret against dispatch authority, resolves only loaded rooms in the authorized zone, rejects forged token-zone/loaded-room-zone mismatches, parses strict `{"flag":"..."}` helper arguments, accepts only the cataloged filtered room flag vocabulary, and rejects `BFS_MARK`, `PERMAFFECT`, `permanentAffect`, raw aliases, malformed JSON, missing authority fields, and wrong/missing token secrets before audit. `Room.addFlag`, `Room.removeFlag`, and raw `Room.setFlags` remain non-callable.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused dispatch tests passed (`19` tests), full root `make test -j16` passed (`1597/1597`), and reviewer-requested no-context fail-closed behavior, token secret validation, bounded token parsing, forged-zone rejection, authority-field coverage, and catalog-derived allowed-flag coverage were added. BuilderClient was unchanged.
- Latest completed slice: JavaScript API room flag helper rollback-safe apply scaffolding slice. Room flag helper transactions now stage internal pending add/remove records after target-token and argument validation, audit before apply, apply room flag helpers before scalar setters, track applied helper counts, and roll `room_data.room_flags` back in reverse order if a helper apply precondition fails. Audit rejection and helper apply failure both leave scalar setters unapplied. `Room.addFlag`, `Room.removeFlag`, and raw `Room.setFlags` remain non-callable.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused dispatch transaction tests passed (`25` tests), full root `make test -j16` passed (`1603/1603`), and reviewer-requested no-applier fail-closed behavior, multi-room rollback coverage, invalid-helper no-write apply coverage, and sanitized apply-precondition callback cleanup were added. BuilderClient was unchanged.
- Latest completed slice: JavaScript API room flag helper authorization enforcement slice. Server helper transaction preparation now rejects admin-only room flags with `authority-rejected` before audit unless dispatch authority carries explicit room-flag admin override evidence. Builder-zone flags remain accepted with normal zone scripting authority, blocked flags still reject as invalid arguments before audit, and room-flag audit requests carry sanitized override evidence plus authority classes, authorized zone vnum/index, and staged previous/new membership for validated helper mutations. BuilderClient offline probes mirror the same builder-zone/admin-only/blocked policy and audit shape using explicit fixture override options. `Room.addFlag`, `Room.removeFlag`, and `Room.setFlags` remain absent from generated/fallback typings, live QuickJS handles, and offline script handles until public helper binding is promoted.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused server room-helper/mapping tests passed (`24` tests), focused BuilderClient offline runner tests passed (`73` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1609/1609`), BuilderClient full `npm test -- --run` passed (`49` files, `602` tests), and root/BuilderClient `git diff --check` passed.
- Latest completed slice: JavaScript command-helper bridge-accepted `do_give` recheck coverage slice. Server dispatch tests now pin that a bridge-accepted `do_give` envelope is still re-resolved against live state before mutation prep/apply, fails closed without transfer or mixed-batch setter side effects when ownership has diverged, and transfers exactly once with carry count/weight updates when the accepted envelope remains applicable.
- Latest validation: focused server dispatch tests passed (`7` tests), and full root `make test -j16` passed (`1648/1648`). Magus, Vincent, and Bazarat review follow-up added mixed-batch no-partial assertions, transaction metadata assertions, failure counter assertions, and corrected stale `perform_give()` documentation. BuilderClient was unchanged for this server-only coverage slice.
- Current slice: JavaScript `do_wait` final result bridge retrofit is in validation/commit handoff. Focused and full root/BuilderClient validation have passed; final diff checks and commit are next.
- Next slice: JavaScript output helper final result bridge retrofit. Retrofit `do_say`, `send_to_char`, and `send_to_room` with inline result codes for target liveness, text validation, audit rejection, descriptor/room applicability, and accepted output queuing.
...
- Small-slice execution queue:
  ...
  - [x] JavaScript API helper mutation gate foundation slice: add server-owned guardrails for helper mutation envelopes, audit-before-apply ordering, and mixed setter/helper atomicity without exposing helper methods.
  - [x] JavaScript API room flag helper server foundation slice: add the internal room flag helper operation catalog and filtered flag vocabulary while keeping `Room.addFlag`, `Room.removeFlag`, and raw `Room.setFlags` non-callable.
  - [x] JavaScript API room flag helper transaction plumbing slice: derive dispatch helper operation acceptance from the room flag catalog, fail closed on catalog drift, and prove registry-backed helper envelopes flow through helper transaction prep and mixed transaction probing.
  - [x] JavaScript API room flag helper target-token and argument validation slice: validate opaque room target tokens, target-zone authority, and filtered room flag arguments before audit while keeping room flag helpers non-callable.
  - [x] JavaScript API room flag helper rollback-safe apply scaffolding slice: add pending room flag mutation records with previous-flag capture, commit-after-full-batch-success, and rollback paths while keeping room flag helpers non-callable.
  - [x] JavaScript API room flag helper offline/generated contract parity slice: add BuilderClient/offline fixture parity, generated API docs/typings entries, and tests that keep helper methods absent until the callable helper shape is ready.
  - [x] JavaScript API room flag helper callable-shape design guard slice: add same-room multi-step rollback coverage, pin helper-method absence in generated/fallback typings and live/offline runtime surfaces, and document side-effect/authorization decisions before callable helper promotion.
  - [x] JavaScript API room flag helper callable promotion preflight slice: design public helper method contracts, add offline fixture state-application probes, and keep methods absent until per-flag authority and side-effect policies are pinned.
  - [x] JavaScript command-result offline room-placement probe foundation: add a non-script-visible room object placement probe for accepted `load_obj(vnum, room)` command intents without mutating fixture room contents.
  - [x] JavaScript command-result offline room-placement execution state parity: wire accepted `load_obj(vnum, room)` calls into private per-run room contents state while keeping script-visible snapshots unchanged.
  - [x] JavaScript command-result offline room-placement failure/isolation parity: prove failed room loads leave hidden placement state unchanged and repeated fixture runs start from original room contents.
  - [x] JavaScript command-result offline room-contained object ownership parity: classify room-contained object `do_give` attempts as `not-carried` unless prior accepted helper state moved the object into character ownership.
  - [x] JavaScript command-result offline room/object state review hardening: run full BuilderClient/root validation and set the next handoff to descriptor-buffering parity.
  - [x] JavaScript command-result offline descriptor-buffer probe foundation: add non-script-visible descriptor output buffer probing for queued versus committed output events.
  - [x] JavaScript command-result offline descriptor-buffer execution parity: wire accepted offline output helpers into private per-run committed descriptor output while preserving source-order command logs.
  - [x] JavaScript command-result offline descriptor-buffer failure parity: prove failed output helpers do not queue committed descriptor output.
  - [x] JavaScript command-result offline descriptor-buffer bounding parity: bound committed descriptor output entries and total payload size.
  - [x] JavaScript command-result offline descriptor-buffering parity: model per-run output buffering so offline fixtures can distinguish queued output events from committed descriptor output while preserving source-order diagnostics.
  - [x] JavaScript command-result offline descriptor-buffer review hardening: run full BuilderClient/root validation, reconcile the descriptor-buffer checklist, and set the next handoff.
  - [x] JavaScript command-result offline descriptor-output review follow-up: surface committed descriptor output in builder-facing fixture results/CLI/expectations and tighten probe/bounds/isolation tests.
  - [x] BuilderClient builder tutorial/help documentation: add builder-facing tutorial/help docs and link them from the client README plus shape-script docs.
  - [x] Legacy script corpus inventory/examples slice: scan active `lib/world/scr/index` scripts, add `BuilderClient/docs/legacy-script-corpus.md`, add executable legacy-derived examples under `BuilderClient/examples/legacy-script/`, and use the corpus to seed the next JavaScript helper slices.
  - [x] BuilderClient legacy modernization policy documentation: record that active `.scr` files inform compatibility coverage, but BuilderClient should teach idiomatic TypeScript APIs, fixtures, and high-level helpers instead of porting ASIMA command rows.
  - [x] BuilderClient modern TypeScript API cleanup: update existing docs/examples/generated/fallback typings/offline tests to teach camelCase helpers first, keep snake-case names as migration aliases, present fixture controls as builder outcomes, and convert opcode-shaped future plans into higher-level TypeScript helper designs.
  - [x] Legacy output/social compatibility examples slice: add executable examples and modern TypeScript helpers for `sendToRoomExcept`, `yell`, `emote`, `social`, and `pageZoneMap`, with snake-case aliases only where useful for migration.
  - [x] Modern reward/custody helper design slice: replace raw legacy temp-object workflows with typed helper APIs that cover object lookup, load, custody, transfer, extraction, result codes, and rollback without exposing raw pointer slots to builders.
  - [x] Legacy object movement/extract compatibility slice: add audited object move/extract/drop helpers and no-partial rollback coverage.
  - [x] Legacy character movement helper slice: `teleportChar`, `teleportCharOnly`, and `teleportCharToTargetRoom` are callable; `loadMob`, `extractChar`, `doFollow`, and `doFlee` have non-callable preflight policies.
  - [x] Modern combat/proc effect helper slice: `doHit`, `applyDamage`, `rawKill`, and `gainExperience` have non-callable preflight policies.
  - [x] Legacy equipment helper slice: `doWear`, `doRemove`, and `equipChar` have non-callable preflight policies for wear-slot validation, `ON_WEAR` blocking, inventory/equipment side effects, audit fields, rollback/no-partial behavior, and hidden offline equipment state.
  - [x] Legacy room/exit mutation helper slice: `setExitState` and `changeExitTo` have non-callable preflight policies for door-state and exit-destination helpers with fixture modeling.
  - [x] Legacy food/drink/pull/examine examples slice: add executable examples for `ON_EAT`, `ON_DRINK`, `ON_PULL`, and `ON_EXAMINE_OBJECT`.
  - [x] Legacy random/war-status/condition examples slice: add deterministic fixture controls for chance-based scripts.
  - [x] JavaScript command-result offline source-order diagnostic parity: add offline result metadata/tests that clarify source-order command logs versus committed server category order without changing live transaction ordering.
