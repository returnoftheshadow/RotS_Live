...
- Latest completed slice: JavaScript API zone faction-power getter promotion slice. `Zone.whitePower`, `Zone.darkPower`, and `Zone.magiPower` are now read-only properties through server API mapping, generated API contract/docs/typings, live QuickJS zone snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. Their setters stay unavailable because faction power is derived from live character movement, race/allegiance power, recalculation sweeps, and battlefield-control messaging.
- Latest validation: full root `make test -j16` passed (`1489/1489`), BuilderClient focused offline/IPC validation tests passed after reviewer follow-up (`2` files, `54` tests), BuilderClient `npm run typecheck` passed, root/BuilderClient `git diff --check` passed, and Magus, Vincent, and Bazarat reported no blocking findings.
...
- Next slice: promote the next safe read-only character scalar getter group from `char_data` mapping into live QuickJS snapshots, BuilderClient offline fixtures, generated typings/docs, and parity tests while keeping unsafe direct setters absent.
