...
- Latest completed slice: JavaScript API typed helper mutation envelope slice. Runtime mutation envelopes now carry an explicit `kind` discriminator, generated live QuickJS scalar setters emit `kind: "setter"`, the dispatcher exposes and uses a support check that accepts only scalar setter envelopes, and helper-style envelopes or setter envelopes polluted with helper operation/token/argument fields fail closed before value or target resolution. Focused runtime/dispatch tests pin emitted setter kinds, empty helper-only fields, helper-kind rejection, unknown-kind rejection, and helper-field rejection on setter envelopes.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused root runtime/dispatch/artifact tests passed (`7` tests) after making mutation `kind` required, keeping helper envelope constructors absent from script-visible API, and pinning generated typings absence; full root `make test -j16` passed (`1576/1576`); root `git diff --check` passed. BuilderClient was unchanged and clean.
- Current slice: JavaScript API helper transaction harness slice. Add the concrete server/offline transaction skeleton for future helper methods: operation registry stubs, validation-before-enqueue, audit precondition callback shape, rollback-safe pending mutations, and no-partial-write tests without exposing a builder-callable helper yet.
- Next slice: JavaScript API helper transaction harness slice. Add the concrete server/offline transaction skeleton for future helper methods: operation registry stubs, validation-before-enqueue, audit precondition callback shape, rollback-safe pending mutations, and no-partial-write tests without exposing a builder-callable helper yet.
