...
- Latest completed slice: JavaScript API character scalar getter promotion slice. `Character.classPoints`, `Character.interruptCount`, `Character.interruptTime`, and `Character.specialBusy` are now read-only properties through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. Their setters stay unavailable because direct mutation of class-point, caster interruption, and special-procedure state needs explicit admin/combat/procedure helper semantics.
- Latest validation: full root `make test -j16` passed (`1489/1489`), BuilderClient focused offline/IPC validation tests passed after reviewer follow-up (`2` files, `55` tests), BuilderClient `npm run typecheck` passed, root/BuilderClient `git diff --check` passed, and Magus, Vincent, and Bazarat reported no blocking findings.
...
- Next slice: promote the next safe structured read-only character getter candidate from `char_data` mapping, starting with a bounded `Character.currentAbilities` snapshot if the live ability fields can be documented and normalized without exposing raw mutable structs.
