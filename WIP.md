...
- Latest completed slice: JavaScript API legacy wait command-helper no-continuation apply slice. `RotS.Script.do_wait(pulses)` now applies a bounded legacy `AFF_WAITING` wait-list state to the current character host when dispatch has target-zone builder authority. The live path deliberately uses a normal no-continuation delay command instead of `CMD_SCRIPT`, so no QuickJS context, JavaScript stack, or live handle is retained for later resume. Missing hosts, stale hosts, invalid rooms, out-of-zone authority, and missing authority fail closed without changing wait state; mobile `SPECIAL_SELF` JavaScript dispatch now also skips hosts that still carry `AFF_WAITING`.
- Latest validation: passed after Magus, Vincent, and Bazarat review follow-up. Focused `JsTriggerDispatch`/`JsLegacyTriggerDispatch`/`JsBuilderArtifacts`/`JsGameRuntime` CTest passed (`332` tests), full root `make test -j16` passed (`1620/1620`), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`50` files, `606` tests), and root/BuilderClient `git diff --check` passed.
- Current slice: JavaScript API command-helper parity hardening slice. Add mixed command-helper ordering/no-partial-write coverage across output, object, wait, setter, and helper mutations; add command-helper audit/diagnostic policy coverage for output/object/wait side effects; document semantic differences from legacy ASIMA command sequencing; and decide whether BuilderClient offline fixtures should emulate wait-state changes beyond command-event logging.
- Next slice: JavaScript API command-helper parity hardening slice. Add mixed command-helper ordering/no-partial-write coverage across output, object, wait, setter, and helper mutations; add command-helper audit/diagnostic policy coverage for output/object/wait side effects; document semantic differences from legacy ASIMA command sequencing; and decide whether BuilderClient offline fixtures should emulate wait-state changes beyond command-event logging.
