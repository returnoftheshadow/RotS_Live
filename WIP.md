...
- Current slice: JavaScript API `Zone` residual setter/internal-field classification audit final commit.
- Next slice: JavaScript API `Character`/`GameObject` residual internal-alias audit. Reuse the Room/Zone forbidden-alias pattern to pin remaining raw C aliases and internal traversal/session/ownership fields as absent or stripped from generated/runtime/offline/IPC/compiler surfaces before declaring the public owner handles closed.
