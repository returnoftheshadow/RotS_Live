...
- Latest completed slice: Legacy room-stash/container custody helper preflight hardening. The non-callable `stashObject(object, room)`, `moveObjectToRoomCustody(object, room)`, and container-helper policies now explicitly require direct room/container ownership before removal, no-room-membership rejection instead of the legacy ternary comparison hazard, room custody authority, nested-container cycle prevention, container capacity/counting and nested weight propagation, room light counters, crash-save side-effect policy, hidden offline room/container state, and rollback/no-partial behavior before any callable promotion. `moveObjectToRoomCustody` is a future higher-level custody helper, not the existing callable `moveObject(object, room)` direct placement helper.
...
- Next slice: Legacy object extraction/stale-handle helper preflight hardening. Tighten `extractObject` follow-up policy around recursive contents extraction, stale handles after accepted extraction, room/container/carrier/equipment cleanup, object index count compensation, light/crash-save side effects, hidden offline extraction state, and rollback/no-partial behavior.
