...
- Latest completed slice: JavaScript API `GameObject.container` getter promotion slice. `GameObject.container` now exposes a nullable shallow read-only `EquipmentObjectSnapshot | null` snapshot through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter validation, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Live exposure requires the object to be out of direct room placement, the containing object to be live, and its `contains` list to reciprocally include the root object within bounded cycle-safe traversal. Nested container snapshots intentionally stay shallow: they expose inspection fields, flags, affects, extra descriptions, `room: null`, owner fields as null, `isValid()`, no recursive `container`, and no setter methods. `GameObject.setContainer` remains deferred until audited move/container helpers can preserve capacity, nested weight propagation, linked-list integrity, crash-save, rollback, and trigger semantics.
- Latest validation: passed for the `GameObject.container` getter promotion slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ JS/API/adapter/runtime/artifact tests passed (`172/172`), full root `make test -j16` passed (`1538/1538`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/IPC/compiler tests passed (`101` tests), BuilderClient full `npm test -- --run` passed (`49` files, `527` tests), and root/BuilderClient `git diff --check` passed. Reviewer follow-up changed the public type to the shallow `EquipmentObjectSnapshot | null` surface, added compiler rejection for container setters, guarded live exposure against inconsistent direct-room placement, added traversal cap boundary coverage, preserved nullable IPC validation, and added weapon-role container parity.
- Latest completed slice: JavaScript API `GameObject.contents` getter promotion slice. `GameObject.contents` now exposes a bounded frozen read-only `readonly EquipmentObjectSnapshot[]` snapshot through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter traversal, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Live exposure includes only direct child objects whose `in_obj` back-pointer reciprocally references the root object, are live, are not directly in a room, are not carried, and appear within the first 100 visited cycle-safe contents-list nodes. Entries intentionally stay shallow: inspection fields, flags, affects, extra descriptions, `room: null`, owner fields as null, `isValid()`, no recursive `container`/`contents`, and no setter methods. `GameObject.setContents` remains unsupported pending audited move/load/extract helpers.
- Latest validation: passed for the `GameObject.contents` getter promotion slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ JS/API/adapter/runtime/artifact tests passed (`181/181`), full root `make test -j16` passed (`1547/1547`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/IPC/compiler tests passed (`107` tests), BuilderClient full `npm test -- --run` passed (`49` files, `533` tests), and root/BuilderClient `git diff --check` passed. Reviewer follow-up added fail-closed live contents traversal before dereferencing stale nodes, self-content rejection, wrong-parent/direct-room/cycle/visited-node cap coverage, object-role runtime parity, contents-only compiler diagnostics, IPC cap coverage, offline cap coverage, and recursive `container`/`contents` type drift checks.
...
- Current slice: JavaScript API `GameObject.contents` getter promotion. Completed; commit the server and BuilderClient changes after final diff checks.
- Next slice: JavaScript API `GameObject.touched` getter audit/classification. Decide whether runtime/player-interaction touched state should stay unavailable, become read-only diagnostics, or require a dedicated helper policy; then align mapping docs, generated artifacts, live/offline runtime absence or exposure, and tests.
