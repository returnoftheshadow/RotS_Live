...
- Latest completed slice: JavaScript API `Character.specials2` getter assessment slice. `Character.specials2` is now a frozen read-only `CharacterSpecials2` snapshot through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. It exposes additional persistent character diagnostics, canonical player/NPC act flags, preference flags, hide flags, condition counters, perception/alignment/tactics/shooting/casting state, and two-handed mode while intentionally excluding persistent identity, owner ids, raw roleplay/teaching bitvectors, and authentication failure counters. `Character.setSpecials2` remains unsupported because direct special-state writes would bypass player/NPC flag, preference, persistence, combat, movement, practice, and admin semantics.
- Latest validation: passed for the `Character.specials2` getter assessment slice. Focused root mapping/contract/adapter/runtime tests passed (`53` tests), full root `make test -j16` passed (`1493/1493`), BuilderClient focused offline/IPC validation tests passed (`58` tests), BuilderClient `npm run typecheck` passed, root/BuilderClient `git diff --check` passed, and Magus, Vincent, and Bazarat reported no blocking findings after follow-up fixes.
...
- Next slice: JavaScript API `Character.professions` getter assessment slice. Determine the safe read-only profession coefficient shape for `char_data.profs`, promote stable diagnostic data with live/offline parity if feasible, and keep `Character.setProfessions` unsupported until admin/training/persistence semantics exist.
