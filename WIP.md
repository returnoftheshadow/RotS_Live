...
- Latest completed slice: JavaScript API `GameObject.extraDescriptions` getter promotion slice. `GameObject.extraDescriptions`, shallow worn equipment object snapshots, and shallow carried inventory object snapshots now expose bounded frozen read-only `readonly ExtraDescription[]` entries through server API mapping, generated contract/docs/typings, live QuickJS serialization, live adapter copies from `extra_descr_data` linked lists, BuilderClient fallback typings, offline fixture normalization, IPC validation, and compiler typings. Entries expose copied keyword and description text only, live traversal is cycle-safe and capped at 32 entries, BuilderClient direct-runner snapshots apply the same count/text bounds, and `GameObject.setExtraDescriptions` remains unsupported pending audited add/update/remove helpers.
- Latest validation: passed for the `GameObject.extraDescriptions` getter promotion slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ JS/API/adapter/runtime/artifact tests passed (`169/169`), full root `make test -j16` passed (`1535/1535`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/IPC/compiler tests passed (`96` tests), BuilderClient full `npm test -- --run` passed (`49` files, `522` tests), and root/BuilderClient `git diff --check` passed. Reviewer follow-up added a pre-cap linked-list cycle regression, live adapter text-bound assertions, `ctx.weapon.extraDescriptions` role coverage, nested fixture malformed-input coverage, non-object extra-description rejection in IPC validation, and documentation that extra descriptions are intentional builder-visible authoring content.
...
- Current slice: JavaScript API `GameObject.extraDescriptions` getter promotion commit. Commit the finalized server and BuilderClient changes, then continue into the `GameObject.container` getter audit.
- Next slice: JavaScript API `GameObject.container` getter audit. Decide whether direct container ownership can be exposed as a bounded nullable shallow object snapshot with cycle/liveness protections, or keep it deferred with explicit helper/lifetime requirements; then align generated docs/typings, live/offline behavior, and tests.
