...
- Current slice: JavaScript API room flag helper target-token and argument validation slice. Add the internal opaque room target token shape plus argument parsing for `room.flags.add/remove`, validate only filtered room flag names, reject `BFS_MARK`/`PERMAFFECT`/raw aliases before audit, bind target-zone authority, and keep helper methods non-callable until rollback-safe apply and offline parity are ready.
- Next slice: JavaScript API room flag helper target-token and argument validation slice. Add the internal opaque room target token shape plus argument parsing for `room.flags.add/remove`, validate only filtered room flag names, reject `BFS_MARK`/`PERMAFFECT`/raw aliases before audit, bind target-zone authority, and keep helper methods non-callable until rollback-safe apply and offline parity are ready.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API room flag helper target-token and argument validation slice: validate opaque room target tokens, target-zone authority, and filtered room flag arguments before audit while keeping room flag helpers non-callable.
