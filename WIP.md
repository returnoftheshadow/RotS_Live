...
- Latest completed slice: Modern combat/proc effect helper preflight. `RotS.Script.doHit(attacker, victim)`, `RotS.Script.applyDamage(victim, amount, options)`, `RotS.Script.rawKill(character)`, and `RotS.Script.gainExperience(character, amount)` remain non-callable, but the promotion guardrails are now documented in server-owned helper metadata and builder docs. Promotion must preserve same-room/visibility, peace-room, self-target, charm/master, big-brother, sanctuary, existing-combat, wait-state, victim/weapon `ON_DAMAGE`, recursive-damage, death-threshold, corpse/death cleanup, player-only experience, high-level scaling, audit, rollback or terminal-helper isolation, and hidden offline combat/progression state.
- Latest validation: focused root `JsApiStructMapping`/`JsBuilderArtifacts` checks passed (`37` tests), full root `make test -j16` passed (`1684/1684`), BuilderClient `npm run typecheck` passed, focused BuilderClient docs/IPC tests passed (`96` tests), full BuilderClient `npm test -- --run` passed (`53` files, `681` tests), and root/BuilderClient `git diff --check` passed.
- Next slice: Legacy equipment helper preflight. Design `doWear`, `doRemove`, and `equipChar` after wear-slot validation, carried/equipped object authority, `ON_WEAR` blocking, equipment stat/light/affect recalculation, inventory/equipment side effects, audit fields, rollback/no-partial policy, and live/offline result-code parity are pinned.
