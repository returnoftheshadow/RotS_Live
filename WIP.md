...
- Active planning update: live package execution now matches the BuilderClient/offline compiled TypeScript shape; read-only character, room, direct object-room, direct object-owner, trigger role snapshots, damage weapon parity, requested wear-slot parity, central death-killer parity, trigger-source room metadata parity for live ASIMA call sites, stable explicit defaults for remaining documented context payload fields, live Mudlle `SPECIAL_COMMAND`/`SPECIAL_TARGET` command payload backing, live Mudlle `SPECIAL_ENTER` scalar payload backing, live Mudlle typed target/damage/death backing, live Mudlle `SPECIAL_SELF` heartbeat tick backing, and reviewer-requested stale target/legacy-consume behavior regressions are in place before any world mutation/output helpers.
...
- Latest validation: Magus, Vincent, and Bazarat reviewed the test-hardening slice. Magus's test-name clarity finding was addressed by renaming the regression to post-legacy JavaScript fallthrough; Vincent found no blocking trust-boundary issues; Bazarat's initial observable concern was resolved by documenting the true/allow versus false/block semantics and adding live-object identity assertions. Focused adapter/dispatch validation passed 132/132, `git diff --check` passed, and the full C++ suite passed 1358/1358 before the final test-name/comment/assertion polish; focused validation passed again afterward.
