...
- Current slice: JavaScript API room flag helper rollback-safe apply scaffolding slice. Add internal pending room flag mutation records that capture previous `room_data.room_flags`, prepare add/remove changes after validation and audit, apply only after the full mixed batch succeeds, restore previous flags on failure, and keep helper methods non-callable until offline parity and generated docs are ready.
- Next slice: JavaScript API room flag helper rollback-safe apply scaffolding slice. Add internal pending room flag mutation records that capture previous `room_data.room_flags`, prepare add/remove changes after validation and audit, apply only after the full mixed batch succeeds, restore previous flags on failure, and keep helper methods non-callable until offline parity and generated docs are ready.
...
- Small-slice execution queue:
  ...
  - [ ] JavaScript API room flag helper rollback-safe apply scaffolding slice: add pending room flag mutation records with previous-flag capture, commit-after-full-batch-success, and rollback paths while keeping room flag helpers non-callable.
