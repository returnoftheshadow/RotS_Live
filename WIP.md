...
- Active slice: BuilderClient `rots-script publish rollback` complete. The CLI rollback command uses the existing publish client transport, requires canonical package id, target live checksum, rollback reason, bearer token, proxy/test-server URL, and workspace id, normalizes server URLs, validates locally before network access, redacts token-bearing metadata, and rejects malformed accepted responses unless they match `rollback.accepted`, package version id, staged digest, audit id, matching package id, and matching target live checksum.
- Next slice: reassess remaining publish workflow gaps, likely renderer activation/rollback wiring or end-to-end CLI publish flow documentation.
