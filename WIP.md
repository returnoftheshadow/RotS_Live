...
- Changes:
  - `add_exploit_record(int, char_data*, const kill_contributor_list&)` overload writes PK trophies and death entries from die()'s list; the four-argument form refuses PK/DEATH; `die()` passes `contributors` to both.
  - Test-only writer seam `set_exploit_record_writer_for_testing()` (`src/db.h`) so record content can be pinned without `write_exploits()` touching disk and autosaving.
  - `exploit_record::lVictimID` is now `long`; the legacy `exploits/` binary layout (80 bytes, 16-bit id) is preserved by a private `legacy_exploit_record` in `src/exploits_json.cpp`; JSON accepts any long; `load_exploit_records_for_character()` returns account-native JSON histories directly instead of round-tripping them through the 16-bit binary encoding.
  - Migration snapshots (`account_management_migration.cpp`, `account_management_internal.cpp`) still use the binary codec, so a rollback of a re-migration narrows wide ids in the restored history; documented, not changed.
  - Existing JSON records already written with narrowed ids are left as they are; no consumer reads the field.
- Tests: new `src/tests/exploit_record_tests.cpp` (7 pins, including the remote-caster regression and the four-argument form no longer walking `combat_list`); three new `ExploitsJson` pins (wide id through JSON, 80-byte legacy record with narrowing, hand-built legacy offsets); `DbLoader.AccountNativeWideVictimIdSurvivesAppendingAnotherRecord` using faked `account_cache` resolvers so it runs under QEMU.

- Follow-up findings verified and addressed (2026-09-15, later the same day):
  - Missing primary attribution suppressed valid contributors and XP: `die()` collected contributors, pkill/PK records and XP only inside `if (killer)`. A non-lethal poison tick that incapacitates a victim clears the victim's own target but not its opponents', so a later lethal tick whose poisoner no longer resolves reached `die()` with a null killer and skipped everyone still fighting. Contributors are now built regardless of credit; `group_gain()` accepts a null killer and still pays fighters in the death room; death entries are written when the killing blow is a player's or nobody's. Pins: `GroupGain.PaysRoomFightersTheirShareWhenNobodyIsCredited` (fight_credit_tests). A `die()`-level PC death pin is not practical here (raw_kill saves player and object files and frees the PC), so the wiring is covered by the group_gain pin plus the existing kill_contributors null-primary pins.
  - Slot-plus-address identity: `register_npc_char()` (used for PCs too) recycles slots after a full wrap of the 64000-slot cursor, and the allocator can hand the freed block to the next character, so a caster snapshot, poison origin or affected_list entry could match an unrelated character. Added `char_data::registration_serial`, stamped by `set_char_exists(num, ch)` from a process-wide counter, and made all three validators (`caster_snapshot::resolve()`/`same_character_as()`, `resolve_poisoner()`, `affect_update()`'s snapshot) require it; `universal_list` nodes and the poison origin carry the serial they captured. Pins: `CasterSnapshot.ResolveRejectsTheSameSlotAndAddressOnceReRegistered`, `PoisonOrigin.ResolveReturnsNullptrAfterTheSlotIsReRegisteredAtTheSameAddress`, `AffectUpdateWalk.DoesNotUpdateACharacterWhoseSlotWasReRegisteredAtTheSameAddress`.
  - Test hygiene found on the way: `MageProcTest` (`src/tests/mage_tests.cpp`) engaged stack characters through `damage()`/`set_fighting()` and never cleared `combat_list`, leaving dangling stack pointers that crashed a later suite's `extract_char()` walk once stack layout shifted; its `TearDown` now resets `combat_list`/`combat_next_dude` like `damage_tests.cpp`. Diagnosed from the QEMU core dump with gdb in the amd64 `rots-dev:trixie-x64` image (the bullseye image cannot install gdb any more).
  - Vacuous credit signal: `RoomAffectTick.BlazeTickCreditsTheRecordedCasterWithoutEngagingIt` treated corpse gear movement as proof of PC credit, but a null killer moves the gear too. The pin now reads the credited identity back through Big Brother (an orc-friend victim's corpse records its killer; `is_corpse_protected()` is true only for a player killer) and a paired control, `BlazeTickWithAnUnresolvableCasterCreditsNobody`, shows the gear still moves with nobody credited. The requested PC death integration test (classification, penalties, XP and persisted records through one `die()`) is not practical in this suite for the reasons above; the pieces are pinned separately.

### Last Validation (this task)
- `cmake --build build --target ageland_tests` in the i386 container, then `./bin/tests`: `570` passed, `211` failed; the failing set is identical to a HEAD baseline built from `git archive HEAD` in `build/baseline-head` (account-directory scan does not resolve under QEMU, plus a few pre-existing 32-bit/RNG pins). No new failures.
- Stack-protector game binary rebuilt clean at `build/sp/bin/ageland`; container `rots-uaf-sp` restarted onto it and reached the game loop (login banner answered on 1024).
- `git diff --check` reports only the carriage returns on the new `src/db.h` lines; that file is CRLF throughout and the new lines match it.

- Deploy caveat: this is a one-way schema change for account-native `*.exploits.json`. A pre-widening binary rejects any history containing a `victim_id` outside `sh_int` range, which then also skips that character's post-record `save_char()`. Do not roll 4810/live back to an older binary once a wide-id kill has been recorded.

### Reviewer Status (this task)
- `Magus`: no blockers. Medium: the migration snapshot (`refresh_linked_character_snapshot`) still narrows wide ids on a rollback re-hydration; now documented at the site, behaviour unchanged (decision pending). Low: identical `__func__` tag on both overloads' SYSERR lines; messages now name the form. Low: per-contributor history rewrite plus autosave is pre-existing O(n); deferred.
- `Vincent`: no blockers. High: the four remaining `sprintf` writes into the 30-byte `chVictimName` field in the four-argument form (`EXPLOIT_STAT`/`MOBDEATH`/`ACHIEVEMENT`/`NOTE`, reachable via `rename_char()` and `wizutil note`) were converted to bounded `snprintf` and pinned. Medium: migration-snapshot narrowing (same as Magus; decision pending). Medium: one-way schema / rollback risk, recorded above as a deploy caveat. Low: the writer seam ships in the server binary like the existing `account_cache` seam; accepted per precedent.
- Follow-up round (three findings):
  - `Magus`: clear; one cosmetic note that `IS_NPC()` already null-guards, so the explicit null case in the death-record gate is documentation rather than logic. Kept explicit.
  - `Bazarat`: the null-killer death-record gate was extracted as `death_names_player_contributors()` and pinned (nullptr/player true, mob false); `do_rehash` rebuilds every character node with its registration serial and is now pinned (`AffectUpdateWalk.RehashRebuildsCharacterEntriesWithTheirRegistrationSerial`); the blaze credit pin now asserts its single-player precondition rather than adding a killer-id accessor to `big_brother` for tests only. Noted for later: `Crash_crashsave()` is the one real blocker for a `die()`-level PC pin (it writes unconditionally relative to the cwd), so a temp-cwd sandbox would make one possible; `wait_type_list` targets still use pointer-plus-`char_exists()` identity without a serial.
  - `Vincent`: clear; no null dereference on the uncredited paths and no missed serial site. Medium (test binary only): the Big Brother singleton was created only by the tick suite, so any earlier suite killing a player or orc-friend with a non-empty corpse would have dereferenced a null instance; `gtest_main.cpp` now creates it once at startup. Low: the serial counter is a 32-bit `long` on i386 and would wrap after ~2^31 registrations in one process; noted, not addressed.
- `Bazarat`: multi-contributor PK, empty-list, immortal-contributor, trophy `iIntParam`, and overlong-name pins were all added; the immortal check was also added to the overload itself so the pin guards the function rather than only its upstream caller.
...
## Integration Test Harness — slice 1 progress (2026-09-19)
Harness built and reviewed (docs/superpowers/plans/2026-09-19-integration-test-harness.md, Tasks 1-12 complete). The real server boots on a committed synthetic zone 11 (tests/integration/world/), logs account-native fixtures in through the account menu, forces ticks via a new `-t` `harness tick` command, and isolates each scenario with a fresh server. `make integration-unit` runs 34 host unit tests; `make integration` runs the scenarios via a Docker or local launcher under the shared /tmp/rots-docker-lock convention.
Boot test and 4 of 6 scenario checks pass against the real i386 build. Harness FINDING (pending owner triage): on account-native character rent/death, `refresh_account_backed_object_file` (src/objsave.cpp) fails to round-trip the binary object file to JSON for an empty-inventory character — "SYSERR: failed to refresh account-native object file ... Truncated objects data while reading follower record" (src/objects_json.cpp parse_follower_record). Not yet fixed; it is the branch owner's call whether to fix here.

Update (2026-09-19, later): full suite green — 41 passed (35 unit + 6 scenarios: boot x3, remote/unengaged poison, blaze-after-quit, remote-credit XP split) against the real i386 build. The poison and blaze scenarios exercise the branch's UAF/poison/credit fixes end to end. The object-refresh SYSERR remains an open finding (tolerated in the crash monitor with a reference, not masked). Slice 1 complete pending the final whole-change review. Harness commits: cdf311d..f009e8c (see docs/superpowers/plans/2026-09-19-integration-test-harness.md).

Update (2026-09-19, final): integration harness slice 1 complete and reviewed. Full suite: 41 passed, 1 xfailed (the poison-death timing race — a harness-determinism gap, not a branch bug; SPELL_POISON is_fast=0 so its DoT ticks only on the time-phase advanced by `harness tick`, which also runs point_update regen; deterministic poison death needs a slice-2 no-regen harness tick). The account-native object-refresh SYSERR remains an open, documented finding. Harness commits cdf311d..9647ac4 (net; the fast-block-gate experiment fe0c9b9-era was reverted by aabcc1f).

Update (2026-09-19, slice 3): CI job `integration-asan` added (docs/superpowers/specs/2026-09-19-ci-integration-asan-design.md): both targets built under AddressSanitizer on ubuntu-24.04, runtime proven by ldd and help=1, gtest non-blocking, integration suite blocking, run directories uploaded on every outcome. The shared install step now installs libcrypt-dev:i386, so the existing job links again (it had failed at the link since 2026-08-14); it now reaches the unit tests and fails there on four pre-existing OlogHaiHelpers gtest cases, an owner item outside this slice. First run: five runs; the first three surfaced pre-existing memory errors that AddressSanitizer caught and that were fixed on this branch (fread_string stack underflow on blank world-file lines, ProtocolDestroy new/free mismatch, empty-prompt read in game_loop), the fourth a fixed-sleep race in one scenario, the fifth green: 48 passed, 1 xfailed in 142 s, job 3 min 54 s end to end. Draft PR: https://github.com/returnoftheshadow/RotS_Live/pull/309.

Update (2026-09-21, fix wave / slice 2 Phase A): both CI jobs green on PR #309 from run 35550360528 (commit d40540a plus the gate flip that follows). The sanitized unit run went from 123 failing cases to 0 (806/806) and the plain job's smoke flow ran end to end for the first time on this branch. Server defects fixed: fread_string's size check left no room for its carriage-return append; idle rent (Crash_idlesave) never wrote the follower section and Crash_follower_save/load closed the caller's FILE (double fclose); purge idle-saved the purging immortal instead of the victim; the olog-hai two-handed multiplier was integer 3/2 (a no-op); write_player_text printed the encrypted password past its 10-byte global; obj_from_room dereferenced a null walker for an object missing from its room; room_data's constructor left most members uninitialised (it replaced a calloc); JsonReader now refuses temporaries. Test fixtures now allocate characters, object nodes and the player table the way the server frees them, and the act()/msdp paths get real room state. The plain job's smoke step needed the committed harness world (lib/world is git-ignored) and two script updates for prompts that changed after PR #274. The sanitized ctest step is now blocking and runs after the integration suite. The account-native object-refresh finding is closed (root cause: idle rent omitted the follower section; the crash monitor no longer tolerates it and a purge scenario exercises the path). Owner findings recorded, not fixed: chd.host printed with %s in write_player_text; the alias writer skips the length on an empty command; MageTestContext stack characters would be freed by extract_char; a dead ScopedPlayerTableEntry copy in account_management_tests.cpp. Plan: docs/superpowers/plans/2026-09-20-fix-wave.md; Phase B plan: docs/superpowers/plans/2026-09-20-slice-2-scenarios.md.

Update (2026-09-21, slice 2 complete): the integration scenario suite is complete and green on PR #309 (run 35590995029, commit 232d6b8): 68 scenarios in 615 s natively under AddressSanitizer, 812/812 unit tests, no xfail or xpass; the job takes 12 min 40 s against its 30-minute budget. Harness additions: `harness affects` (one forced tick per person affect behind harness_force_affect_phase; the room sweep still runs but blaze's roll is not forced and regen is skipped), a fifth roster character Harncaller (human mage with summon and blaze, needed because summon fails across sides), room 1136 Crevice Floor below arena room 1130 so earthquake's crevice opens deterministically, Transcript.abilities(), GameSession.expect_room, and the poison_support/blaze_support helper modules (regen allowance, death budgets, restore-per-tick loops, log-line waits). Scenario files: test_affect_expiry_with_death.py test_blaze_after_caster_gone.py test_blaze_after_quit.py test_boot.py test_earthquake_order.py test_kill_credit.py test_poison_attribution_when_poisoner_is_gone.py test_poison_punishment_player_poison_mob_fight.py test_poison_punishment_snake.py test_poison_remote_player.py test_purge_refreshes_object_file.py test_remote_credit_xp_split.py test_summon.py . Server defects the new scenarios exposed under the sanitizer, all fixed with gtests: affect_update_person read an expiring affect's type after freeing it (any anger expiry; since 2019); do_stat_room indexed obj_index[-1] for a corpse; plus the snake special was never bound (MOB_SPEC missing on the harness mob). Fixed in the same wave, found by the fix-wave code reviewer rather than the sanitizer: do_fame_war_bonuses wrote through a freed affect (6ff901a). Scenario-author notes recorded in the plan ledger: wizset level does not recompute an NPC's willpower and wizset maxhit is inert for NPCs; do_quit refuses while SPELL_ANGER lingers; a magus renders as "*an Uruk*" to non-magi observers; kill credit for a mob death is observable only through the XP share line (no exploit record is written for a mob's death). Plans: docs/superpowers/plans/2026-09-20-fix-wave.md (Phase A) and docs/superpowers/plans/2026-09-20-slice-2-scenarios.md (Phase B).

Update (2026-09-21, slice 2 final review closed): the Phase B whole-range review and its single fix wave are done; CI green at ab52d0c (68 scenarios, 812/812 sanitized units, sanitized ctest blocking). The fix wave replaced the last blind waits with event waits (earthquake waits for the caster's damage line, summon for the "Closing link" log line, expect_room drains instead of sleeping), moved the shared combat waits into scenarios/combat_support.py, and trimmed comments. One correction from the re-review: `wizset maxhit` does raise a PC's max hit (affect_total recomputes it from constabilities.hit), so the earthquake scenario keeps that line; blaze_support's docstring had understated the term. Parked for the owner: test_affect_expiry_with_death's retry budget is bounded by blaze duration (~36 ticks) and wizset cannot raise a profession level, so a rare all-miss run fails loudly (fix path: fixture mage level or a harness profession-level command). Owner finding: do_fame_war_bonuses re-creates SPELL_FAME_WAR after a rank drop-out, now reachable since the UAF fix, and hands tier-4 bonuses to anyone ranked 11 or worse. The PR #309 description now carries all three phases. Ledger: .superpowers/sdd/2026-09-20-slice-2-scenarios/progress.md.
