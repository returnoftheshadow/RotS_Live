...
- Latest completed slice: JavaScript API legacy command-helper foundation slice. `RotS.Script` now exposes bounded `MutationResult` helpers for `do_wait`, `do_say`, `send_to_char`, `send_to_room`, `load_obj`, and `do_give` plus camelCase aliases. Live QuickJS queues strict `command` mutation envelopes for those helpers, dispatch accepts only the supported operation/argument shapes, and BuilderClient fallback typings plus offline execution mirror the helper names with command-event logs. This slice establishes compile/runtime acceptance for executable examples; live object creation, inventory transfer, output delivery, and continuation side effects remain follow-up audited helper-apply slices.
- Latest validation: focused C++ API/artifact/runtime/dispatch tests passed (`38` tests), focused BuilderClient compiler/offline/fallback/Monaco tests passed (`103` tests), BuilderClient `npm run typecheck` passed, full root `make test -j16` passed (`1611/1611`), and BuilderClient full `npm test -- --run` passed (`49` files, `604` tests). New review agents could not be spawned because the subagent thread pool was full; the slice still incorporates Magus, Vincent, and Bazarat's prior boundary findings by avoiding generic command passthrough and documenting live side-effect apply work as follow-up.
- Current slice: JavaScript API legacy command-helper executable examples slice. Add concrete TypeScript example files under `BuilderClient`, reference them from `docs/shape_script.md`, and verify each example compiles and runs against BuilderClient offline fixtures using only helpers accepted by the live runtime.
- Next slice: JavaScript API legacy command-helper live side-effect apply slice. Promote safe apply semantics for output helpers first (`do_say`, `send_to_char`, `send_to_room`) with bounded text, target validation, recursion/depth handling, audit, and parity tests before object load/give or wait continuations.
