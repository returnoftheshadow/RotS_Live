...
- Latest completed slice: JavaScript API `Character.followers`/`Character.master` getter assessment slice. `Character.followers` is now a frozen read-only `readonly CharacterRelationshipSnapshot[]` and `Character.master` is a frozen nullable `CharacterRelationshipSnapshot | null` through server API mapping, generated API contract/docs/typings, live QuickJS character snapshots, live game adapter snapshots, BuilderClient offline fixtures, fallback typings, and fixture validation. Relationship snapshots are shallow, invocation-local, capped at 100 follower-list nodes, require live reciprocal follow/master links in the adapter, expose identity/race/vnum/level/NPC/player flags plus `isValid()`, and deliberately exclude raw `follow_type` nodes, recursive room/equipment/inventory/follower/master handles, and setter methods. `Character.setFollowers` remains unsupported and `Character.setMaster` remains deferred because follow/unfollow changes need explicit helpers for back-pointers, follower caps, charm/orc/tamed behavior, loop prevention, group interactions, movement propagation, persistence, rollback, and audit semantics.
- Latest validation: passed for the `Character.followers`/`Character.master` getter assessment slice after Magus, Vincent, and Bazarat review follow-up. Focused C++ API/adapter/runtime/artifact tests passed (`151` tests), full root `make test -j16` passed (`1525/1525`), BuilderClient focused offline/IPC validation tests passed (`76` tests), BuilderClient `npm run typecheck` passed, BuilderClient full `npm test -- --run` passed (`49` files, `510` tests), and root/BuilderClient `git diff --check` passed.
...
- Latest validation: documentation-only classification slice; root/BuilderClient `git diff --check` should pass before commit.
- Current slice: JavaScript API `Character.mount`/`Character.group` getter assessment slice. Determine safe relationship snapshots for mounts and groups while keeping movement, follow, and group membership mutation behind explicit future helpers.
- Next slice: JavaScript API remaining character relationship/state getter audit. Confirm whether any remaining `char_data` relationship/state fields after mount/group can be safely exposed as read-only snapshots, otherwise keep them documented as deferred/unsupported with tests.
