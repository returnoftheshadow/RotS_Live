...
- Current slice: JavaScript API next safe object scalar classification slice. Decide whether `GameObject.setRarity(value: number)` should be promoted as a narrow `obj_data.obj_flags.rarity` setter or remain deferred, using persistence, value-domain, side-effect, authority, and offline parity criteria.
- Next slice: JavaScript API next safe object scalar implementation or deferral slice based on the rarity classification.
