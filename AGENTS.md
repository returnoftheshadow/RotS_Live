...
- Latest completed slice: JavaScript API `Room.level` setter promotion slice. `Room.setLevel(value: number): MutationResult` is now callable through server API mapping, generated typings/manifest/docs, live QuickJS integer validation, target-scoped persistent mutation dispatch to byte `room_data.level`, BuilderClient offline fixtures, and parity tests; callable values are integers `0` through `100`, persisted as the room-file level scalar, and used by legacy same-level room filtering.
- Latest validation: focused C++ mapping/runtime/dispatch/artifact/manifest checks passed (`23` tests), full root `make test -j16` passed (`1463/1463`), BuilderClient `npm run typecheck` passed, BuilderClient focused offline/compiler/IPC tests passed (`59` tests), BuilderClient full `npm test -- --run` passed (`49` files, `485` tests), BuilderClient `npm run build` passed, and root/BuilderClient `git diff --check` passed.
...
- Current slice: JavaScript API `Room.sectorType` setter promotion slice. Promote `Room.setSectorType(value: string): MutationResult` with canonical sector-name validation, target-scoped persistent room mutation authority, live/offline parity, generated typings/docs, and dispatch atomicity coverage while documenting immediate movement, swimming, weather/sunlight, tracking, and MOB_STAY_TYPE side effects.
- Next slice: JavaScript API room flag/alignment deferral or implementation slice after `Room.setSectorType`, likely keeping room flags and alignment deferred until affect synchronization and persistence policy are explicit.
