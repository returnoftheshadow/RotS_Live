...
- Current slice: classify the next candidate setter group from character relationships/state and promote the first field with clear persistence, validation, side-effect, ownership, live QuickJS, BuilderClient offline, and atomic dispatch semantics.
- Next slice: JavaScript API character relationship/state setter classification slice, starting with `Character.room`, follower/master relationships, equipment/inventory, combat wait/interrupt state, and special procedure busy state.
