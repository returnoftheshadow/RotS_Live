...
- Latest completed slice: JavaScript API next safe setter group selection slice. The next implementation target is `GameObject.setLevel(value: number)` over `obj_data.obj_flags.level`, because the scalar is already exposed read-only as `ctx.object.flags.level`, is persisted by the object loader/shaper/writer path alongside rarity/material/script number, and has lower immediate side-effect risk than object weight, cost, rent cost, timer, rarity, material, item type, wear flags, extra flags, or raw value slots.
- Current slice: JavaScript API `GameObject.level` setter promotion slice. Promote `GameObject.setLevel(value: number): MutationResult` with integer `0` through `100` validation, target-scoped persistent object mutation authority, live/offline parity, generated typings/docs, and dispatch atomicity coverage while updating the nested `flags.level` snapshot value.
- Next slice: JavaScript API next safe object scalar classification slice after `GameObject.setLevel`, likely deciding whether `rarity` is safe or should remain deferred with the other object economy/behavior fields.
