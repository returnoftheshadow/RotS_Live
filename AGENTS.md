...
- Current slice: select the next implementable setter from the mapped `char_data`, `obj_data`, `room_data`, and `zone_data` catalog.
- Next slice: classify the next candidate setter group and promote the first field with clear persistence, validation, side-effect, ownership, live QuickJS, BuilderClient offline, and atomic dispatch semantics.
