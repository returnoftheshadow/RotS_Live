...
- Latest completed slice: JavaScript API next safe object scalar classification slice. The next implementation target is `GameObject.setRarity(value: number)` over `obj_data.obj_flags.rarity`, because the scalar is already exposed read-only as `ctx.object.flags.rarity`, is persisted by the object loader/shaper/writer path alongside level/material/script number, uses `ubyte` storage, and has lower immediate side-effect risk than object weight, cost, rent cost, timer, material, item type, wear flags, extra flags, or raw value slots. The callable domain should be integer `0` through `255`.
- Current slice: JavaScript API `GameObject.rarity` setter promotion slice. Promote `GameObject.setRarity(value: number): MutationResult` with integer `0` through `255` validation, target-scoped persistent object mutation authority, live/offline parity, generated typings/docs, hidden-helper shadowing, and dispatch atomicity coverage while updating the nested `flags.rarity` snapshot value.
- Next slice: JavaScript API next safe object scalar classification after `GameObject.setRarity`, likely deferring weight/cost/timer/material/type/flag bitsets until side-effect policies are explicit.
