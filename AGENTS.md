...
- Current slice: JavaScript API room alignment/flags setter classification slice. Decide and document whether `Room.alignment` and `Room.flags` stay deferred or can be implemented safely, including persistence path, filtered flag vocabulary, transient/internal bit policy, and gameplay side effects.
- Next slice: if room alignment/flags remain deferred, select the next implementable setter from the mapped `char_data`, `obj_data`, `room_data`, and `zone_data` catalog; otherwise promote the chosen room setter with live/offline parity and atomic dispatch coverage.
