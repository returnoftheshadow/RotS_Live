...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Final output-helper bridge parity now returns inline `MutationResult` outcomes from live dispatch for `do_say`, `send_to_char`, and `send_to_room`. Output helpers are audited before descriptor reachability is exposed, so `audit-rejected` takes precedence over `no-recipient`. Invalid or stale targets return `invalid-target`, command-audit failures return `audit-rejected`, and existing targets with no connected playing descriptor recipient return `no-recipient` without queuing output. Accepted output helpers are audited once, queued with an internal bridge marker, and still write descriptor output only at the final transaction category after setter, room-flag, object, and wait side effects have passed validation; script output uses the same connected-playing descriptor filter at apply time. BuilderClient offline fixtures still validate handle/text shape and log output command events, but they currently assume valid fixture handles are reachable and do not emulate descriptor `no-recipient` state.
    - Next command-helper result bridge slice: add the BuilderClient/server parity fixture catalog for command-result branches that still need richer offline state, starting with output descriptor reachability, wait-list state, and object prototype catalogs.
