...
- CLI-first builder workflow requirements before Electron implementation:
  ...
  - implemented initial CLI slices include `doctor`, `typecheck`, `build`, `fixture run`, no-write `package` metadata, explicit `package --write` artifact creation under `packages/`, and a no-network `publish stage` scaffold; the next CLI slice is an explicit transport-enabled publish-stage mode that sends only when requested
