...
- Publish authentication and authorization model:
  ...
  - idempotency keys must be operation-specific and actor-bound; retrying the same request after a transport failure can return the prior result only when authorization, digest, live checksum, manifest, package version, and permission scope still match
