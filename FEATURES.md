...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current object relationship/lifecycle setter classification keeps `GameObject.setRoom`, `GameObject.setCarriedBy`, `GameObject.setContainer`, `GameObject.setContents`, and `GameObject.setTouched` deferred or unsupported. The existing read-only `GameObject.room`, `GameObject.carriedBy`, and `GameObject.container` getters remain available. `GameObject.container` exposes only a nullable shallow frozen `EquipmentObjectSnapshot` when the root object is out of direct room placement, the containing object is live, and its contents list reciprocally includes the root object within bounded cycle-safe traversal; nested container snapshots do not recurse into another `container` and expose no setter methods. Object placement writes must go through explicit audited movement/load/extract helpers because raw field assignment would bypass room contents, inventory/equipment lists, carried weight, riding mount carry weight, light recounting, player crash-save flags, container capacity/counting, nested weight propagation, decay/extraction paths, cycle prevention, stale linked-list protection, and worn-vs-carried ownership checks. `GameObject.touched` remains unavailable because it is runtime/player-interaction state initialized on prototype instantiation and changed by player/admin object actions, not an object shaper persisted prototype field.
