...
- The Rust proxy is the client-facing API boundary for BuilderClient. It runs on the same host as the game server, accepts BuilderClient API traffic, enforces request/upload limits and transport policy, and forwards only trusted local/private-host requests to the game engine. The proxy-to-game path still needs an internal trust boundary such as localhost-only binding plus a shared local secret, Unix socket permissions, or an equivalent reviewed mechanism.
