...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Legacy command-helper foundation now exposes bounded `RotS.Script` helpers for `do_wait`, `do_say`, `send_to_char`, `send_to_room`, `load_obj`, and `do_give` plus camelCase aliases. The live runtime queues strict `command` mutation envelopes, dispatch validates only the supported operation/argument shapes, and BuilderClient fallback typings plus offline execution mirror the helper names with command-event logs. This makes BuilderClient examples compile and execute against a live-accepted helper shape; output delivery, object creation/placement, inventory transfer, and wait continuation apply semantics remain separate audited follow-up slices before those side effects are considered fully game-applied.
