...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next character movement helper slice: design `doFollow(follower, leader)` only after reciprocal follower/master links, loop prevention, charm/tame/ownership policy, follower caps, mount/group interactions, movement propagation, combat XP sharing, audit fields, rollback behavior, and offline hidden relationship state are pinned.
