...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current first setter coverage includes safe text fields, the printable zone map symbol, and the bounded zone map coordinates: `GameObject.setName`, `GameObject.setDescription`, `GameObject.setShortDescription`, `GameObject.setActionDescription`, `Room.setName`, `Room.setDescription`, `Zone.setName`, `Zone.setDescription`, `Zone.setMap`, `Zone.setSymbol`, `Zone.setX`, and `Zone.setY`. These methods remain available on root and nested handle snapshots for local script logic and offline parity. Live package dispatch now collects successful setter calls through a private mutation envelope and can apply them after a successful handler run to resolved live `obj_data`, `room_data`, and `zone_data` fields, but persistent application is fail-closed unless the dispatch caller provides a complete target-scoped mutation authority context with explicit allow, builder account id, eligible immortal character id, target zone, and decision evidence. The legacy trigger facade threads that structured context for server-backed builder/test execution paths while keeping normal gameplay dispatch read-only by default. Persistent object writes only resolve invocation-root `ctx.object`/`ctx.weapon` handles until per-instance object tokens exist; room/zone/object targets must resolve to the authorized zone, carried/worn/container object authority must prove live list membership, `Zone.setMap` rejects zone-file terminators and leading map record markers, `Zone.setSymbol` accepts only one printable ASCII character, and `Zone.setX`/`Zone.setY` accept only integers from `0` through `25` inclusive. Mutation envelopes retain a `valueKind` discriminator and dispatcher validation binds text, null, symbol, and coordinate values to the correct field family so forged numeric/text envelopes fail closed. Committed global zone redraw skips stale out-of-range loaded coordinates instead of indexing the map buffer. Malformed targets, excessive mutation counts, invalid text/scalars, unauthorized dispatch, missing authority evidence, failed handlers, and thrown scripts fail closed without partial live writes. Binding that context directly to live BuilderClient session and zone-ownership resolution remains a follow-up slice.
    - Current scalar setter classification keeps `Zone.minimumLookLevel`, `Zone.lifespan`, `Zone.resetMode`, and `Zone.level` deferred because they affect visibility, reset timing, or gameplay balance and need explicit authority/domain rules before becoming callable. Zone age, vnum/topology, owner, power, reset-command, and linked-list fields remain unsupported or internal-only.
