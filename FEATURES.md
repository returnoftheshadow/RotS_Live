...
- Package integrity model:
  ...
  - initial live package pointer/store model is implemented in `src/js_live_package_store.{h,cpp}` without live gameplay dispatch or public activation:
    - immutable live package records are stored from staged records only after package/identity/checksum/audit shape validation, and package ids are normalized to server-owned staged identity ids before registry snapshots
