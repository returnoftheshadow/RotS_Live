...
- Publish protocol and server authority:
  ...
  - the proxy translates BuilderClient publish routes from `/api/builder/js/{status,stage,activate,rollback}` to the game-owned `/api/js-scripts/{status,stage,activate,rollback}` routes; account login/session, logout, and manifest routes are separate game-owned endpoint surfaces so session/manifest traffic cannot accidentally enter publish handlers
  - the game-owned manifest route is `GET /api/builder/js/manifest`; it is trusted-proxy only, accepts no body, rejects query/path smuggling, and returns the server-exported trigger/API manifest used by BuilderClient generated typings, IntelliSense, LSP configuration, documentation refresh, and offline validation caches
