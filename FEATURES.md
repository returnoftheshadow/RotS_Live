...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current first setter coverage is invocation-snapshot-only for safe text fields: `GameObject.setName`, `GameObject.setDescription`, `GameObject.setShortDescription`, `GameObject.setActionDescription`, `Room.setName`, `Room.setDescription`, `Zone.setName`, and `Zone.setDescription`. These methods are available on root and nested handle snapshots for local script logic and offline parity, but persistent live world mutation and ownership checks are deferred to the server-side mutation bridge slice.
