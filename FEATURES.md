...
- CLI-first builder workflow requirements before Electron implementation:
  ...
  - implemented initial CLI slices include `doctor`, `typecheck`, `build`, `fixture run`, no-write `package` metadata, explicit `package --write` artifact creation under `packages/`, no-network `publish stage` validation, explicit `publish stage --send`, `publish status`, `publish activate`, `publish rollback`, and `--bearer-token-stdin` safer token input for publish commands
  ...
  - next offline-first CLI slice should add a read-only `docs` command that reports the local generated API documentation/provenance through deterministic JSON without requiring auth, network, upload, or server state mutation
