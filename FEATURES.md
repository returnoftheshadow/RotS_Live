...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next command-helper result bridge slice: retrofit `do_wait` so builders get final `MutationResult` codes for authority, audit, invalid wait state, and accepted wait scheduling without needing a separate preflight helper, while preserving transaction apply ordering and no-partial-write guarantees.
