...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Legacy command-helper foundation now exposes bounded `RotS.Script` helpers for `do_wait`, `do_say`, `send_to_char`, `send_to_room`, `load_obj`, and `do_give` plus camelCase aliases. The live runtime queues strict `command` mutation envelopes, dispatch validates only the supported operation/argument shapes, and BuilderClient fallback typings plus offline execution mirror the helper names with command-event logs. Builder-facing examples now compile and execute against live-accepted helper shapes, and live dispatch applies output, explicit-target object load/give, and bounded no-continuation wait-state side effects.
    ...
    - BuilderClient executable shape-script examples now live under `BuilderClient/examples/shape-script/` and are linked from `docs/shape_script.md`. The example corpus is tested as documentation contract: every linked file must compile through the TypeScript pipeline and run through BuilderClient offline fixtures while emitting the expected command-helper events for `do_say`, `send_to_char`, `send_to_room`, `do_wait`, `load_obj`, and `do_give`.
