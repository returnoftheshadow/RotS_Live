...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Character movement helper foundation now has server-owned operation metadata for `loadMob`, `teleportChar`, `teleportCharOnly`, `teleportCharToTargetRoom`, `extractChar`, `doFollow`, and `doFlee`, mapping the legacy `LOAD_MOB`, `TELEPORT_CHAR`, `TELEPORT_CHAR_X`, `TELEPORT_CHAR_XL`, `EXTRACT_CHAR`, `DO_FOLLOW`, and `DO_FLEE` families into modern TypeScript helper names. The metadata records target policy, authority, side effects, audit fields, stable diagnostics, rollback expectations, offline fixture parity, and adversarial tests. `teleportCharOnly` is now promoted as the first callable helper in this family; the remaining helpers stay absent from generated typings, fallback typings, live QuickJS handles, and offline runtime functions until live/offline implementations are promoted together. `loadMob` must start from a hidden prototype catalog and not return mutable live handles until character handle lifetime tokens exist; teleport variants must distinguish follower propagation from leader-only movement and target-room resolution; `extractChar` must reject player/account-backed characters unless a future admin path is designed; `doFollow` must own reciprocal follow/master links and loop prevention; and `doFlee` must avoid generic command execution while modeling deterministic eligible-exit selection.
    - `teleportCharOnly(character, room)` now promotes the narrow legacy `TELEPORT_CHAR_X` path through generated typings, live QuickJS command bridging, dispatch preflight, transaction apply, BuilderClient fallback typings, offline result parity, and builder docs. Live dispatch requires persistent target-zone authority, live invocation-local character and room handles with current rooms, command audit, non-blocked target rooms, and no `NO_TELEPORT`; accepted apply stops riding, moves only the selected character, leaves followers behind, does not run movement triggers, and is kept after object, room-flag, setter, and wait phases so later fallible phases do not require mount/riding rollback. BuilderClient offline fixtures return `blocked-room` and `no-teleport` from room flags and keep object/drop helpers aligned to live category order rather than source-order teleport state. `loadMob`, broader teleport variants, `extractChar`, `doFollow`, and `doFlee` remain planned.
    - Next command-helper result bridge slice: continue closing BuilderClient/server parity gaps for command-result branches that need richer offline state, starting with offline object-placement/inventory-transfer state mutation for accepted `load_obj` and `do_give` helper events.
