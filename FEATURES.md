...
- Publish authentication and authorization model:
  - publish credentials are separate from normal in-game play sessions and must be issued by the server with explicit scope, actor identity, builder account id, zone/vnum authority, expiration, token id, issued-at time, and revocation state
