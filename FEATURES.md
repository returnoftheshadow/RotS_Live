...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current scalar setter classification keeps `Zone.minimumLookLevel` deferred because it affects visibility and still needs its persisted builder edit path mapped. Zone age, vnum/topology, owner, power, reset-command, and linked-list fields remain unsupported or internal-only.
