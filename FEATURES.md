...
- Read-only character/object/room parameter parity:
  ...
  - Remaining read-only drift to close before side effects: normalize BuilderClient offline fixture handles and context fields to the same live-shaped read-only objects QuickJS receives, then wire any remaining live backing for trigger-specific `ScriptContext` payload fields such as broader equipment/death payload variants and any deferred non-Mudlle generic targets against the legacy ASIMA call sites. Until each live call site is wired, documented unbacked fields must stay present as immutable `null` or an explicit empty default rather than being omitted.
