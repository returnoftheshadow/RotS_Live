...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - The next helper foundation layer must add a reusable mutation gate harness before any deferred helper is promoted. The harness must define opaque target tokens, target-scoped authority, stale-handle denial, per-helper mutation limits, sanitized `MutationResult` failures, audit-before-mutation ordering, and atomic no-partial-write behavior shared by live execution and BuilderClient offline fixtures.
