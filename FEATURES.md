...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next command-helper result bridge slice: retrofit output helpers (`do_say`, `send_to_char`, and `send_to_room`) so builders get final `MutationResult` codes for target liveness, text validation, audit rejection, descriptor/room applicability, and accepted output queuing without relying on deferred whole-transaction failure.
