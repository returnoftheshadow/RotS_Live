...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next character movement helper slice: design `extractChar(character)` only after player/account-backed character rejection, NPC extraction side effects, follower/master cleanup, combat/riding cleanup, audit fields, rollback/no-mixed-batch policy, and offline fixture state are pinned.
