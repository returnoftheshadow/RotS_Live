...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next selected setter: `GameObject.setRarity(value: number)` should update `obj_data.obj_flags.rarity` through the same guarded mutation-result path. The object loader, shaper save/apply path, and object file writer persist this scalar with level, material, and script number, and it is already visible as `ctx.object.flags.rarity`. V1 should accept only integer values `0` through `255`, require target-scoped persistent object authority, and update the invocation snapshot `flags.rarity` for offline/live parity. Object weight, cost, cost-per-day, timer, material, item type, wear flags, extra flags, and value slots remain deferred until each field has a side-effect and value-domain policy; weight can affect encumbrance/container totals, cost fields affect economy/rent behavior, timer affects decay, material/type/value slots affect item behavior, and flag bitsets need named allowlists plus equipment/combat/restriction recalculation rules.
