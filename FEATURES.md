...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current first setter coverage includes safe text fields: `GameObject.setName`, `GameObject.setDescription`, `GameObject.setShortDescription`, `GameObject.setActionDescription`, `Room.setName`, `Room.setDescription`, `Zone.setName`, `Zone.setDescription`, and `Zone.setMap`. These methods remain available on root and nested handle snapshots for local script logic and offline parity. Live package dispatch now collects successful setter calls through a private mutation envelope and can apply them after a successful handler run to resolved live `obj_data`, `room_data`, and `zone_data` text fields, but persistent application is fail-closed unless the dispatch caller provides a complete target-scoped mutation authority context with explicit allow, builder account id, eligible immortal character id, target zone, and decision evidence. The legacy trigger facade threads that structured context for server-backed builder/test execution paths while keeping normal gameplay dispatch read-only by default. Persistent object writes only resolve invocation-root `ctx.object`/`ctx.weapon` handles until per-instance object tokens exist; room/zone/object targets must resolve to the authorized zone, carried/worn/container object authority must prove live list membership, and `Zone.setMap` rejects zone-file terminators and leading map record markers. Malformed targets, excessive mutation counts, invalid text, unauthorized dispatch, missing authority evidence, failed handlers, and thrown scripts fail closed without partial live writes. Binding that context directly to live BuilderClient session and zone-ownership resolution remains a follow-up slice.
    - Current scalar setter classification keeps `Zone.x`, `Zone.y`, and `Zone.symbol` documented as planned validated setters only; they are exported as documentation-only, omitted from generated/fallback typings, and absent from live/offline runtime handles until validation and persistence are implemented. `Zone.minimumLookLevel`, `Zone.lifespan`, `Zone.resetMode`, and `Zone.level` remain deferred because they affect visibility, reset timing, or gameplay balance. Zone age, vnum/topology, owner, power, reset-command, and linked-list fields remain unsupported or internal-only.
