...
- Publish protocol and server authority:
  ...
  - the proxy translates BuilderClient publish routes from `/api/builder/js/{status,stage,activate,rollback}` to the game-owned `/api/js-scripts/{status,stage,activate,rollback}` routes; account login/session, logout, and manifest routes require separate game-side endpoint slices
