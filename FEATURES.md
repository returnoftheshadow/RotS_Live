...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next selected room setter: `Room.setSectorType(value: string)` should update `room_data.sector_type` through the guarded mutation-result path. The room loader, room shaper apply path, and room writer persist this scalar, and it is already visible as `ctx.room.sectorType` through server-owned sector names. V1 should accept only canonical live sector names from the server vocabulary, reject `"Unknown"` and raw numeric values, require target-scoped persistent room authority, and update the invocation snapshot `sectorType` for offline/live parity. Documentation must call out immediate gameplay side effects: movement cost, swim/no-swim/underwater checks, MOB_STAY_TYPE movement, weather/sunlight messaging, track aging, and any other room-sector consumers can change as soon as the mutation commits.
