...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Final `do_wait` bridge parity now returns inline `MutationResult` outcomes from live dispatch for missing authority, invalid live host, already-waiting host state, command-audit rejection, and accepted success. Accepted waits are audited once, queued with an internal bridge marker, and scheduled only once during transaction apply after final host/zone/not-already-waiting rechecks. Already-waiting hosts now fail closed instead of silently no-oping at transaction apply, so builders can branch on `already-waiting` when the live server can classify it before queuing. BuilderClient offline fixtures still validate pulse bounds and log successful wait command events, but persistent wait-list state and offline `already-waiting` parity remain deferred until fuller offline state emulation exists.
    ...
    - Next command-helper result bridge slice: add the BuilderClient/server parity fixture catalog for command-result branches that still need richer offline state, starting with wait-list state and object prototype catalogs.
