...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next room setter classification: decide whether `Room.alignment` and `Room.flags` remain deferred or can be implemented safely. `Room.alignment` needs a confirmed persistence/editing path and gameplay side-effect policy before mutation. `Room.flags` needs a filtered builder-facing vocabulary, internal/transient bit exclusions, room affect/synchronization rules, and explicit behavior for movement/combat/teleport/security flags before becoming callable.
