...
- CLI-first builder workflow requirements before Electron implementation:
  ...
  - implemented initial no-write/read-only command slices include `doctor`, `typecheck`, `build`, `fixture run`, and no-write `package` metadata; the next CLI slice is an explicit package artifact-write mode that writes deterministic package JSON under `packages/` using safe project-relative output paths and atomic replacement
