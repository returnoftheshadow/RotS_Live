...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current scalar setter classification keeps `Zone.minimumLookLevel` deferred because it affects visibility and the inspected legacy zone loader, saver, and shaper paths do not currently persist or edit `zone_data.min_level_look` with the other zone scalar fields. Zone age, vnum/topology, owner, power, reset-command, and linked-list fields remain unsupported or internal-only.
