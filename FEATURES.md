...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Legacy output command-helper live application now supports `do_say`, `send_to_char`, and `send_to_room` as bounded, current-trigger-context output helpers. The dispatcher resolves only live script role handles and loaded room handles, writes through the existing descriptor output queues, allows output-only helper scripts without persistent mutation authority, and rejects forged or stale output targets without writing. `load_obj`, `do_give`, and `do_wait` stay validated until their own audited slices are completed.
