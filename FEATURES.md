...
- CLI-first builder workflow requirements before Electron implementation:
  ...
  - implemented initial CLI slices include `doctor`, `typecheck`, `build`, `fixture run`, no-write `package` metadata, explicit `package --write` artifact creation under `packages/`, no-network `publish stage` validation, explicit `publish stage --send`, `publish status`, `publish activate`, and `publish rollback`
  ...
  - next CLI credential hardening must add a safer token input path for publish commands, such as stdin, OS credential/session lookup, or a dedicated login/logout command, so examples and normal builder workflows do not require bearer tokens in argv or shell history
