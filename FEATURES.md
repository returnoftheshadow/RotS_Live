...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Next command-helper result bridge slice: add the BuilderClient/server parity fixture catalog for command-result branches that still need richer offline state, starting with object prototype catalogs for `load_obj` `not-found` and prototype-weight/capacity branches.
