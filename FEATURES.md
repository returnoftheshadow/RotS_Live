...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Current first setter coverage includes safe text fields: `GameObject.setName`, `GameObject.setDescription`, `GameObject.setShortDescription`, `GameObject.setActionDescription`, `Room.setName`, `Room.setDescription`, `Zone.setName`, and `Zone.setDescription`. These methods remain available on root and nested handle snapshots for local script logic and offline parity. Live package dispatch now collects successful setter calls through a private mutation envelope and can apply them after a successful handler run to resolved live `obj_data`, `room_data`, and `zone_data` text fields, but persistent application is fail-closed unless the dispatch caller provides a complete mutation authority context with explicit allow, builder account id, eligible immortal character id, target zone, and decision evidence. The legacy trigger facade threads that structured context for server-backed builder/test execution paths while keeping normal gameplay dispatch read-only by default. Malformed targets, excessive mutation counts, invalid text, unauthorized dispatch, missing authority evidence, failed handlers, and thrown scripts fail closed without partial live writes. Binding that context directly to live BuilderClient session and zone-ownership resolution remains a follow-up slice.
