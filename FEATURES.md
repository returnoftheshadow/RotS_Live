...
- Publish authentication and authorization model:
  ...
  - CLI/Electron authentication must store refresh/session material only in OS credential storage; when unavailable, tokens are memory-only and commands require reauthentication; project files, packages, fixtures, logs, crash reports, and exported diagnostics must never contain tokens or refresh material
...
- Harden the Electron app:
  ...
  - crash reports, exported logs, IPC traces, devtools exports, local recent-file state, and problem/output panels must redact tokens, account identifiers, local absolute paths, source snippets from private scripts, sourcemap paths, and live player text; full local paths may be displayed only inside the active local UI when not exported or persisted
