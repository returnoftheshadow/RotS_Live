...
- Initial server-owned host API contract implementation:
  ...
  - Initial accessor/mutator rollout order:
    ...
    - Character movement helper foundation now has server-owned operation metadata for `loadMob`, `teleportChar`, `teleportCharOnly`, `teleportCharToTargetRoom`, `extractChar`, `doFollow`, and `doFlee`, mapping the legacy `LOAD_MOB`, `TELEPORT_CHAR`, `TELEPORT_CHAR_X`, `TELEPORT_CHAR_XL`, `EXTRACT_CHAR`, `DO_FOLLOW`, and `DO_FLEE` families into modern TypeScript helper names. The metadata records target policy, authority, side effects, audit fields, stable diagnostics, rollback expectations, offline fixture parity, and adversarial tests. `teleportChar` and `teleportCharOnly` are now promoted as callable helpers in this family; the remaining helpers stay absent from generated typings, fallback typings, live QuickJS handles, and offline runtime functions until live/offline implementations are promoted together. `loadMob` must start from a hidden prototype catalog and not return mutable live handles until character handle lifetime tokens exist; the target-room teleport variant must pin destination resolution; `extractChar` must reject player/account-backed characters unless a future admin path is designed; `doFollow` must own reciprocal follow/master links and loop prevention; and `doFlee` must avoid generic command execution while modeling deterministic eligible-exit selection.
    - `teleportChar(character, room)` and `teleportCharOnly(character, room)` now promote the narrow legacy `TELEPORT_CHAR` and `TELEPORT_CHAR_X` paths through generated typings, live QuickJS command bridging, dispatch preflight, transaction apply, BuilderClient fallback typings, offline result parity, and builder docs. Live dispatch requires persistent target-zone authority, live invocation-local character and room handles with current rooms, command audit, non-blocked target rooms, and no `NO_TELEPORT`; accepted apply stops riding, does not run movement triggers, and is kept after object, room-flag, setter, and wait phases so later fallible phases do not require mount/riding rollback. `teleportChar` moves same-room NPC followers before the selected character while leaving player followers behind; `teleportCharOnly` moves only the selected character and leaves all followers behind. BuilderClient offline fixtures return `blocked-room` and `no-teleport` from room flags, record same-room NPC follower movement in hidden per-run state, and keep object/drop helpers aligned to live category order rather than source-order teleport state. `loadMob`, target-room teleport, `extractChar`, `doFollow`, and `doFlee` remain planned.
    - Next character movement helper slice: continue closing BuilderClient/server parity gaps for target-room movement with `teleportCharToTargetRoom(character, target)`, including destination resolution, stale target handling, result codes, and hidden offline movement state.
...
- World/entity mutation that must stay unsupported or admin-gated:
  - `SCRIPT_LOAD_MOB`, `SCRIPT_LOAD_OBJ_X`, `SCRIPT_EQUIP_CHAR`, `SCRIPT_EXTRACT_CHAR`, `SCRIPT_TELEPORT_CHAR_XL`, `SCRIPT_SET_EXIT_STATE`, `SCRIPT_CHANGE_EXIT_TO`, `SCRIPT_RAW_KILL`, and `SCRIPT_GAIN_EXP` remain unsupported for builder JavaScript v1 unless a future admin-grade permission/audit/rollback design explicitly enables one narrow operation. `SCRIPT_LOAD_OBJ`, `SCRIPT_OBJ_TO_CHAR`, `SCRIPT_OBJ_TO_ROOM`, `SCRIPT_OBJ_FROM_ROOM`, `SCRIPT_OBJ_FROM_CHAR`, `SCRIPT_EXTRACT_OBJ`, `SCRIPT_TELEPORT_CHAR`, and `SCRIPT_TELEPORT_CHAR_X` now have narrow promoted helper coverage and remain limited to those audited helper shapes rather than raw legacy command-row access.
- Continuations:
